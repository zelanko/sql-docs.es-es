---
title: Examinar un modelo Bayes Naive | Documentos de Microsoft
ms.custom: ''
ms.date: 03/06/2017
...
ms.locfileid: "36103221"
---
# <a name="browsing-a-naive-bayes-model"></a>Examinar un modelo Bayes naive
  Cuando se abre un modelo de Naïve Bayes con **examinar**, el modelo se muestra en un visor interactivo con cuatro paneles diferentes. El visor se usa para explorar las correlaciones y obtener información sobre el modelo y los datos subyacentes.  
...
##  <a name="BKMK_Tabs"></a> Explorar el modelo  
 ...
 Si desea experimentar con el Visor Naïve Bayes, use la [Asistente para clasificar &#40;complementos minería de datos para Excel&#41; ](classify-wizard-data-mining-add-ins-for-excel.md) asistente en la cinta de opciones de minería de datos, haga clic en el **avanzadas** y cambie el algoritmo que se utiliza el algoritmo Naïve Bayes  
  
 Para estos ejemplos, se usan los datos de origen en el libro de ejemplo y agrupa la columna **ingresos anuales**, en cinco grupos de ingresos, desde **muy pocos** a **muy alto**. Posteriormente, el modelo Bayes naive analiza los factores en correlación con cada categoría de ingresos.  
  
###  <a name="bkmk_DepNet"></a> Red de dependencias  
 Es la primera ventana que se va a utilizar el **red de dependencias**. Muestra de un solo vistazo las entradas que están estrechamente correlacionadas con el resultado seleccionado.  
...
1.  En primer lugar, haga clic en el resultado de destino, **ingresos anuales**, que se representa como un nodo en el gráfico.  
...
###  <a name="bkmk_AttProf"></a> Perfiles del atributo  
 El **perfiles del atributo** windows proporciona una indicación visual de cómo el resto de variables se relacionan con los resultados individuales.  
...
1.  Para ocultar algunos valores de forma que pueda comparar los resultados con mayor facilidad, haga clic en el encabezado de columna y arrástrelo a otra columna.  
     ...
     ![atributo perfiles en Visor Bayes Naive](media/dm13-nb-attprof.gif "perfiles en Visor Bayes Naive del atributo")  
...
###  <a name="bkmk_AttChar"></a> Características del atributo  
 El **características del atributo** vista es útil para efectuar un examen de una variable de resultado determinado y los factores que contribuyen.  
  
 ![atributo características en Visor Bayes Naive](media/dm13-nb-viewer.gif "atributo características en Visor Bayes Naive")  
...
###  <a name="bkmk_AttDisc"></a> Distinción del atributo  
 El **distinción del atributo** vista le ayuda a comparar dos resultados o un resultado frente a todos los demás resultados.  
  
 ![atributo distinción en Visor Bayes Naive](media/dm13-nb-attdisc.gif "atributo distinción en Visor Bayes Naive")  
...
1.  Use los controles, **valor 1** y **valor 2**, para seleccionar los resultados que desea comparar.  
     ...
     Por ejemplo, en este modelo había ciertos atributos interesantes en el grupo de ingresos baja, por lo que elegimos el grupo de ingresos más bajo en la primera lista desplegable y elige **todos los demás Estados** en la segunda lista desplegable.  
     ...
     Para ver las cifras exactas, haga clic en la barra de color y la vista de la **leyenda de minería de datos**.  
