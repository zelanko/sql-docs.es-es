...
## <a name="cell-properties-keyword-syntax"></a>Sintaxis de la palabra clave CELL PROPERTIES  
 Utilice la siguiente sintaxis para el `CELL PROPERTIES` palabra clave de MDX `SELECT` instrucción:  
...
|**FONT_FLAGS**|Máscara de bits que detalla los efectos de la fuente. Por ejemplo, el valor 5 representa la combinación de negrita (`MDFF_BOLD`) y subrayado (`MDFF_UNDERLINE`) los efectos de fuente. El valor es el resultado de una operación de bits OR de una o varias de las siguientes constantes:<br /><br /> `MDFF_BOLD` = 1<br /><br /> `MDFF_ITALIC` = 2<br /><br /> `MDFF_UNDERLINE` = 4<br /><br /> `MDFF_STRIKEOUT` = 8|  
|**FONT_NAME**|La fuente que se usará para mostrar el `VALUE` o `FORMATTED_VALUE` propiedad.|  
...
|`FORMAT_STRING`|La cadena de formato utilizada para crear el `FORMATTED_VALUE` valor de propiedad. Para obtener más información, vea [FORMAT_STRING, contenido &#40;MDX&#41;](mdx-cell-properties-format-string-contents.md).|  
...
|`VALUE`|Valor sin formato de la celda.|  
 ...
 Solo el `CELL_ORDINAL`, `FORMATTED_VALUE`, y `VALUE` se requieren las propiedades de celda. Todas las propiedades de celda, intrínsecas o específicas del proveedor, se definen en el conjunto de datos del esquema `PROPERTIES`, incluidos los tipos de datos y la compatibilidad con el proveedor. Para obtener más información sobre la `PROPERTIES` de filas de esquema, vea [filas MDSCHEMA_PROPERTIES](../../schema-rowsets/ole-db-olap/mdschema-properties-rowset.md).  
  
 De forma predeterminada, si la `CELL PROPERTIES` no se usa la palabra clave, las propiedades de celda devueltas son `VALUE`, `FORMATTED_VALUE`, y `CELL_ORDINAL` (en ese orden). Si el `CELL PROPERTIES` se utiliza la palabra clave, se devuelven solo las propiedades de celda especificadas explícitamente con la palabra clave.  
...
```  
 ...
 No se devuelven las propiedades de celda de consultas MDX que devuelven conjuntos de filas planas; en este caso, cada celda se representa como si solo el `FORMATTED_VALUE` devolvió la propiedad de celda.  
