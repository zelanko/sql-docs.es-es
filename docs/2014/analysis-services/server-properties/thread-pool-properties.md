...
-   [Establecer PerNumaNode en establecer afinidad entre subprocesos de e/s en los procesadores de un nodo NUMA](#bkmk_pernumanode)  
  
-   [Determinar la configuración actual del grupo de subprocesos](#bkmk_currentsettings)  
...
-   **Analizar \ Short** es un grupo de análisis para solicitudes cortas. Las solicitudes que caben dentro de un único mensaje de red se consideran cortas.  
  
-   **Analizar \ Long** es un grupo de análisis para todas las demás solicitudes que no caben en un único mensaje de red.  
...
> [!NOTE]  
>  Aunque msmdsrv. ini tiene la configuración del grupo de `VertiPaq` subprocesos en la sección, `VertiPaq` \\ `ThreadPool` \\ `GroupAffinity` y `ThreadPool` \\ `CPUs` no se documentan intencionadamente. Estas propiedades no están operativas actualmente y se reservan para uso futuro.  
...
##  <a name="bkmk_propref"></a>Referencia de propiedades de grupo de subprocesos  
...
|Nombre|Tipo|Descripción|Valor predeterminado|Guía|  
|----------|----------|-----------------|-------------|--------------|  
...
##  <a name="bkmk_groupaffinity"></a>Establecer GroupAffinity en subprocesos de establecer afinidad entre en procesadores en un grupo de procesadores  
 ...
 **Máscara de máscara para procesadores lógicos**  
  
 Puede tener hasta 64 procesadores lógicos dentro de un solo grupo de procesadores. La máscara de bits es 1 (o 0) para cada procesador lógico del grupo utilizado (o no utilizado) por un grupo de subprocesos. Una vez que calcule la máscara de la máscara, calcule el valor hexadecimal como el valor `GroupAffinity`de.  
...
3.  **Calcular la máscara de la de los grupos de procesadores que desea utilizar**  
...
###  <a name="bkmk_workdistrib"></a>Distribución del trabajo entre los subprocesos de IOProcess  
...
-   Para los grupos de medida que tienen pocas particiones (por ejemplo, que tienen una sola partición), aumente el número de particiones. El uso de una sola partición hará que el motor siempre ponga en cola las tareas en un grupo de subprocesos (grupo de subprocesos 0). Si se agregan más particiones, el motor puede utilizar grupos de subprocesos adicionales.  
     ...
     Como alternativa, si no puede crear particiones adicionales, pruebe a `PerNumaNode`establecer = 0 como una manera de aumentar el número de subprocesos disponibles para el grupo de subprocesos 0.  
  
-   En el caso de las bases de datos en las que los recorridos de los segmentos se `PerNumaNode` distribuyen uniformemente entre varias particiones, establecer en 1 o 2 puede mejorar `IOProcess` el rendimiento de las consultas porque aumenta el número total de grupos de subprocesos utilizados por el sistema.  
  
-   En el caso de las soluciones que tienen varias particiones, pero solo una se examina con `PerNumaNode`mucha frecuencia, pruebe a establecer = 0 para ver si mejora el rendimiento.  
  
 Aunque los recorridos de particiones y dimensiones `IOProcess` utilizan el grupo de subprocesos, los recorridos de dimensiones solo utilizan el grupo de subprocesos 0. Esto puede producir una carga ligeramente desigual en ese grupo de subprocesos, pero el desequilibrio debe ser temporal, ya que los recorridos de dimensiones suelen ser muy rápidos e infrecuentes.  
...
##  <a name="bkmk_related"></a>Propiedades dependientes o relacionadas  
 Como se explica en la sección 2,4 de la [Guía de operaciones de Analysis Services](https://msdn.microsoft.com/library/hh226085.aspx), si aumenta el grupo de subprocesos de procesamiento `CoordinatorExecutionMode` , debe asegurarse de que la `CoordinatorQueryMaxThreads` configuración, así como la configuración, tengan valores que le permitan usar por completo el mayor tamaño del grupo de subprocesos.  
...
##  <a name="bkmk_currentsettings"></a>Determinar la configuración actual del grupo de subprocesos  
...
## <a name="see-also"></a>Consulte también  
 ...
 [Analysis Services cambios en el grupo de subprocesos en SQL Server 2012](https://blogs.msdn.com/b/psssql/archive/2012/01/31/analysis-services-thread-pool-changes-in-sql-server-2012.aspx)   
 [Analysis Services opciones de configuración de 2012 (blog de WordPress)](https://go.microsoft.com/fwlink/?LinkId=330387)   
 [Sistemas compatibles que tienen más de 64 procesadores](https://msdn.microsoft.com/library/windows/hardware/gg463349.aspx)   
 [SQL Server 2008 R2 Analysis Services guía de operaciones](https://go.microsoft.com/fwlink/?LinkID=225539)  
