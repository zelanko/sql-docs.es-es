...
|Property|Valor predeterminado|Descripción|  
|--------------|---------------------|-----------------|  
|**Configuración de suplantación**|**Predeterminado**|Esta propiedad especifica la configuración de suplantación que se usa cuando un modelo que se ejecuta en el modo DirectQuery se conecta con los orígenes de datos. Las credenciales de suplantación no se usan al consultar la memoria caché en memoria. El valor de esta propiedad tiene las opciones siguientes:<br /><br /> **Default** -esta configuración especifica que Analysis Services utilizará la opción especificada en la página de información de suplantación cuando se creó la conexión de origen de datos mediante el Asistente para importación de tablas.<br /><br /> **ImpersonateCurrentUser** -esta configuración especifica la cuenta de usuario del usuario que ha iniciado sesión actualmente se usará al conectarse a todos los orígenes de datos.|  
...
|------------|-----------------|----------|  
|**Implementar el comando en Herramientas de datos de SQL Server**|El comando Implementar proporciona un método sencillo e intuitivo para implementar un proyecto de modelos tabulares del entorno de creación de [!INCLUDE[ssBIDevStudio](../../includes/ssbidevstudio-md.md)] .<br /><br /> **\*\* Precaución \*\*** Este método no se debe usar para implementar servidores de producción. Con este método, puede sobrescribir ciertas propiedades de un modelo existente.|[Implementar con SQL Server Data Tools &#40;SSAS tabular&#41;](deploy-from-sql-server-data-tools-ssas-tabular.md)|  
