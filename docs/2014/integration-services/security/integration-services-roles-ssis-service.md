...
ms.locfileid: "62766677"
---
# <a name="integration-services-roles-ssis-service"></a>Roles de Integration Services (servicio SSIS)
  [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)][!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)] incluye los tres roles fijos de nivel de base `db_ssisadmin`de datos,, **db_ssisltduser**y **db_ssisoperator**para controlar el acceso a los paquetes. Los roles solo se pueden implementar en los paquetes que se guardan `msdb` en [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]la base de datos de. Para asignar roles a un paquete se utiliza [!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)]. Las asignaciones de roles se guardan en la `msdb` base de datos.  
...
## <a name="sysssispackages-table"></a>Tabla Sysssispackages  
 La tabla **sysssispackages** de `msdb` contiene los paquetes que se guardan [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]en. Para obtener más información, vea [sysssispackages &#40;Transact-SQL&#41;](/sql/relational-databases/system-tables/sysssispackages-transact-sql).  
...
## <a name="permissions"></a>Permisos  
 De forma predeterminada, los permisos de `db_ssisadmin` y **db_ssisoperator** roles fijos de nivel de base de datos y el identificador de seguridad único del usuario que creó el paquete se aplican al rol de lector para paquetes, y `db_ssisadmin` los permisos del rol y el identificador de seguridad único del usuario que creó el paquete se aplican al rol de escritor. Un usuario debe ser miembro del rol `db_ssisadmin`, **db_ssisltduser**o **db_ssisoperator** para tener acceso de lectura al paquete. Un usuario debe ser miembro del `db_ssisadmin` rol para tener acceso de escritura.  
...
## <a name="access-to-packages"></a>Acceso a paquetes  
 ...
 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)](el servicio MSSQLSERVER) debe iniciarse antes de poder conectarse al Motor de base de datos y obtener acceso a `msdb` la base de datos.  
  
 Para asignar roles a paquetes, debe completar las tareas siguientes.  
  
-   **Abra Explorador de objetos y conéctese a Integration Services**  
