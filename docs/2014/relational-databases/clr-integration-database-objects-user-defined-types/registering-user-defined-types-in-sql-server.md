...
```  
 ...
 El siguiente [!INCLUDE[tsql](../../includes/tsql-md.md)] instrucción registra el ensamblado utilizando *< assembly_bits >* argumento en la cláusula FROM. Este valor `varbinary` representa el archivo como un flujo de bytes.  
...
### <a name="using-alter-assembly-to-add-source-code"></a>Usar ALTER ASSEMBLY para agregar código fuente  
 ...
 El siguiente [!INCLUDE[tsql](../../includes/tsql-md.md)] instrucción ALTER ASSEMBLY agrega el código de origen de clase Point.cs para el `Point` UDT. De esta forma, el texto incluido en el archivo Point.cs se copia y se almacena en la base de datos con el nombre "PointSource".  
