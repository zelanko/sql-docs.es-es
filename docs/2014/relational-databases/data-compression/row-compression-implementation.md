...
-   Usa el formato de almacenamiento de longitud variable para los tipos numéricos (por ejemplo `integer`, `decimal`, y `float`) y los tipos que se basan en numérico (por ejemplo `datetime` y `money`).  
...
|`float`|Sí|Los bytes menos significativos con ceros no se almacenan. `float` la compresión es aplica principalmente a valores no decimales de mantisa.|  
|`real`|Sí|Los bytes menos significativos con ceros no se almacenan. `real` la compresión es aplica principalmente a valores no decimales de mantisa.|  
