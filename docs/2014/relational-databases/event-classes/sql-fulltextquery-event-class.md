...
|EndTime|`datetime`|Tiempo del evento terminado|15|Yes|  
|Error|`int`|Número del mensaje de error.|31|Yes|  
...
|IntegerData|`int`|Número de filas devueltas. Si la consulta devuelve un error, el valor es NULL. Si la consulta no devuelve ninguna fila, el valor es 0.|25|Yes|  
...
|RequestID|`int`|Solicita la identificación que inició la consulta de texto completo.|49|Yes|  
