...
|9|AVAILABILITY_REPLICA|Una réplica secundaria de un grupo de disponibilidad está aplicando entradas del registro de transacciones de esta base de datos a una base de datos secundaria correspondiente. (Modelo de recuperación completa)<br /><br /> Para obtener más información, consulte [información general de grupos de disponibilidad AlwaysOn &#40;SQL Server&#41;](../../database-engine/availability-groups/windows/overview-of-always-on-availability-groups-sql-server.md).|  
...
|16|XTP_CHECKPOINT|Cuando una base de datos tiene un grupo de archivos con optimización para memoria, el registro de transacciones no se puede truncar hasta automático [!INCLUDE[hek_2](../../includes/hek-2-md.md)] punto de control se desencadena (lo que sucede cada 512 MB de crecimiento del registro).<br /><br /> Nota: Para truncar el registro de transacciones antes de tamaño de 512 MB, activa el comando Checkpoint manualmente en la base de datos en cuestión.|  
...
-   Si la base de datos está establecida en el modelo de recuperación optimizado para cargas masivas de registros o simple, algunas operaciones DDL de índices se registran mínimamente con independencia de si la operación se ejecuta sin conexión o con conexión. Las operaciones de índice con registro mínimo son:  
    ...
    -   Regeneración del nuevo montón DROP INDEX (si procede).  
        ...
        > [!NOTE]  
        >  Índice de desasignación de páginas durante un [DROP INDEX](/sql/t-sql/statements/drop-index-transact-sql) operación siempre se registra completamente.  
