...
# <a name="bulk-copying-from-program-variables"></a>Copia masiva de variables de programa
 ...
 El **bcp_bind *** pData* parámetro contiene la dirección de la variable que se está enlazaba a la columna. Los datos de cada columna pueden almacenarse de una de estas dos formas:  
...
-   Usar punteros de terminador. Carga el **bcp_bind *** pTerm* parámetro con la dirección del patrón de bits que finaliza los datos. Si se utiliza uno de los otros dos métodos, especifique NULL para la *pTerm*.  
 ...
 El **bcp_bind *** tipo* identificadores de tipo de datos de parámetro usa DB-Library, no ODBC identificadores de tipo de datos. Identificadores de tipo de datos de DB-Library se definen en sqlncli.h para usarse con ODBC **bcp_bind** función.  
...
```  
 ...
 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] no admite directamente los tipos de datos interval. Una aplicación sin embargo, puede almacenar secuencias de escape interval como cadenas de caracteres en un [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] columna de caracteres. La aplicación puede leerlas para usarlas más tarde, pero no pueden usarse en instrucciones [!INCLUDE[tsql](../../includes/tsql-md.md)].  
  
 Las funciones de copia masiva pueden utilizarse para cargar rápidamente los datos en [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] que se ha leído desde un origen de datos ODBC. Usar [SQLBindCol](../native-client-odbc-api/sqlbindcol.md) para enlazar las columnas de un conjunto de resultados a variables de programa, a continuación, use **bcp_bind** para enlazar las mismas variables de programa a una operación de copia masiva. Una llamada a [SQLFetchScroll](../native-client-odbc-api/sqlfetchscroll.md) o **SQLFetch** , a continuación, se captura una fila de datos del origen de datos ODBC en las variables de programa y llamar al método [bcp_sendrow](../native-client-odbc-extensions-bulk-copy-functions/bcp-sendrow.md) copias masivas de los datos las variables de programa a [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] .  
  
 Una aplicación puede utilizar el [bcp_colptr](../native-client-odbc-extensions-bulk-copy-functions/bcp-colptr.md) función siempre que sea necesario cambiar la dirección de la variable de datos originalmente especificada en el **bcp_bind** *pData* parámetro. Una aplicación puede utilizar el [bcp_collen](../native-client-odbc-extensions-bulk-copy-functions/bcp-collen.md) función siempre que sea necesario para cambiar la longitud de los datos originalmente especificada en el **bcp_bind *** cbData* parámetro.  
  
 No se puede leer los datos [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] en variables de programa utilizando la copia masiva; no hay nada como una función "bcp_readrow". Solo pueden enviarse datos de la aplicación al servidor.  
