---
title: Copia masiva con el servidor de SQL Server ODBC Driver temas de procedimientos (ODBC) | Microsoft Docs
ms.custom: ''
ms.date: 06/13/2017
ms.prod: sql-server-2014
ms.reviewer: ''
ms.technology: native-client
ms.topic: reference
helpviewer_keywords:
- bulk copy [ODBC]
- bulk copy [ODBC], about bulk copy
ms.assetid: 6037b2c2-62b5-4a9e-b36d-83e5b315eef2
author: MightyPen
ms.author: genemi
manager: craigg
ms.openlocfilehash: 6c2cca868cbbdeddc98264160e646dde3676c585
ms.sourcegitcommit: ceb7e1b9e29e02bb0c6ca400a36e0fa9cf010fca
ms.translationtype: MT
ms.contentlocale: es-ES
ms.lasthandoff: 12/03/2018
ms.locfileid: "52766317"
---
# <a name="bulk-copying-with-the-sql-server-odbc-driver-how-to-topics-odbc"></a>Temas de procedimientos de copia masiva con el controlador ODBC de SQL Server (ODBC)
  Cuando se usa con Microsoft?? SQL Server??? 2005, el controlador ODBC de SQL Server admite las mismas funciones de copia masiva compatibles con la API de DB-Library.  
  
 Para obtener información sobre un importante cambio en copia masiva a partir de [!INCLUDE[ssVersion2005](../../../includes/ssversion2005-md.md)], consulte [realizar operaciones de copia masiva &#40;ODBC&#41;](../../native-client-odbc-bulk-copy-operations/performing-bulk-copy-operations-odbc.md).  
  
## <a name="in-this-section"></a>En esta sección  
  
-   [Copia masiva sin un archivo de formato &#40;ODBC&#41;](bulk-copy-without-a-format-file-odbc.md)  
  
-   [Una copia masiva de un conjunto de resultados seleccione &#40;ODBC&#41;](bulk-copy-a-select-result-set-odbc.md)  
  
-   [Crear un archivo de formato de copia masiva &#40;ODBC&#41;](create-a-bulk-copy-format-file-odbc.md)  
  
-   [Realizar una copia masiva mediante el uso de un archivo de formato &#40;ODBC&#41;](bulk-copy-by-using-a-format-file-odbc.md)  
  
-   [Copia masiva de datos desde Variables de programa &#40;ODBC&#41;](bulk-copy-data-from-program-variables-odbc.md)  
  
## <a name="see-also"></a>Vea también  
 [Temas de procedimientos de ODBC](../odbc-how-to-topics.md)  
  
  
