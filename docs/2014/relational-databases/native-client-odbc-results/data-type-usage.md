...
ms.locfileid: "63200182"
---
# <a name="data-type-usage"></a>Uso de tipos de datos
  El [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] controlador ODBC de Native Client [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] y imponen el siguiente uso de los tipos de datos.  
  
|Tipo de datos|Limitación|  
|---------------|----------------|  
|Literales de fecha|Los literales de fecha, cuando se almacenan en[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] una SQL_TYPE_TIMESTAMP columna (tipos de datos de **DateTime** o **smalldatetime**), tienen un valor de tiempo de 12:00:00.000 A.M.|  
|**Money** y **smallmoney**|Solo las partes enteras de los tipos de datos **Money** y **smallmoney** son significativas. Si la parte decimal de los datos de **Money** de SQL se trunca durante la conversión del [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] tipo de datos, el controlador ODBC de Native Client devuelve una advertencia, no un error.|  
|SQL_BINARY (admite valores NULL)|Cuando se conecta a una instancia de [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] versión 6.0 y anteriores, si una columna SQL_BINARY admite valores NULL, los datos que se almacenan en el origen de datos no se rellenan con ceros. Cuando se recuperan los datos de una columna de [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] este tipo, el controlador ODBC de Native Client los rellena con ceros a la derecha. Sin embargo, en los datos que se crean en operaciones realizadas por [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)], como la concatenación, no se realiza este relleno.<br /><br /> Además, cuando los datos se colocan en este tipo de columna en una instancia de [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 6.0 o versiones anteriores, [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] trunca los datos a la derecha si son demasiado largos para ajustarse a la columna. **Nota:**  El [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] controlador ODBC de Native Client admite la conexión [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] a 6,5 y versiones anteriores.|  
...
|Literales de hora|Los literales de hora, cuando se almacenan en[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] una SQL_TIMESTAMP columna (tipos de datos de **DateTime** o **smalldatetime**), tienen un valor de fecha del 1 de enero de 1900.|  
|**timestamp**|Solo se puede insertar manualmente un valor NULL en una columna **timestamp** . Sin embargo, **** dado que las columnas de marca [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]de tiempo se actualizan automáticamente mediante, se sobrescribe un valor null.|  
