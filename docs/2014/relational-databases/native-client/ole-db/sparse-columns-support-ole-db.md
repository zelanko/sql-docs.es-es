...
|-----------------------------|-----------------|  
|IColumnsInfo::GetColumnsInfo|Un nuevo valor de marca DBCOLUMNFLAGS DBCOLUMNFLAGS_SS_ISCOLUMNSET se establece `column_set` para las columnas en *dwFlags*.<br /><br /> DBCOLUMNFLAGS_WRITE está establecido para las columnas `column_set`.|  
...
|ICommand::Execute|Si se usa ** \* Select from** *TABLE* , devuelve todas las columnas que no son miembros del disperso `column_set`, además de una columna XML que contiene los valores de todas las columnas no NULL que son miembros del `column_set`disperso, si están presentes.|  
