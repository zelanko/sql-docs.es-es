...
|![El primer nodo de una topología](media/p2pwizard-firstnode.gif "El primer nodo en una topología")|El nodo original en la topología. Los nodos nuevos se inicializan utilizando una copia de la base de datos de publicación del nodo original.|
|![Un nodo para el que se ha completado la información](media/p2pwizard-complete.gif "Un nodo para el que tenemos información completa") o una versión posterior, para la que la replicación tiene información completa. Para especificar las opciones de configuración, haga clic con el botón secundario en el nodo.|
...
### <a name="options-for-the-design-surface"></a>Opciones para la superficie de diseño
 Volver a **dibujar el gráfico** Vuelva a dibujar los objetos en la superficie de diseño sin actualizar la topología. Al actualizar, se podría proporcionar una mejor vista de la topología.

 **Actualizar topología** Consulta cada nodo en la topología y muestra información actualizada sobre cada nodo. Con muchos nodos, este proceso puede tardar varios minutos.
 ...
 **Establecer tiempo de espera de actualización** Especifique cuánto tiempo se puede ejecutar el proceso de actualización antes de que se agote el tiempo de espera de la operación.

### <a name="options-for-each-node"></a>Opciones para cada nodo
 **Agregar una nueva conexión del mismo nivel** Agregue una conexión entre dos nodos. Por ejemplo, si agrega una conexión entre el Nodo A y el Nodo B, la replicación agrega dos suscripciones: la primera habilita el Nodo A para recibir los cambios de la publicación en el Nodo B y la segunda permite al Nodo B recibir cambios de la publicación en el Nodo A.
