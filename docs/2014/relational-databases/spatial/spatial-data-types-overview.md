...
# <a name="spatial-data-types-overview"></a>Información general de los tipos de datos espaciales
 ...
 Además, [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] es compatible con la `geography` tipo de datos, que almacena datos elípticos (tierra redonda), como coordenadas de latitud y longitud GPS.  
...
##  <a name="objects"></a> Objetos de datos espaciales  
 Los tipos de datos `geometry` y `geography` admiten dieciséis objetos de datos espaciales o tipos de instancia. Pero solo se pueden *crear instancias*de once de estos tipos de instancia; puede crear y trabajar con estas instancias (o crear instancias de ellas) en una base de datos. Estas instancias obtienen determinadas propiedades de sus tipos de datos primarios que los distinguen como `Points`, **LineStrings, CircularStrings**, `CompoundCurves`, `Polygons`, `CurvePolygons` o como varios `geometry`o `geography` instancias en un `GeometryCollection`. El tipo `Geography` tiene un tipo de instancia adicional, `FullGlobe`.  
  
 La figura siguiente describe el `geometry` jerarquía en la que el `geometry` y `geography` se basan los tipos de datos. Los tipos a partir de `geometry` y `geography` se indican en azul.  
  
 ![Jerarquía del tipo geometry](../../database-engine/media/geom-hierarchy.gif "jerarquía del tipo geometry")  
  
 Tal y como se indica en la ilustración, los diez tipos a partir de la `geometry` y `geography` tipos de datos son `Point`, `MultiPoint`, `LineString`, `CircularString`, `MultiLineString`, `CompoundCurve`, `Polygon`, `CurvePolygon`, `MultiPolygon`, y `GeometryCollection`. Existe un tipo adicional a partir del que pueden crearse instancias para el tipo de datos Geography: `FullGlobe`. El `geometry` y `geography` tipos pueden reconocer una instancia concreta siempre y cuando sea una instancia bien formada, aunque la instancia no está definida explícitamente. Por ejemplo, si define un `Point` instancia explícitamente mediante el método STPointFromText(), `geometry` y `geography` reconocen la instancia como un `Point`, siempre que la entrada de método es correcto. Si define la misma instancia mediante el método `STGeomFromText()`, los tipos de datos `geometry` y `geography` reconocen la instancia como `Point`.  
...
### <a name="measurements-in-spatial-data-types"></a>Medidas en tipos de datos espaciales  
 En el sistema plano, o de tierra plana, las medidas de distancias y las áreas se proporcionan en la misma unidad de medida que las coordenadas. Mediante el `geometry` tipo de datos, la distancia entre (2, 2) y (5, 6) es 5 unidades, independientemente de las unidades usadas.  
  
 En el sistema elíptico o de tierra redonda, las coordenadas se proporcionan en grados de latitud y longitud. Sin embargo, longitudes y las áreas normalmente se miden en metros y metros cuadrados, aunque la medida puede depender del identificador de referencia espacial (SRID) de la `geography` instancia. La unidad más común de medida para el `geography` tipo de datos es el metro.  
...
### <a name="orientation-of-spatial-data"></a>Orientación de datos espaciales  
 ...
 Cuando el nivel de compatibilidad es 100 o menor en [!INCLUDE[ssCurrent](../../../includes/sscurrent-md.md)] la `geography` tipo de datos tiene las siguientes restricciones:  
...
### <a name="outer-and-inner-rings-not-important-in-geography-data-type"></a>Anillos externos e internos no importantes en el tipo de datos Geography  
 Las características simples de OGC para especificación de SQL analizan los anillos externos y los anillos internos, pero esta distinción tiene poco sentido para la [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] `geography` de tipo de datos; puede tomar cualquier anillo de un polígono como anillo externo.  
...
### <a name="linestring-and-circularstring-comparison"></a>Comparación de LineString y CircularString  
 ...
 Este ejemplo muestra cómo almacenar los triángulos isósceles anteriores tanto con una `LineString` instancia y `CircularString` instancia:  
...
```  
 ...
 En los ejemplos anteriores, una `LineString` instancia o un `CompoundCurve` instancia podría almacenar en la ilustración.  En el ejemplo siguiente se utiliza un `CompoundCurve` para almacenar un segmento del gráfico circular:  
  
```tsql  
SET @g = geometry::Parse('COMPOUNDCURVE(CIRCULARSTRING(2 2, 1 3, 0 2),(0 2, 1 0, 2 2))');  
```  
  
 A `CompoundCurve` instancia puede almacenar el segmento de arco circular (2 2, 3 1, 0 2) directamente, mientras que un `LineString` instancia tendría que convertir la curva en varios segmentos de línea más pequeños.  
...
```  
 ...
 `CompoundCurve` las instancias de ambos permiten `LineString` y `CircularString` componentes para que solo dos puntos en los segmentos de línea del segmento del gráfico circular se necesiten conocer.  Este ejemplo de código muestra cómo utilizar un `CompoundCurve` para almacenar la misma figura:  
...
### <a name="polygon-and-curvepolygon-comparison"></a>Comparación de Polygon y CurvePolygon  
 `CurvePolygon` pueden usar instancias `CircularString` y `CompoundCurve` instancias cuando se definen sus anillos exterior e interior.  `Polygon` instancias no pueden utilizar los tipos de segmentos de arco circular: `CircularString` y `CompoundCurve`.  
