...
> [!NOTE]  
>  Si no se especifica la directiva TYPE, se devuelve el resultado de la consulta FOR XML como tipo `nvarchar(max)`.  
...
### <a name="using-for-xml-query-results-in-insert-update-and-delete-transact-sql-dml"></a>Utilizar resultados de consultas FOR XML en INSERT, UPDATE y DELETE (DML de Transact-SQL)  
 En el ejemplo siguiente se muestra el modo en el que se pueden utilizar las consultas FOR XML en instrucciones del lenguaje de manipulación de datos (DML). En el ejemplo, el `FOR XML` devuelve una instancia de `xml` tipo. La instrucción `INSERT` inserta esta instancia de tipo XML en una tabla.  
