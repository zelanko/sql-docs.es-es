...
-   Para ver la descripción de cada elemento de la `URLReservations` sección, vea [archivo de configuración RSReportServer](../report-server/rsreportserver-config-configuration-file.md) en [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] libros en pantalla.  
  
-   Para obtener más información acerca de la sintaxis del `UrlString` elemento, vea [sintaxis de reserva de direcciones URL &#40;Administrador de configuración de SSRS&#41;](url-reservation-syntax-ssrs-configuration-manager.md).  
  
-   Para obtener instrucciones sobre cómo configurar las direcciones URL para el acceso a la aplicación, consulte [configurar una dirección URL &#40;Administrador de configuración de SSRS&#41;](configure-a-url-ssrs-configuration-manager.md).  
  
#### <a name="urls-for-report-access"></a>Direcciones URL para el acceso de los informes  
 [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] incluye una extensión de entrega de correo electrónico del servidor de informes que puede usar para enviar vínculos o datos adjuntos de informes. Un vínculo de informe se construye cuando se entrega el informe. Usos de la extensión de entrega de correo electrónico del servidor de informes la `UrlRoot` establecer en el archivo de configuración para crear el vínculo. `UrlRoot` También se utiliza para resolver los vínculos en un informe representado que se genera mediante el procesamiento de informes en modo desatendido.  
  
 `UrlRoot` se especifica automáticamente en el archivo RSReportServer.config al configurar direcciones URL para el acceso a la aplicación. Si modifica este valor en el archivo de configuración, debe especificar una dirección URL válida para un servicio web del servidor de informes que esté conectado a una base de datos del servidor de informes que contenga los informes que desea entregar. Solo puede especificar una `UrlRoot` para una única instancia del servidor de informes; solo puede existir una entrada `UrlRoot` en el archivo RSReportServer.config para una instancia del servidor de informes determinada. Si tiene varias direcciones URL reservadas para el servicio Web del servidor de informes, debe elegir uno de los valores disponibles para `UrlRoot`.  
  
 En la mayoría de los casos, no es necesario modificar `UrlRoot`. Sin embargo, si el servidor de informes se accederá a través de una dirección URL completa, y no ha configurado una dirección URL que use un encabezado de host para el nombre del sitio completo, debe editar el archivo RSReportServer.config manualmente para establecer el `UrlRoot` para el nombre completo dirección URL del servidor que se usará para representar el informe de informes (por ejemplo, https://www.adventure-works.com/mywebapp/reportserver).  
...
> [!NOTE]  
>  Las versiones anteriores de Reporting Services incluían el `ReportServerVirtualDirectory` elemento. Este valor es obsoleto en [!INCLUDE[ssKatmai](../../includes/sskatmai-md.md)] y versiones posteriores. Si actualizó una instalación existente y usa un archivo de configuración que contiene este valor, el servidor de informes ya no lee este valor.  
