...
2.  Busque la sección ** \<>del servicio** y agregue la siguiente información al archivo de configuración, reemplazando el `Hostname` valor por el nombre del servidor virtual del servidor NLB:  
  
    ```  
    <Hostname>virtual_server</Hostname>  
    ```  
  
3.  Busque `UrlRoot`. El elemento no está especificado en el archivo de configuración, pero el valor predeterminado que se usa es una dirección URL con este formato\<: http://o https://*NombreDeEquipo*>/\<*ReportServer*>, donde \< *ReportServer*> es el nombre del directorio virtual del servicio Web del servidor de informes.  
