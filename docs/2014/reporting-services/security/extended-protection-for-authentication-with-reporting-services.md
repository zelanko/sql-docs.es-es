...
-   Cuando se agregan los valores de configuración `rsreportserver.config` al archivo de configuración, el comportamiento predeterminado es [!INCLUDE[ssRSnoversion](../../../includes/ssrsnoversion-md.md)] que la característica de protección ampliada esté desactivada y debe habilitar la característica como se describe en este tema. Para obtener más información, vea la sección de [configuración](#ConfigurationSettings) de este tema.
...
## <a name="configuring-reporting-rervices-extended-protection"></a>Configurar la protección ampliada de Reporting Services
 El `rsreportserver.config` archivo contiene los valores de configuración que controlan el [!INCLUDE[ssRSnoversion](../../../includes/ssrsnoversion-md.md)] comportamiento de la protección ampliada.

 Para obtener más información sobre el uso y `rsreportserver.config` la edición del archivo, vea [RSReportServer Configuration File](../report-server/rsreportserver-config-configuration-file.md). La configuración de la protección ampliada también se puede cambiar e inspeccionar mediante las API WMI. Para más información, vea [Método SetExtendedProtectionSettings &#40;WMI MSReportServer_ConfigurationSetting&#41;](../wmi-provider-library-reference/configurationsetting-method-setextendedprotectionsettings.md).
