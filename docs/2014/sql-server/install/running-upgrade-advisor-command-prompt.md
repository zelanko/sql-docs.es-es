 ...
 **-CSV**  
 Proporciona los resultados como valores separados por comas en un archivo .csv, además de los resultados XML estándar. Los resultados se escriben en la\\ [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] carpeta My Documents upgrade Advisor\110\Reports  
...
|`PackagePath`|Especifica la ruta de acceso de un conjunto de paquetes de [!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)].|Opcional una vez `IntegrationServices` por elemento. Si este elemento no está presente, el análisis se produce [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] en la instancia y no se analiza ningún paquete almacenado externamente. No hay ningún valor predeterminado.|  
