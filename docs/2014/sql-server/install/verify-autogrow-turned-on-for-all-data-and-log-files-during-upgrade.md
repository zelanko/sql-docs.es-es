...
|Tipos de datos de objetos grandes (LOB)<br /><br /> Variables y parámetros LOB|Los `varchar(max)`tipos de datos `nvarchar(max)`,, el **texto varbinary (Max)** `ntext`, `xml` `image,` y son tipos de objetos grandes.<br /><br /> Cuando se habilita un nivel de aislamiento de transacción basado en versiones de fila en la base de datos y se realizan modificaciones de objetos grandes, el fragmento cambiado del LOB se copia en el almacén de versiones de **tempdb**.<br /><br /> Los parámetros definidos como un tipo de datos de objetos grandes se almacenan en **tempdb**.|  
