...
# <a name="testing-a-filtered-model-basic-data-mining-tutorial"></a>Probar un modelo filtrado (Tutorial básico de minería de datos)
  Ahora que ha determinado que el `TM_Decision_Tree` modelo es el más preciso, personalizará el modelo para satisfacer mejor las necesidades de la [!INCLUDE[ssSampleDBCoFull](../includes/sssampledbcofull-md.md)] campaña de correo directo. Concretamente, el departamento de marketing desea saber si hay alguna diferencia entre los clientes masculinos y femeninos. Esta información puede ayudarles a decidir qué revistas utilizar para los anuncios y qué productos ofrecer en sus campañas.  
...
## <a name="using-filters"></a>Usar filtros  
 ...
 En primer lugar, realizará una copia `TM_Decision_Tree` del modelo.  
...
3.  Haga clic con `TM_Decision_Tree` el botón secundario en el modelo y seleccione **nuevo modelo de minería de datos.**  
...
1.  Haga clic con el `TM_Decision_Tree_Male` botón secundario en el modelo de minería de datos para abrir el menú contextual.  
...
8.  Repita los pasos anteriores, pero esta vez asigne un nombre `TM_Decision_Tree_Female` al modelo y escriba **F** en el cuadro de texto **valor** .  
...
1.  Haga clic con el `TM_Decision_Tree_Male` botón secundario en el modelo y seleccione **procesar estructura de minería de datos y todos los modelos**.  
...
2.  En el cuadro modelo de minería de `TM_Decision_Tree_Male`datos, seleccione.  
...
6.  Repita los pasos 1-5 `TM_Decision_Tree_Female`para.  
