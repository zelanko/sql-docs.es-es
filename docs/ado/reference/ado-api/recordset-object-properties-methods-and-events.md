---
description: Eventos, métodos y propiedades del objeto de conjunto de registros
title: Propiedades, métodos y eventos del objeto de conjunto de registros | Microsoft Docs
ms.prod: sql
ms.prod_service: connectivity
ms.technology: connectivity
ms.custom: ''
ms.date: 01/19/2017
ms.reviewer: ''
ms.topic: conceptual
helpviewer_keywords:
- Recordset object [ADO], members
ms.assetid: 4295a6e5-112d-4595-b18a-57728893ac2d
author: rothja
ms.author: jroth
ms.openlocfilehash: 88751b720685700d37d775208afd38e100910b69
ms.sourcegitcommit: 7345e4f05d6c06e1bcd73747a4a47873b3f3251f
ms.translationtype: MT
ms.contentlocale: es-ES
ms.lasthandoff: 08/24/2020
ms.locfileid: "88772164"
---
# <a name="recordset-object-properties-methods-and-events"></a>Eventos, métodos y propiedades del objeto de conjunto de registros
## <a name="propertiescollections"></a>Propiedades/colecciones  
 [Propiedad AbsolutePage](./absolutepage-property-ado.md)  
  
 [Propiedad AbsolutePosition](./absoluteposition-property-ado.md)  
  
 [Propiedad ActiveCommand](./activecommand-property-ado.md)  
  
 [ActiveConnection (propiedad)](./activeconnection-property-ado.md)  
  
 [Propiedades BOF, EOF](./bof-eof-properties-ado.md)  
  
 [Bookmark (propiedad)](./bookmark-property-ado.md)  
  
 [Propiedad CacheSize](./cachesize-property-ado.md)  
  
 [Propiedad CursorLocation](./cursorlocation-property-ado.md)  
  
 [Propiedad CursorType](./cursortype-property-ado.md)  
  
 [Propiedad DataMember](./datamember-property.md)  
  
 [Propiedad DataSource](./datasource-property-ado.md)  
  
 [Propiedad EditMode](./editmode-property.md)  
  
 [Colección Fields](./fields-collection-ado.md)  
  
 [Propiedad Filter](./filter-property.md)  
  
 [Propiedad Index](./index-property.md)  
  
 [LockType (propiedad)](./locktype-property-ado.md)  
  
 [MarshalOptions (propiedad)](./marshaloptions-property-ado.md)  
  
 [Propiedad MaxRecords](./maxrecords-property-ado.md)  
  
 [PageCount (propiedad)](./pagecount-property-ado.md)  
  
 [Propiedad PageSize](./pagesize-property-ado.md)  
  
 [Colección de propiedades](./properties-collection-ado.md)  
  
 [Propiedad RecordCount](./recordcount-property-ado.md)  
  
 [Propiedad de ordenación](./sort-property.md)  
  
 [Propiedad Source (ADO Recordset)](./source-property-ado-recordset.md)  
  
 [Propiedad State](./state-property-ado.md)  
  
 [Propiedad Status (conjunto de registros ADO)](./status-property-ado-recordset.md)  
  
 [Propiedad StayInSync](./stayinsync-property.md)  
  
## <a name="methods"></a>Métodos  
 [AddNew (método)](./addnew-method-ado.md)  
  
 [CANCEL (método)](./cancel-method-ado.md)  
  
 [CancelBatch (método)](./cancelbatch-method-ado.md)  
  
 [CancelUpdate (método)](./cancelupdate-method-ado.md)  
  
 [Método Clone](./clone-method-ado.md)  
  
 [Close (método)](./close-method-ado.md)  
  
 [CompareBookmarks (método)](./comparebookmarks-method-ado.md)  
  
 [Delete (método) (conjunto de registros ADO)](./delete-method-ado-recordset.md)  
  
 [Find (método)](./find-method-ado.md)  
  
 [GetRows (método)](./getrows-method-ado.md)  
  
 [GetString (Método)](./getstring-method-ado.md)  
  
 [Move (método)](./move-method-ado.md)  
  
 [Métodos MoveFirst, MoveLast, MoveNext y MovePrevious](./movefirst-movelast-movenext-and-moveprevious-methods-ado.md)  
  
 [NextRecordset (método)](./nextrecordset-method-ado.md)  
  
 [Open (método) (conjunto de registros ADO)](./open-method-ado-recordset.md)  
  
 [Requery (método)](./requery-method.md)  
  
 [Método Resync](./resync-method.md)  
  
 [Save (método)](./save-method.md)  
  
 [El método de búsqueda](./seek-method.md)  
  
 [Método Supports](./supports-method.md)  
  
 [Update (método)](./update-method.md)  
  
 [Método UpdateBatch](./updatebatch-method.md)  
  
## <a name="events"></a>Eventos  
 [Evento EndOfRecordset](./endofrecordset-event-ado.md)  
  
 [Evento FetchComplete](./fetchcomplete-event-ado.md)  
  
 [Evento FetchProgress](./fetchprogress-event-ado.md)  
  
 [Eventos WillChangeField y FieldChangeComplete](./willchangefield-and-fieldchangecomplete-events-ado.md)  
  
 [Eventos de WillChangeRecord y RecordChangeComplete](./willchangerecord-and-recordchangecomplete-events-ado.md)  
  
 [Eventos WillChangeRecordset y RecordsetChangeComplete](./willchangerecordset-and-recordsetchangecomplete-events-ado.md)  
  
 [Eventos WillMove y MoveComplete](./willmove-and-movecomplete-events-ado.md)  
  
## <a name="see-also"></a>Consulte también  
 [Objeto de conjunto de registros (ADO)](./recordset-object-ado.md)