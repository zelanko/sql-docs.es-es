---
title: Colocar objeto (ADO MD) | Documentos de Microsoft
ms.prod: sql
ms.prod_service: connectivity
ms.technology: connectivity
ms.custom: ''
ms.date: 01/19/2017
ms.reviewer: ''
ms.suite: sql
ms.tgt_pltfrm: ''
ms.topic: conceptual
apitype: COM
f1_keywords:
- Position
helpviewer_keywords:
- Position object [ADO MD]
ms.assetid: 91eab784-3ce9-41d6-a840-9b0939ca0608
caps.latest.revision: 9
author: MightyPen
ms.author: genemi
manager: craigg
ms.openlocfilehash: 844997df808dbb1c61939164d72a9b2c601a9823
ms.sourcegitcommit: 62826c291db93c9017ae219f75c3cfeb8140bf06
ms.translationtype: MT
ms.contentlocale: es-ES
ms.lasthandoff: 06/11/2018
ms.locfileid: "35284574"
---
# <a name="position-object-ado-md"></a>Objeto Position (ADO MD)
Representa un conjunto de uno o varios miembros de distintas dimensiones que define un punto a lo largo de un eje.  
  
## <a name="remarks"></a>Notas  
 Con las propiedades y colecciones de una **posición** objeto puede hacer lo siguiente:  
  
-   Use la **Ordinal** propiedad para devolver la posición ordinal de la **posición** a lo largo de la [eje](../../../ado/reference/ado-md-api/axis-object-ado-md.md).  
  
-   Use la [miembros](../../../ado/reference/ado-md-api/members-collection-ado-md.md) colección para devolver los miembros que componen la posición a lo largo de la **eje**.  
  
 Esta sección contiene el siguiente tema.  
  
-   [Propiedades, métodos y eventos](../../../ado/reference/ado-md-api/position-object-properties-methods-and-events.md)  
  
## <a name="see-also"></a>Vea también  
 [Ejemplo de Axis (VBScript)](../../../ado/reference/ado-md-api/axis-example-vbscript.md)   
 [Objeto Axis (ADO MD)](../../../ado/reference/ado-md-api/axis-object-ado-md.md)   
 [Objeto Cell (ADO MD)](../../../ado/reference/ado-md-api/cell-object-ado-md.md)   
 [Colección Members (ADO MD)](../../../ado/reference/ado-md-api/members-collection-ado-md.md)   
 [Colección de posiciones (ADO MD)](../../../ado/reference/ado-md-api/positions-collection-ado-md.md)
