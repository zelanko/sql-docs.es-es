---
title: "Comprobador de coherencia de base de datos (DBCC) para bases de datos multidimensionales y tabulares de Analysis Services | Microsoft Docs"
ms.custom: 
  - "SQL2016_New_Updated"
ms.date: "03/04/2017"
...
manager: "erikre"
---
# Comprobador de coherencia de base de datos (DBCC) para bases de datos multidimensionales y tabulares de Analysis Services
...
-   Los metadatos de las nuevas bases de datos del modelo tabular en el nivel de compatibilidad 1200 constan de descriptores como **TableName** y **PartitionName**.  
...
## Sintaxis de comandos para bases de datos tabulares 1200.  
 Las bases de datos tabulares en el nivel de compatibilidad 1200 usan los metadatos tabulares para definiciones de objeto. En el ejemplo siguiente, se muestra la sintaxis completa de DBCC para una base de datos tabular creada en un nivel funcional de SQL Server 2016.  
  
 Las principales diferencias entre las dos sintaxis son un espacio de nombres de XMLA más reciente y que no hay elementos \<Object> ni \<Model> (solo hay un modelo por base de datos).  
...
### Seguimiento del resultado de DBCC en SQL Server Profiler 2016  
...
4.  En SQL Server Profiler, la actividad de DBCC se indica mediante eventos **Command** con una subclase de eventos de DBCC:  
     ...
     Los mensajes de estado empiezan por "Comprobando la coherencia de \<objeto>", "Se inició la comprobación de \<objeto>" o "Finalizó la comprobación de \<objeto>".  
  
    > [!NOTE]  
    >  En CTP 3.0, los objetos se identifican por nombres internos. Por ejemplo, una jerarquía de categorías se expresa como H$Categories-\<IdDeObjeto>. Los nombres internos deben reemplazarse por nombres descriptivos en una próxima versión de CTP.  
...
|-----------|-----------|----------------|  
|**Errores del administrador de metadatos.**<br /><br /> La referencia del objeto '\<idDeObjeto>' no es válida. No coincide con la estructura de la jerarquía de clases de metadatos.|comando incorrecto|Compruebe la sintaxis del comando. Lo más probable es que haya incluido un objeto de nivel inferior sin especificar uno o varios de sus objetos primarios.|  
|**Errores del administrador de metadatos.**<br /><br /> El \<objeto> con identificador '\<idDeObjeto>' no existe en '\<objetoPrincipal>' con identificador '\<idDeObjetoPrincipal>' o el usuario no tiene permiso para acceder al objeto.|Daños en el índice (multidimensional)|Vuelva a procesar el objeto y los objetos dependientes.|  
|**Error durante la comprobación de coherencia de la partición**<br /><br /> Se produjo un error al comprobar la coherencia de la partición \<nombre-partición> del grupo de medida \<nombre-grupo-medida> para el cubo \<nombre-cubo> de la base de datos \<nombre-base-datos>. Vuelva a procesar la partición o los índices para corregir los daños.|Daños en el índice (multidimensional)|Vuelva a procesar el objeto y los objetos dependientes.|  
...
|**Daños en los metadatos de una tabla**<br /><br /> El archivo de metadatos de la tabla \<nombre-tabla> está dañado. La tabla principal no se encuentra en el nodo DataFileList.|Daños en los metadatos (solo en tipo tabular)|Elimine y vuelva a implementar el proyecto, o restaure una copia de seguridad y procese de nuevo.<br /><br /> Para obtener instrucciones, vea la entrada de blog [How to handle corruption in Analysis Services databases (blog)](http://blogs.msdn.com/b/karang/archive/2010/08/11/how-to-deal-with-corruption-in-analysis-services.aspx) (Reparar bases de datos dañadas de Analysis Services).|  
|**Daños en la capa de almacenamiento**<br /><br /> Daños en la capa de almacenamiento: la colección de \<nombre-tipo> en \<nombre-primario> \<tipo-primario> está dañada.|Daños en los metadatos (solo en tipo tabular)|Elimine y vuelva a implementar el proyecto, o restaure una copia de seguridad y procese de nuevo.<br /><br /> Para obtener instrucciones, vea la entrada de blog [How to handle corruption in Analysis Services databases (blog)](http://blogs.msdn.com/b/karang/archive/2010/08/11/how-to-deal-with-corruption-in-analysis-services.aspx) (Reparar bases de datos dañadas de Analysis Services).|  
|**Falta tabla del sistema**<br /><br /> Falta la tabla del sistema \<nombre-tabla>.|Daños en objeto (solo en tipo tabular)|Vuelva a procesar el objeto y los objetos dependientes.|  
|**Daños en las estadísticas de la tabla**<br /><br /> Faltan las estadísticas de la tabla del sistema \<nombre-tabla>.|Daños en los metadatos (solo en tipo tabular)|Elimine y vuelva a implementar el proyecto, o restaure una copia de seguridad y procese de nuevo.<br /><br /> Para obtener instrucciones, vea la entrada de blog [How to handle corruption in Analysis Services databases (blog)](http://blogs.msdn.com/b/karang/archive/2010/08/11/how-to-deal-with-corruption-in-analysis-services.aspx) (Reparar bases de datos dañadas de Analysis Services).|  
  
## Deshabilite las comprobaciones de coherencia automáticas en las operaciones de carga de base de datos a través del archivo de configuración msmdsrv.ini.  
