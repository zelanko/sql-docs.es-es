...
### <a name="connecting-to-the-server-object"></a>Conectar al objeto Server  
 ...
 El siguiente código en el ejemplo devuelve una <xref:Microsoft.AnalysisServices.Server> objeto si la conexión es correcta, o devuelve **null** si se produce un error. Los errores durante el proceso de conexión se administran en una construcción de try/catch. Los errores AMO se detectan mediante la clase de excepción <xref:Microsoft.AnalysisServices.AmoException>. En este ejemplo, el error se muestra al usuario en un cuadro de mensaje.  
...
### <a name="processing-a-database"></a>Procesar una base de datos  
 ...
 El método Process puede incluir parámetros, pero no son necesarios. Si se especifica ningún parámetro, todos los objetos secundarios se procesarán con su **ProcessDefault** opción. Para obtener más información acerca de las opciones de procesamiento, vea <xref:Microsoft.AnalysisServices.Database>.  
...
1.  Crear un objeto <xref:Microsoft.AnalysisServices.DataSourceView>.  
     ...
     Compruebe primero si el <xref:Microsoft.AnalysisServices.DataSource> objeto existe; si **true**, a continuación, coloque el <xref:Microsoft.AnalysisServices.DataSource> y crearla. Si no existe <xref:Microsoft.AnalysisServices.DataSource>, créelo.  
...
> [!NOTE]  
>  La cláusula `'WHERE 1=0'` consiste en evitar que la consulta devuelva filas a la **conjunto de datos** objeto.  
...
```  
 ...
 En el código de ejemplo, el `AddTable` y `AddComputedColumn` métodos utilizan la `FillSchema` método de la **DataAdapter** objeto que se va a agregar un **DataTable** para un **delconjuntodedatos**y configurar el esquema para que coincida con el origen de datos. Las propiedades extendidas agregan la información necesaria para configurar el esquema para [!INCLUDE[ssASnoversion](../../../includes/ssasnoversion-md.md)].  
