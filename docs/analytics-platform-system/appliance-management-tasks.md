---
title: 'Tareas de administración de dispositivo: Analytics Platform System | Documentos de Microsoft'
description: Esta guía describe cómo realizar tareas de administración de dispositivo para Analytics Platform System (APS).
author: mzaman1
manager: craigg
ms.prod: sql
ms.technology: data-warehouse
ms.topic: conceptual
ms.date: 04/17/2018
ms.author: murshedz
ms.reviewer: martinle
ms.openlocfilehash: 9012ddc562e9252367c7204383fa4ccc06f06345
ms.sourcegitcommit: 056ce753c2d6b85cd78be4fc6a29c2b4daaaf26c
ms.translationtype: MT
ms.contentlocale: es-ES
ms.lasthandoff: 04/19/2018
ms.locfileid: "31538835"
---
# <a name="appliance-management-tasks-for-analytics-platform-system"></a>Tareas de administración de dispositivo de sistema de la plataforma de análisis
Esta guía describe cómo realizar tareas de administración de dispositivo para Analytics Platform System (APS).  
  
## <a name="guide-sections"></a>Secciones de la Guía  
  
-   [Introducción a la configuración e instalación de dispositivo &#40;sistema de la plataforma de análisis&#41;](appliance-installation-and-configuration-overview.md)  
  
-   [Conectarse a los nodos de dispositivo &#40;sistema de la plataforma de análisis&#41;](connect-to-appliance-nodes.md)  
  
-   [El Software antivirus &#40;sistema de la plataforma de análisis&#41;](antivirus-software.md)  
  
-   [Configuración de dispositivo &#40;sistema de la plataforma de análisis&#41;](appliance-configuration.md)  
  
-   [Mantenimiento de software &#40;sistema de la plataforma de análisis&#41;](software-servicing.md)  
  
-   [Estado de los servicios PDW &#40;sistema de la plataforma de análisis&#41;](pdw-services-status.md)  
  
-   [Encender el dispositivo de APS o apagar &#40;sistema de la plataforma de análisis&#41;](power-the-aps-appliance-on-or-off.md)  
  
-   [Supervisión de dispositivo &#40;sistema de la plataforma de análisis&#41;](appliance-monitoring.md)  
  
