...
description: Describe cómo usar los nuevos tipos de datos de fecha y hora introducidos en SQL Server 2008.
ms.date: 09/30/2019
dev_langs:
- csharp
...
ms.locfileid: "72452253"
---
# <a name="date-and-time-data"></a>Datos de fecha y hora

![Download-DownArrow-Circled](../../../ssdt/media/download.png)[Descargar ADO.NET](../../sql-connection-libraries.md#anchor-20-drivers-relational-access)

SQL Server 2008 introduce nuevos tipos de datos para controlar la información de fecha y hora. Los nuevos tipos de datos incluyen tipos independientes para la fecha y la hora, y tipos de datos expandidos con mayor conocimiento de rango, precisión y zona horaria. El proveedor de datos SqlClient de Microsoft para SQL Server (<xref:Microsoft.Data.SqlClient>) proporciona compatibilidad total con todas las características nuevas del Motor de base de datos SQL Server 2008. Debe instalar el .NET Framework 3,5 SP1 (o posterior) o .NET Core 1,0 (o posterior) para usar estas nuevas características con SqlClient.  
  
Las versiones de SQL Server anteriores a SQL Server 2008 solo tenían dos tipos de datos para trabajar con valores de fecha y hora: `datetime` y `smalldatetime`. Ambos tipos de datos contienen el valor de fecha y el valor de hora, lo que dificulta trabajar solo con valores de fecha o de hora. Además, estos tipos de datos solo admiten fechas que se producen después de la introducción del calendario gregoriano en Inglaterra en 1753. Otra limitación es que estos tipos de datos más antiguos no reconocen la zona horaria, lo que dificulta el trabajo con datos que se originan en varias zonas horarias.  
  
La documentación completa de los tipos de datos de SQL Server está disponible en Libros en pantalla de SQL Server. Vea [usar datos de fecha y hora](https://go.microsoft.com/fwlink/?LinkID=98361) para temas de nivel de entrada sobre datos de fecha y hora.
...
Las enumeraciones siguientes se han agregado a <xref:System.Data.SqlDbType> para admitir los nuevos tipos de datos de fecha y hora.  
...
|Tipo de datos de SQL Server|Tipo de .NET|System. Data. SqlDbType|System. Data. DbType|  
|--------------------------|-------------------------|---------------------------|------------------------|  
...
|<xref:Microsoft.Data.SqlClient.SqlParameter.Size%2A>|Se omite para los tipos de datos de fecha y hora.|  
...
> [!NOTE]
>  Los valores de hora que son menores que cero o mayor o igual que 24 horas producirán una <xref:System.ArgumentException>.  
  
### <a name="creating-parameters"></a>Creación de parámetros  
Puede crear un objeto <xref:Microsoft.Data.SqlClient.SqlParameter> mediante su constructor o agregándolo a un <xref:Microsoft.Data.SqlClient.SqlCommand>. <xref:Microsoft.Data.SqlClient.SqlCommand.Parameters%2A> colección llamando al método `Add` del <xref:Microsoft.Data.SqlClient.SqlParameterCollection>. El método `Add` tomará como entrada los argumentos del constructor o un objeto de parámetro existente.  
...
En el fragmento de código siguiente se muestra cómo especificar un parámetro de `date`.  
...
### <a name="time-example"></a>Ejemplo de tiempo  
En el fragmento de código siguiente se muestra cómo especificar un parámetro de `time`.  
...
En el fragmento de código siguiente se muestra cómo especificar un parámetro de `DateTimeOffSet` con una fecha, una hora y un ajuste de zona horaria de 0.  
...
También puede proporcionar parámetros mediante el método `AddWithValue` de una <xref:Microsoft.Data.SqlClient.SqlCommand>, como se muestra en el fragmento de código siguiente. Sin embargo, el método `AddWithValue` no permite especificar el <xref:Microsoft.Data.SqlClient.SqlParameter.DbType%2A> o <xref:Microsoft.Data.SqlClient.SqlParameter.SqlDbType%2A> para el parámetro.  
...
El parámetro `@date` podría asignarse a un tipo de datos `date`, `datetime` o `datetime2` en el servidor. Al trabajar con los nuevos tipos de datos de `datetime`, debe establecer explícitamente la propiedad <xref:System.Data.SqlDbType> del parámetro en el tipo de datos de la instancia. El uso de <xref:System.Data.SqlDbType.Variant> o el suministro implícito de valores de parámetros puede causar problemas con la compatibilidad con versiones anteriores de los tipos de datos `datetime` y `smalldatetime`.  
  
En la tabla siguiente se muestra qué `SqlDbTypes` se deducen a partir de los tipos de CLR:  
...
|<xref:Microsoft.Data.SqlClient.SqlDataReader.GetProviderSpecificFieldType%2A>|Devuelve el tipo que es el tipo específico del proveedor subyacente para el campo. Devuelve los mismos tipos que `GetFieldType` para los nuevos tipos de fecha y hora.|  
...
|<xref:Microsoft.Data.SqlClient.SqlDataReader.GetSqlString%2A>|Recupera el valor de la columna como un <xref:System.Data.SqlTypes.SqlString>. Se produce una <xref:System.InvalidCastException> si los datos no se pueden expresar como `SqlString`.|  
...
|<xref:Microsoft.Data.SqlClient.SqlDataReader.GetString%2A>|Recupera el valor de columna como una cadena si Type System Version se ha establecido en SQL Server 2005. Se produce una <xref:System.InvalidCastException> si los datos no se pueden expresar como una cadena.|  
...
|<xref:Microsoft.Data.SqlClient.SqlDataReader.GetSchemaTable%2A>|Devuelve un <xref:System.Data.DataTable> que describe los metadatos del conjunto de resultados.|  
...
Puede especificar tipos de datos de fecha y hora mediante una variedad de diferentes formatos de cadena literales, que SQL Server evalúa en tiempo de ejecución, convirtiéndolos en estructuras de fecha y hora internas. SQL Server reconoce los datos de fecha y hora que se incluyen entre comillas simples ('). En los siguientes ejemplos se muestran algunos formatos:  
...
> [!NOTE]
>  Puede encontrar documentación completa de todos los formatos de cadena literales y otras características de los tipos de datos de fecha y hora de Libros en pantalla de SQL Server.  
  
Los valores de hora que son menores que cero o mayor o igual que 24 horas producirán una <xref:System.ArgumentException>.  
  
## <a name="resources-in-sql-server-2008-books-online"></a>Recursos en los libros en pantalla de SQL Server 2008  
Para obtener más información sobre cómo trabajar con valores de fecha y hora en SQL Server 2008, vea los siguientes recursos en SQL Server 2008 libros en pantalla.  
...
|[Tipos de datos (Transact-SQL)](https://go.microsoft.com/fwlink/?LinkId=98362)|Describe los tipos de datos del sistema en SQL Server 2008.|  
