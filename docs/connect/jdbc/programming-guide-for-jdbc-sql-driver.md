---
title: Guía de programación para el controlador JDBC para SQL | Documentos de Microsoft
ms.custom: ''
ms.date: 01/19/2017
ms.prod: sql
ms.prod_service: connectivity
ms.reviewer: ''
ms.suite: sql
ms.technology: connectivity
ms.tgt_pltfrm: ''
ms.topic: conceptual
ms.assetid: 99f0db69-ce1a-49cf-9c24-0b9abfa4e0c3
caps.latest.revision: 2
author: MightyPen
ms.author: genemi
manager: craigg
ms.openlocfilehash: 1d697ecfc653843efb15cb9717de1170817270b9
ms.sourcegitcommit: 1740f3090b168c0e809611a7aa6fd514075616bf
ms.translationtype: MT
ms.contentlocale: es-ES
ms.lasthandoff: 05/03/2018
ms.locfileid: "32828630"
---
# <a name="programming-guide-for-jdbc-sql-driver"></a>Guía para el controlador JDBC para SQL de programación
[!INCLUDE[Driver_JDBC_Download](../../includes/driver_jdbc_download.md)]

* [Conectarse a SQL Server con el controlador JDBC](../../connect/jdbc/connecting-to-sql-server-with-the-jdbc-driver.md)
* [Describir los tipos de datos del controlador JDBC](../../connect/jdbc/understanding-the-jdbc-driver-data-types.md)
* [Usar instrucciones con el controlador JDBC](../../connect/jdbc/using-statements-with-the-jdbc-driver.md)
* [Administrar conjuntos de resultados con el controlador JDBC](../../connect/jdbc/managing-result-sets-with-the-jdbc-driver.md)
* [Realizar transacciones con el controlador JDBC](../../connect/jdbc/performing-transactions-with-the-jdbc-driver.md)
* [Controlar metadatos con el controlador JDBC](../../connect/jdbc/handling-metadata-with-the-jdbc-driver.md)
* [Uso de la copia masiva con el controlador JDBC](../../connect/jdbc/using-bulk-copy-with-the-jdbc-driver.md)
* [Usar Always Encrypted con el controlador JDBC](../../connect/jdbc/using-always-encrypted-with-the-jdbc-driver.md)
* [Usar parámetros con valores de tabla](../../connect/jdbc/using-table-valued-parameters.md)
* [Características internacionales del controlador JDBC](../../connect/jdbc/international-features-of-the-jdbc-driver.md)
* [Referencia de API del controlador JDBC](../../connect/jdbc/reference/jdbc-driver-api-reference.md)
