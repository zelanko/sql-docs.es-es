---
title: Con Azure Active Directory con el controlador ODBC | Documentos de Microsoft
ms.custom: 
ms.date: 04/18/2017
...
ODBC Driver 13.1 para SQL Server permite a las aplicaciones de ODBC para conectarse a una instancia de SQL Azure con una identidad federada en Azure Active Directory con un nombre de usuario y una contraseña, un token de acceso de Azure Active Directory (solo controladores de Windows) o integrada de Windows Autenticación (sólo controladores de Windows). Esto se logra mediante el uso de nuevo DSN y palabras clave de cadena de conexión y atributos de conexión.
...
|Attribute|Tipo|Valores|Predeterminado|Description|
|-|-|-|-|-|
|`SQL_COPT_SS_AUTHENTICATION`|`SQL_IS_INTEGER`|`SQL_AU_NONE`, `SQL_AU_PASSWORD`, `SQL_AU_AD_INTEGRATED`, `SQL_AU_AD_PASSWORD`, `SQL_AU_RESET`|(sin establecer)|Consulte la descripción de `Authentication` palabra clave anterior. `SQL_AU_NONE`se proporciona con el fin de reemplazar explícitamente un conjunto `Authentication` valor en la cadena de conexión o de DSN, mientras que `SQL_AU_RESET` unsets el atributo si ya se ha establecido, permitiendo que el valor de cadena de conexión o DSN tendrá prioridad.|
|`SQL_COPT_SS_ACCESS_TOKEN`|`SQL_IS_POINTER`|Puntero a `ACCESSTOKEN` o NULL|NULL|_Sólo el controlador Windows_. Si no es null, especifica el Token de acceso de la organización a usar. Es un error especificar un token de acceso y también `UID`, `PWD`, `Trusted_Connection`, o `Authentication` palabras clave de cadena de conexión o sus atributos equivalente.|
...
Las cuatro opciones corresponden a `Trusted_Connection=Yes` (heredado de Windows existentes solo SSPI la autenticación integrada) y `Authentication=` `ActiveDirectoryIntegrated`, `SqlPassword`, y `ActiveDirectoryPassword`, respectivamente.
...
El cuadro de diálogo muestra SQLDriverConnect cuando solicita la información necesaria para completar la conexión contiene dos nuevas opciones para la autenticación de Azure AD:

![SQLServerLogin.png](windows/SQLServerLogin.png)

Estas opciones corresponden a los mismos cuatro disponibles en la configuración DSN anterior de la interfaz de usuario.
...
## <a name="authenticating-with-an-access-token-windows-driver-only"></a>Autenticar con un Token de acceso (sólo controladores de Windows)
