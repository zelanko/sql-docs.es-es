...
|Palabra clave|Propiedad de inicialización|Descripción|  
|-------------|-----------------------------|-----------------|  
|**Token de acceso**<a href="#table2_1"><sup id="table2_accesstoken">**1**</sup></a>|SSPROP_AUTH_ACCESS_TOKEN|El token de acceso que se usa para autenticarse en Azure Active Directory. <br/><br/>**Nota:** Es un error especificar esta palabra clave `UID`y también `Trusted_Connection`, `PWD`, o `Authentication` palabras clave de cadena de conexión o sus propiedades o palabras clave correspondientes.|
...
|**Autenticación** de <a href="#table2_1"> <sup> **1**</sup></a>|SSPROP_AUTH_MODE|Especifica la autenticación de SQL o Active Directory utilizada. Los valores válidos son:<br/><ul><li>`(not set)`: El modo de autenticación determinado por otras palabras clave.</li><li>`ActiveDirectoryPassword:`Active Directory la autenticación mediante el identificador de inicio de sesión y la contraseña.</li><li>`ActiveDirectoryIntegrated:`Autenticación integrada en Active Directory con las credenciales de la cuenta de Windows del usuario que ha iniciado sesión actualmente.</li><br/>**Nota:** Se **recomienda** que las aplicaciones que `Integrated Security` usen palabras `Trusted_Connection`clave de autenticación (o) o sus propiedades correspondientes establezcan `Authentication` el valor de la palabra clave (o `ActiveDirectoryIntegrated` su propiedad correspondiente) en para habilitar New comportamiento de cifrado y validación de certificados.<br/><br/><li>`SqlPassword:`Autenticación mediante el identificador y la contraseña de inicio de sesión.</li><br/>**Nota:** Se **recomienda** que las aplicaciones que `SQL Server` usan la autenticación establezcan el `Authentication` valor de la palabra clave (o su `SqlPassword` propiedad correspondiente) en para habilitar el nuevo cifrado y el comportamiento de validación de certificados.</ul>|
|**Auto Translate**|SSPROP_INIT_AUTOTRANSLATE|Sinónimo de "AutoTranslate".|  
...
|**Application Intent**|SSPROP_INIT_APPLICATIONINTENT|Declara el tipo de carga de trabajo de la aplicación al conectarse a un servidor. Los valores posibles son **ReadOnly** y **ReadWrite**.<br /><br /> El valor predeterminado es **ReadWrite**. Para obtener más información acerca del controlador de OLE DB para la [!INCLUDE[ssHADR](../../../includes/sshadr-md.md)]compatibilidad de SQL Server con, consulte [OLE DB driver for SQL Server Support for High Availability, Disaster Recovery](../../oledb/features/oledb-driver-for-sql-server-support-for-high-availability-disaster-recovery.md).|  
|**Application Name**|SSPROP_INIT_APPNAME|Cadena que identifica la aplicación.|  
|**Autenticación** de <a href="#table3_1"> <sup> **1**</sup></a>|SSPROP_AUTH_MODE|Especifica la autenticación de SQL o Active Directory utilizada. Los valores válidos son:<br/><ul><li>`(not set)`: El modo de autenticación determinado por otras palabras clave.</li><li>`ActiveDirectoryPassword:`Active Directory la autenticación mediante el identificador de inicio de sesión y la contraseña.</li><li>`ActiveDirectoryIntegrated:`Autenticación integrada en Active Directory con las credenciales de la cuenta de Windows del usuario que ha iniciado sesión actualmente.</li><br/>**Nota:** Se **recomienda** que las aplicaciones que `Integrated Security` usen palabras `Trusted_Connection`clave de autenticación (o) o sus propiedades correspondientes establezcan `Authentication` el valor de la palabra clave (o `ActiveDirectoryIntegrated` su propiedad correspondiente) en para habilitar New comportamiento de cifrado y validación de certificados.<br/><br/><li>`SqlPassword:`Autenticación mediante el identificador y la contraseña de inicio de sesión.</li><br/>**Nota:** Se **recomienda** que las aplicaciones que `SQL Server` usan la autenticación establezcan el `Authentication` valor de la palabra clave (o su `SqlPassword` propiedad correspondiente) en para habilitar el nuevo cifrado y el comportamiento de validación de certificados.</ul>|
|**Auto Translate**|SSPROP_INIT_AUTOTRANSLATE|Sinónimo de "AutoTranslate".|  
|**AutoTranslate**|SSPROP_INIT_AUTOTRANSLATE|Configura la traducción de caracteres OEM/ANSI. Los valores reconocidos son "true" y "false".|  
|**Connect Timeout**|DBPROP_INIT_TIMEOUT|Cantidad de tiempo (en segundos) que hay que esperar a que se complete la inicialización del origen de datos.|  
|**Current Language**|SSPROPT_INIT_CURRENTLANGUAGE|Nombre del lenguaje [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)].|  
