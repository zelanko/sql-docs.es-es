...
## Pasos de configuración previa antes de activar la Auditoría local 
...
1. Crear una configuración de clave del Registro para configurar el directorio de destino de Auditoría local. 
    ...
    Para Motor de base de datos, cree la clave en *HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Microsoft SQL Server\\MSSQL13.\<NOMBREINSTANCIA\>\\CPE*. 
    
    Para Analysis Services, cree la clave en *HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Microsoft SQL Server\\MSAS13.\<NOMBREINSTANCIA\>\\CPE*.

### Obtener la cuenta de inicio de sesión del servicio CEIP de SQL Server
...
2. Vaya al servicio correspondiente. Por ejemplo, para el motor de base de datos, ubique **Servicio CEIP de SQL Server *nombre de instancia***. En el caso de Analysis Services, ubique **CEIP de SQL Server Analysis Services *nombre de instancia***. 
...
### Configurar una carpeta nueva para los archivos de Auditoría local.    
...
> Nota: Configurar la ruta de acceso al directorio para Auditoría local fuera de la ruta de instalación de SQL Server para evitar que permitir la funcionalidad de auditoría y la aplicación de revisiones provoquen problemas potenciales con SQL Server.
  ...
  |------|-----------------|----------|  
  |![Checkbox](../../database-engine/availability-groups/windows/media/checkboxemptycenterxtraspacetopandright.gif "Checkbox")|Disponibilidad de espacio |En una carga de trabajo moderada con alrededor de 10 bases de datos, disponga de unos 2 MB de espacio en disco por día por instancia.|  
|![Checkbox](../../database-engine/availability-groups/windows/media/checkboxemptycenterxtraspacetopandright.gif "Checkbox")|Directorios independientes | Cree un directorio para cada instancia. Por ejemplo, use *C:\\SQLCEIPAudit\\MSSQLSERVER\\DB\\* para una instancia de SQL Server denominada `MSSQLSERVER`. Esto simplifica la administración de archivos.
|![Checkbox](../../database-engine/availability-groups/windows/media/checkboxemptycenterxtraspacetopandright.gif "Checkbox")|Carpetas independientes |Use una carpeta específica para cada servicio. Por ejemplo, para un nombre de instancia determinado, debe tener una carpeta para el motor de base de datos. Si una instancia de SSAS usa el mismo nombre de instancia, cree una carpeta independiente para SSAS. Si tiene las instancias de Motor de base de datos y de Analysis Services configuradas en la misma carpeta, Auditoría local escribirá todo en el mismo archivo de registro desde ambas instancias.| 
|![Checkbox](../../database-engine/availability-groups/windows/media/checkboxemptycenterxtraspacetopandright.gif "Checkbox")|Conceder permisos a la cuenta de inicio de sesión del servicio Telemetría de CIEP de SQL Server|Habilite **Mostrar el contenido de la carpeta**, Acceso de **lectura** y **escritura** a la cuenta de inicio de sesión del servicio Telemetría CEIP de SQL Server.|


### Conceder permisos a la cuenta de inicio de sesión del servicio Telemetría de CIEP de SQL Server
...
### Crear una configuración de clave del Registro para configurar el directorio de destino de Auditoría local.

1. Inicie regedit.  

1. Vaya a la ruta de CPE correspondiente. 

    Para Motor de base de datos, use *HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Microsoft SQL Server\\MSSQL13.\<NOMBREINSTANCIA\>\\CPE*. 
    
    Para Analysis Services, use *HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Microsoft SQL Server\\MSAS13.\<NOMBREINSTANCIA\>\\CPE*.
...
## Activar o desactivar Auditoría local
...
## Diccionario de datos de estructura de datos de salida de Auditoría local 
...
### Definición y ejemplos de pares nombre-valor 
...
|instanceName| El nombre de la instancia de SQL Server anónima| e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855 
...
|secuencia | El número de secuencia de los puntos de datos enviados dentro de la sesión | 15 
...
|consulta| Si corresponde, la definición de consulta T-SQL relacionada con el identificador queryIdentifier que genera los datos.        El servicio CEIP de SQL Server no carga este componente. Solo se incluye en Auditoría local como referencia para los clientes.| SELECT\n      SERVERPROPERTY(\u0027Collation\u0027) AS [Collation],\n      SERVERPROPERTY(\u0027IsFullTextInstalled\u0027) AS [SqlFTinstalled],\n      SERVERPROPERTY(\u0027IsIntegratedSecurityOnly\u0027) AS [SqlIntSec],\n      SERVERPROPERTY(\u0027IsSingleUser\u0027) AS [IsSingleUser],\n      SERVERPROPERTY (\u0027FileStreamEffectiveLevel\u0027) AS [SqlFilestreamMode],\n      SERVERPROPERTY(\u0027IsPolybaseInstalled\u0027) AS [SqlPbInstalled],\n      SERVERPROPERTY(\u0027PolybaseRole\u0027) AS [SqlPbNodeRole],\n      SERVERPROPERTY(\u0027ProductMajorVersion\u0027) AS [SqlVersionMajor],\n      SERVERPROPERTY(\u0027ProductMinorVersion\u0027) AS [SqlVersionMinor],\n      SERVERPROPERTY(\u0027ProductBuild\u0027) AS [SqlVersionBuild],\n      SERVERPROPERTY(\u0027ProductBuildType\u0027) AS ProductBuildType,\n      SERVERPROPERTY(\u0027ProductLevel\u0027) AS ProductLevel,\n      SERVERPROPERTY(\u0027ProductUpdateLevel\u0027) AS ProductUpdateLevel,\n      SERVERPROPERTY(\u0027ProductUpdateReference\u0027) AS ProductUpdateReference,\n      RIGHT(CAST(SERVERPROPERTY(\u0027ProductVersion\u0027) AS NVARCHAR(30)),CHARINDEX(\u0027.\u0027, REVERSE(CAST(SERVERPROPERTY(\u0027ProductVersion\u0027) AS NVARCHAR(30)))) - 1) AS ProductRevision,\n      SERVERPROPERTY(\u0027EditionID\u0027) AS SQLEditionId,\n      SERVERPROPERTY(\u0027IsClustered\u0027) AS IsClustered,\n      SERVERPROPERTY(\u0027IsHadrEnabled\u0027) AS IsHadrEnabled,\n      SERVERPROPERTY(\u0027IsAdvancedAnalyticsInstalled\u0027) AS [SqlAdvAInstalled],\n      @@PACK_RECEIVED AS PacketReceived,\n      @@VERSION AS Version
...
### Ejemplos de archivos de Auditoría local
