...
## <a name="screen-shot---what-you-see-if-youre-going-to-copy-tables"></a>Captura de pantalla: lo que verá si va a copiar tablas  
 En la siguiente captura de pantalla se muestra la página **Seleccionar tablas y vistas de origen** del asistente después de seleccionar la opción **Copiar los datos de una o varias tablas o vistas** en la página **Especificar copia de tabla o consulta**. En la lista, se ven todas las tablas y vistas disponibles del origen de datos.
 
En este ejemplo, el usuario quiere copiar la tabla **Sales.Customer** del origen en la nueva tabla **Sales.CustomerNew** del destino. 
...
## <a name="screen-shot---what-you-see-if-you-provided-a-query"></a>Captura de pantalla: lo que verá si ha proporcionado una consulta  
 En la siguiente captura de pantalla se muestra la página **Seleccionar tablas y vistas de origen** del asistente después de seleccionar la opción **Escribir una consulta para especificar los datos que se van a transferir** en la página **Especificar copia de tabla o consulta** y proporcionar la consulta en la página **Proporcionar una consulta de origen**. En la lista, verá una única fila, donde el elemento de la columna de origen representa la consulta que ha proporcionado.
...
**Origen**  
Utilice las casillas para seleccionar en la lista las tablas y vistas disponibles que deben copiarse en el destino. De forma predeterminada, los datos del origen de datos se copian sin cambios. Si crea una tabla de destino, también se copiará sin cambios el esquema desde el origen de datos.

Si está proporcionando una consulta, la lista contiene solo un elemento con el nombre \[Query\]. 
...
**Valores nulos o que faltan en datos importados**. Cuando parece que una columna de Excel contiene tipos de datos mixtos (por ejemplo, valores numéricos mezclados con valores de texto) en las primeras ocho filas que muestrea el asistente, el asistente elige el tipo de datos mayoritario como el tipo de datos de la columna y devuelve valores nulos para las celdas que contienen datos de otros tipos. No hay ninguna forma de cambiar este comportamiento del asistente.
...
**Especificar un rango existente**. Al especificar un rango existente como destino, obtendrá un error si el rango no es lo suficientemente ancho; es decir, si tiene menos columnas que el origen de datos. En cambio, si el rango que especifica no es lo suficientemente largo (es decir, si tiene menos filas que los datos de origen), el asistente sigue escribiendo filas y extiende la definición del rango para que coincida con el nuevo número de filas.
...
-   Si la tabla de destino ya contiene filas de datos, las primeras ocho filas que muestree el asistente deberán contener por lo menos una fila con un valor de más de 255 caracteres en la columna de memorando.
-   Si el asistente crea la tabla de destino, la instrucción **CREATE TABLE** deberá usar **LONGTEXT** (o uno de sus sinónimos) como el tipo de datos de la columna de memorando. Revise la instrucción **CREATE TABLE**. Para ello, haga clic en **Editar SQL** junto a la opción **Crear tabla de destino** de la página **Asignaciones de columnas**.
