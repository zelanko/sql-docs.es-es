...
[!INCLUDE[ssis-appliesto](../../includes/ssis-appliesto-ssvrpluslinux-asdb-asdw-xxx.md)]
 ...
 En tiempo de diseño, [!INCLUDE[ssBIDevStudioFull](../../includes/ssbidevstudiofull-md.md)] proporciona puntos de interrupción para pausar la ejecución de los paquetes, la ventana de progreso y los visores de datos. Los puntos de interrupción permiten ver los datos a medida que pasan por el flujo de datos. Pero estas características no están disponibles cuando se ejecutan paquetes implementados. Las principales técnicas para solucionar problemas de paquetes implementados son las siguientes:  
...
## <a name="catch-and-handle-package-errors-by-using-event-handlers"></a>Detectar y controlar errores de paquetes mediante controladores de eventos  
 Use controladores de eventos para responder a eventos generados por el paquete y los objetos de paquete.  
  
-   **Crear un controlador de eventos para el evento OnError**. En el controlador de eventos, puede usar una tarea Enviar correo para notificar el error a un administrador. Use una tarea Script y lógica personalizada para obtener información del sistema a fin de solucionar problemas o para limpiar recursos temporales y salidas incompletas. Para más información, vea [Controladores de eventos de Integration Services &#40;SSIS&#41;](../../integration-services/integration-services-ssis-event-handlers.md).  
...
-   **Agregar información descriptiva a las salidas de error**. Para mejorar la claridad de la salida de error, agregue el mensaje de error y el nombre de la columna además de los dos identificadores numéricos que proporciona la salida de error. Para obtener un ejemplo de cómo agregar estas dos columnas adicionales mediante el uso de scripting, consulte [Mejorar una salida de errores con el componente de script](../../integration-services/extending-packages-scripting-data-flow-script-component-examples/enhancing-an-error-output-with-the-script-component.md).  
...
## <a name="troubleshoot-package-execution-by-using-logging"></a>Solución de problemas de la ejecución de paquetes mediante el registro  
 Con el registro puede realizar el seguimiento de lo que sucede en los paquetes en ejecución. Los proveedores de registro capturan información sobre los eventos especificados para su análisis posterior y la guardan en uno de varios formatos compatibles.  Los formatos de proveedor de registro admitidos incluyen tabla de base de datos, un archivo plano y un archivo XML.  
...
-   **Configure la propiedad DelayValidation en elementos de paquete no válidos conocidos**. Para evitar errores de validación de elementos de paquete con problemas de configuración conocidos, establezca **DelayValidation** en **true**. Por ejemplo, el paquete contiene una tarea Flujo de datos que usa una tabla de destino inexistente antes de que una tarea de ejecución de SQL la cree en tiempo de ejecución. La propiedad **DelayValidation** se puede habilitar en el nivel de paquete o en el de tareas y contenedores individuales incluidos en el paquete.  
...
## <a name="troubleshoot-errors-without-a-description"></a>Solución de problemas de errores sin descripción  
 Si se encuentra un error de [!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)] que no tiene ninguna descripción asociada, puede localizarla en [Referencia de errores y mensajes de Integration Services](../../integration-services/integration-services-error-and-message-reference.md). En este momento, la lista no incluye información sobre cómo solucionar problemas.  
