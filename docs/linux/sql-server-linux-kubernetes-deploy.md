...
- El acceso a la [ejemplos de sql server](https://github.com/Microsoft/sql-server-samples/tree/master/samples/features/high%20availability/Kubernetes/sample-manifest-files) repositorio de github
  ...
  >[!NOTE]
  >Puede usar cualquier tipo de clúster de Kubernetes. Para crear un clúster de Kubernetes en Azure Kubernetes Service (AKS), consulte [crear un clúster de AKS](http://docs.microsoft.com/azure/aks/create-cluster).
...
1. Crear un secreto de Kubernetes con las contraseñas para el `sa` cuenta y la clave maestra de instancia de SQL Server.
...
Puede usar [panel de Kubernetes con Azure Kubernetes Service (AKS)](https://docs.microsoft.com/azure/aks/kubernetes-dashboard) para supervisar la implementación.
...
- La salida de `kubectl get services` del espacio de nombres `ag1`.
 ...
 Los servicios de los servicios de equilibrio de carga que se crean para cada contenedor de SQL Server. Use estas direcciones IP como puntos de conexión para conectarse directamente a la instancia de SQL Server en el clúster.
...
Puede ver el estado del grupo de disponibilidad desde el panel grupo de disponibilidad de SQL Server Management Studio.
