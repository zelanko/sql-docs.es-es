...
|*fOption*|Comentarios|  
|---------------|--------------|  
|SQL_ASYNC_ENABLE|No se admite el procesamiento asincrónico. El SQL_ASYNC_ENABLE fOption devolverá SQLSTATE S1C00 (Driver no compatible).|  
|SQL_KEYSET_SIZE|El único tamaño válido del conjunto de claves es 0, porque no se admiten cursores mixtos y dinámicos. Si este valor se establece en cualquier otro número, se cambiará a 0 y la llamada devolverá SQL_SUCCESS_WITH_INFO y SQLSTATE 01S02 (valor de opción cambiado).|  
|SQL_MAX_ROWS|El único tamaño de conjunto de filas válido es 0, porque los controladores de base de datos de escritorio no admiten limitar el número de filas que se devuelven. Si este valor se establece en cualquier otro número, se cambiará a 0 y la llamada devolverá SQL_SUCCESS_WITH_INFO y SQLSTATE 01S02 (valor de opción cambiado).|  
