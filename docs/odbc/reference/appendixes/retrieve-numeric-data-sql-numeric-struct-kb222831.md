...
description: C/C++ mediante ODBC recupera el tipo de datos numéricos de SQL Server mediante SQL_NUMERIC_STRUCT, relacionados con SQL_C_NUMERIC.
editor: ''
ms.prod: sql
...
ms.locfileid: "81219104"
---
# <a name="retrieve-numeric-data-with-sql_numeric_struct"></a>Recuperar datos numéricos con SQL\_NUMERIC\_STRUCT

En este artículo se describe cómo recuperar datos numéricos del controlador ODBC de SQL Server SQL Server en una estructura numérica. También describe cómo obtener los valores correctos utilizando valores de precisión y escala específicos.

Este tipo de datos permite a las aplicaciones controlar directamente los datos numéricos. Alrededor del año 2003, ODBC 3.0 introdujo un nuevo tipo de datos ODBC C, identificado por **SQL\_C\_NUMERIC**. Este tipo de datos sigue siendo relevante a partir de 2017.

El búfer de C que se utiliza tiene la definición de tipo **de ESTRUCTURA\_NUMERICDE.\_** Esta estructura tiene campos para almacenar la precisión, la escala, el signo y el valor de los datos numéricos. El propio valor se almacena como un entero escalado con el byte menos significativo que comienza en la posición más a la izquierda. 

El artículo Tipos de [datos de C](c-data-types.md) proporciona\_\_más información sobre el formato y el uso de SQL NUMERIC STRUCT. Por lo general, el [Apéndice D](appendix-d-data-types.md) de la referencia del programador ODBC 3.0 describe los tipos de datos.


## <a name="sql_numeric_struct-overview"></a>Descripción general de SQL\_NUMERIC\_STRUCT


EL\_STRUCT\_NUMERIC de SQL se define en el archivo de encabezado sqltypes.h de la siguiente manera:
...
Los campos de precisión y escala de la estructura numérica nunca se utilizan para la entrada de una aplicación, solo para la salida del controlador a la aplicación.
...
## <a name="sql_numeric_struct-code-sample"></a>Ejemplo\_\_de código DE ESTRUCTURA NUMERICA de SQL

Este ejemplo de código muestra cómo:
...
> [!Note]
> CUALQUIER USO POR USTED DEL CÓDIGO PROPORCIONADO EN ESTE ARTÍCULO ES BAJO SU PROPIO RIESGO. 
>
> Microsoft proporciona estas muestras de código "tal cual" sin garantía de ningún tipo, ya sea expresa o implícita, incluidas, entre otras, las garantías implícitas de comerciabilidad y/o idoneidad para un propósito particular.
...
- El carácter equivalente de 7C, que es '' (pipe) en el primer elemento de la matriz de caracteres.
- El equivalente a 62, que es 'b' en el segundo elemento.
- Los restos de los elementos de la matriz contienen ceros, por lo que el búfer contiene '-b-0'.

Ahora el desafío es construir el entero escalado a partir de esta matriz de cadenas. Cada carácter de la cadena corresponde a dos dígitos hexadecimales, por ejemplo, el dígito menos significativo (LSD) y el dígito más significativo (MSD). El valor entero escalado podría generarse multiplicando cada dígito (LSD & MSD) con un múltiplo de 16, empezando por 1.

Código que implementa la conversión del modo endian pequeño al entero escalado. Depende del desarrollador de aplicaciones implementar esta funcionalidad. El ejemplo de código siguiente es solo una de las muchas maneras posibles.
...
La información anterior\_sobre\_SQL NUMERIC STRUCT se aplica a las siguientes versiones de producto:

- Controlador ODBC de Microsoft para Microsoft SQL Server 3.7
...
## <a name="sql_c_numeric-overview"></a>Descripción general de SQL\_C\_NUMERIC


El siguiente programa de ejemplo\_ilustra\_el uso de SQL C NUMERIC, insertando 123.45 en una tabla. En la tabla, la columna se define como numérica o decimal, con precisión 5 y con escala 2.

El controlador ODBC que utilice para ejecutar este programa debe admitir la funcionalidad ODBC 3.0.
