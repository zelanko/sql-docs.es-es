...
|----------------|-------------|  
|SQL_ALTER_TABLE (ODBC 2.0) **Nota:** este tipo de información no está en desuso; están en desuso las máscaras de bits en la columna a la derecha.|Una máscara de bits SQLINTEGER enumerar las cláusulas de la **ALTER TABLE** instrucción admitida por el origen de datos.<br /><br /> Las máscaras de bits siguientes se utilizan para determinar qué cláusulas son compatibles con:<br /><br /> SQL_AT_DROP_COLUMN = se admite la posibilidad de quitar las columnas. Si esto da como resultado cascade o restringir el comportamiento está definido por el controlador. (ODBC 2.0)<br /><br /> SQL_AT_ADD_COLUMN = la capacidad de agregar varias columnas en una sola instrucción ALTER TABLE se admite. Este bit no combinar con otros SQL_AT_ADD_COLUMN_XXX ó SQL_AT_CONSTRAINT_XXX bits. (ODBC 2.0)|  
