---
title: Elegir un nivel de interoperabilidad | Microsoft Docs
ms.custom: ''
ms.date: 01/19/2017
ms.prod: sql
ms.prod_service: connectivity
ms.reviewer: ''
ms.technology: connectivity
ms.topic: conceptual
helpviewer_keywords:
- interoperability [ODBC], levels
ms.assetid: 1f915832-a7c1-41cd-8c4f-bf6d976951b0
author: MightyPen
ms.author: genemi
manager: craigg
ms.openlocfilehash: 11145ad7c63ff36686b70a6a4b30b4d9084d7be8
ms.sourcegitcommit: 3026c22b7fba19059a769ea5f367c4f51efaf286
ms.translationtype: MT
ms.contentlocale: es-ES
ms.lasthandoff: 06/15/2019
ms.locfileid: "63125421"
---
# <a name="choosing-a-level-of-interoperability"></a>Elección de un nivel de interoperabilidad
Suponiendo que la aplicación utilizará ODBC, el siguiente paso es determinar qué nivel de interoperabilidad es necesario. El nivel básico de interoperabilidad suele ser una función del tipo de aplicación: Aplicaciones personalizadas no suelen ser interoperable, aplicaciones verticales suelen ser interoperable entre un número limitado de DBMS y aplicaciones genéricas tienden a ser interoperable entre todos los DBMS.  
  
 Esta sección contiene los temas siguientes.  
  
-   [Aplicaciones personalizadas](../../../odbc/reference/develop-app/custom-applications.md)  
  
-   [Aplicaciones verticales](../../../odbc/reference/develop-app/vertical-applications.md)  
  
-   [Aplicaciones genéricas](../../../odbc/reference/develop-app/generic-applications.md)
