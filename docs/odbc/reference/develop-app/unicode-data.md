...
```  
 ...
 El administrador de controladores almacena los nombres de controlador y DSN en Unicode y los asigna a ANSI según sea necesario. Si un carácter Unicode no se puede asignar a un carácter ANSI (como puede ocurrir si los caracteres de una página de códigos que no es la página de códigos nativa del equipo se usan en los nombres de controlador y DSN), los caracteres que no se pudieron convertir se representan mediante un SUP de carácter predeterminado. plied por el sistema.
