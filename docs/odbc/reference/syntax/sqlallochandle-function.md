...
## <a name="diagnostics"></a>Diagnóstico  
 Cuando **SQLAllocHandle** devuelve SQL_ERROR u SQL_SUCCESS_WITH_INFO, se puede obtener un valor SQLSTATE asociado mediante una llamada a **SQLGetDiagRec** con el *HandleType* y el *identificador* adecuados establecidos en el valor de *InputHandle* . Se puede devolver SQL_SUCCESS_WITH_INFO (pero no SQL_ERROR) para el argumento *OutputHandle* . En la tabla siguiente se enumeran los valores de SQLSTATE que suele devolver **SQLAllocHandle** y se explica cada uno de ellos en el contexto de esta función. la notación "(DM)" precede a las descripciones de SQLSTATEs devueltas por el administrador de controladores. El código de retorno asociado a cada valor SQLSTATE es SQL_ERROR, a menos que se indique lo contrario.  
  
|SQLSTATE|Error|Descripción|  
|--------------|-----------|-----------------|  
|01000|ADVERTENCIA general|Mensaje informativo específico del controlador. (La función devuelve SQL_SUCCESS_WITH_INFO).|  
|08003|Conexión no abierta|(DM) el argumento *HandleType* era SQL_HANDLE_STMT o SQL_HANDLE_DESC, pero la conexión especificada por el argumento *InputHandle* no estaba abierta. El proceso de conexión debe completarse correctamente (y la conexión debe estar abierta) para que el controlador asigne un identificador de instrucción o descriptor.|  
...
|HY010|Error de secuencia de función|(DM) el argumento *HandleType* era SQL_HANDLE_DBC y no se ha llamado a **SQLSetEnvAttr** para establecer el atributo SQL_ODBC_VERSION Environment.<br /><br /> (DM) se llamó a una función que se ejecuta de forma asincrónica para **InputHandle** y que todavía se estaba ejecutando cuando se llamó a la función **SQLAllocHandle** con **HANDLETYPE** establecido en SQL_HANDLE_STMT o SQL_HANDLE_DESC.|  
...
|HYC00|Característica opcional no implementada|El argumento *HandleType* era SQL_HANDLE_DESC y el controlador era ODBC 2. controlador *x* .|  
