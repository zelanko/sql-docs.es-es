...
|HY010|Error de secuencia de función|(DM) se llamó a una función que se ejecuta de forma asincrónica para el identificador de conexión que está asociado el *StatementHandle*. Aún estaba ejecutando esta función asincrónica cuando el **SQLNumResultsCols** se llamó a la función.<br /><br /> (DM) **SQLExecute**, **SQLExecDirect**, o **SQLMoreResults** se llamó para el *StatementHandle* y devuelven SQL_PARAM_DATA_ ESTÁ DISPONIBLE. Esta función se invoca antes de que se recuperan los datos para todos los parámetros transmitidos.<br /><br /> (DM) que se llamó la función antes de llamar a **SQLPrepare** o **SQLExecDirect** para el *StatementHandle*.<br /><br /> (DM) se llamó a una función que se ejecuta asincrónicamente (no ésta) para el *StatementHandle* y aún se estaba ejecutando cuando se llamó a esta función.<br /><br /> (DM) **SQLExecute**, **SQLExecDirect**, **SQLBulkOperations**, o **SQLSetPos** se llamó para el  *StatementHandle* y devuelve SQL_NEED_DATA. Esta función se invoca antes de que se enviaron datos para todas las columnas o parámetros de datos en ejecución.<br /><br /> Consulte [SQLPrepare Function](../../../odbc/reference/syntax/sqlprepare-function.md) para obtener más información sobre cuándo se puede liberar un identificador de instrucción.|  
 ...
 **SQLNumResultCols** puede devolver cualquier SQLSTATE, que puede devolver **SQLPrepare** o **SQLExecute** cuando se llama después **SQLPrepare** y antes de  **SQLExecute**, dependiendo de si el origen de datos se evalúa como la instrucción SQL asociada con la instrucción.  
  
## <a name="comments"></a>Comentarios  
 **SQLNumResultCols** puede llamarse correctamente solo cuando la instrucción está en estado preparada, ejecutada o posicionada.  
  
 Si la instrucción asociada *StatementHandle* no devuelve las columnas, **SQLNumResultCols** establece **ColumnCountPtr* en 0.  
  
 El número de columnas devueltas por **SQLNumResultCols** es el mismo valor que el campo SQL_DESC_COUNT de IRD.  
  
 Para obtener más información, consulte [fue un resultado se establece crean?](../../../odbc/reference/develop-app/was-a-result-set-created.md) y [es cómo usan metadatos?](../../../odbc/reference/develop-app/how-is-metadata-used.md).  
...
|Establecer opciones de desplazamiento de cursor|[Función SQLSetStmtAttr](../../../odbc/reference/syntax/sqlsetstmtattr-function.md)|  
