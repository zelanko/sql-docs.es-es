...
Si restaura una base de datos de una versión anterior en [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)], la base de datos se actualizará automáticamente a [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)].   
...
7.  En la sección **Opciones de restauración** , active **Sobrescribir la base de datos existente (WITH REPLACE)**.
    ...
    > **NOTA**: Si no activa esta opción, podría recibir el siguiente mensaje de error: "System.Data.SqlClient.SqlError: el conjunto de copia de seguridad contiene una copia de una base de datos distinta de la '`Sales`' existente. (Microsoft.SqlServer.SmoExtended)"
...
9.  En la sección **Conexiones de servidor** , active **Cerrar las conexiones existentes con la base de datos de destino**.
    ...
    > **NOTA**: Si no activa esta opción, podría recibir el siguiente mensaje de error: "System.Data.SqlClient.SqlError: no se pudo obtener acceso exclusivo porque la base de datos está en uso. (Microsoft.SqlServer.SmoExtended)"
    
10. [!INCLUDE[clickOK](../../includes/clickok-md.md)] 

### <a name="c--restore-an-earlier-disk-backup-with-a-new-database-name-where-the-original-database-still-exists"></a>**C.  Restaurar una copia de seguridad de disco anterior con un nuevo nombre de base de datos en que todavía existe la base de datos original**
...
8.  En la sección **Copia del final del registro** , desactive "**Realizar copia del final del registro de la cola antes de la restauración**".
    ...
    > **IMPORTANTE:** Si no desactiva esta opción, la base de datos existente ( `Sales`) cambiará al estado de restauración.

9. [!INCLUDE[clickOK](../../includes/clickok-md.md)] 

    > **NOTA**: Es posible que reciba el mensaje de error siguiente: "System.Data.SqlClient.SqlError: no se hizo copia de seguridad del final del registro de la base de datos "`Sales`". Use BACKUP LOG WITH NORECOVERY para realizar una copia de seguridad del registro si contiene trabajo que no desea perder. Utilice la cláusula WITH REPLACE o WITH STOPAT de la instrucción RESTORE para sobrescribir el contenido del registro. (Microsoft.SqlServer.SmoExtended)".  
...
En el ejemplo siguiente se restaura una base de datos al estado en que se encontraba a las 1:23:17 p. m. del 30 de mayo de 2016 y se muestra una operación de restauración que implica varias copias de seguridad de registros.  La base de datos no existe actualmente en el servidor.
...
### <a name="e--restore-a-backup-from-the-microsoft-azure-storage-service"></a>**E.  Restaurar una copia de seguridad desde el servicio de Microsoft Azure Storage**
...
*  
    7.  Seleccione `https://mystorageaccount.blob.core.windows.net/myfirstcontainer` en la lista desplegable **Contenedor de almacenamiento de Azure:** si ya existe una credencial de SQL Server. En caso de que no exista, escriba manualmente el nombre del contenedor, `https://mystorageaccount.blob.core.windows.net/myfirstcontainer`.
    
    8.  Escriba la firma de acceso compartido en el cuadro de texto enriquecido **Firma de acceso compartido:** .
       9.   Haga clic en **Aceptar** y se abrirá el cuadro de diálogo **Buscar archivo de copia de seguridad en Microsoft Azure** .
    10. Expanda **Contenedores** y vaya a `https://mystorageaccount.blob.core.windows.net/myfirstcontainer`.
    
    11. Mantenga pulsada la tecla CTRL y seleccione los archivos `Sales_stripe1of2_20160601.bak` y `Sales_stripe2of2_20160601.bak`.
    ...
    7.  Haga clic en **Agregar** y se abrirá el cuadro de diálogo **Conectarse a una suscripción de Microsoft** .  
    ...
    9.  Haga clic en **Aceptar** en el cuadro de diálogo **Seleccionar ubicación de archivo de copia de seguridad** y se abrirá el cuadro de diálogo **Buscar archivo de copia de seguridad en Microsoft Azure** .
    10. Expanda **Contenedores** y vaya a `https://mystorageaccount.blob.core.windows.net/myfirstcontainer`.
    11. Seleccione el archivo de copia de seguridad y haga clic en **Aceptar**.
    12. Haga clic en **Aceptar** para volver a la página **General** .
    13. Haga clic en **Aceptar**.

#### <a name="f---restore-local-backup-to-microsoft-azure-storage-url"></a>**F.   Restaurar copia de seguridad local en Microsoft Azure Storage (dirección URL)**
