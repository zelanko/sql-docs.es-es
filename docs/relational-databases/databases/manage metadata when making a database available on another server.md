---
title: "Administrar los metadatos cuando una base de datos pasa a estar disponible en otra instancia de servidor (SQL Server) | Microsoft Docs"
ms.custom: ""
ms.date: "03/13/2017"
...
ms.topic: "article"
helpviewer_keywords: 
  - "consultas entre bases de datos [SQL Server]"
  - "inicios de sesión [SQL Server], recrear en otra instancia de servidor"
  - "desencadenadores [SQL Server], DLL"
  - "mensajes de error definidos por el usuario [SQL Server]"
  - "permisos [SQL Server], acceso a metadatos"
  - "motor de texto completo [SQL Server]"
  - "metadatos [SQL Server], bases de datos disponibles para otras instancias"
  - "trabajos [Agente SQL Server], recrear en otra instancia de servidor"
  ...
  - "restaurar [SQL Server], en otra instancia de servidor"
  - "servidores vinculados [SQL Server], metadatos"
  - "Proveedor WMI para eventos de servidor, metadatos"
  - "adjuntar bases de datos [SQL Server]"
  - "trasvase de registros [SQL Server], metadatos"
  - "cifrado [SQL Server], metadatos"
  - "configuración de servidor [SQL Server]"
  - "consultas distribuidas [SQL Server], metadatos"
  - "procedimientos almacenados extendidos [SQL Server], metadatos"
  - "credenciales [SQL Server], metadatos"
...
##  <a name="server_configuration_settings"></a> Parámetros de configuración del servidor  
...
##  <a name="user_defined_error_messages"></a> Mensajes de error definidos por el usuario  
 Los mensajes de error definidos por el usuario residen en la vista de catálogo [sys.messages](../Topic/sys.messages%20\(Transact-SQL\).md). Esta vista se almacena en la base de datos **maestra**. Si una aplicación de la base de datos depende de los mensajes de error definidos por el usuario y la base de datos pasa a estar disponible en otra instancia del servidor, use [sp_addmessage](../../relational-databases/system-stored-procedures/sp-addmessage-transact-sql.md) para agregar esos mensajes de error en la instancia de servidor de destino.  

  
##  <a name="event_notif_and_wmi_events"></a> Notificaciones de eventos y eventos del Instrumental de administración de Microsoft Windows (WMI) (en el nivel de servidor)  
...
> **IMPORTANTE:** [!INCLUDE[ssNoteDepFutureAvoid](../../includes/ssnotedepfutureavoid-md.md)] En su lugar, use la [integración con CLR](../Topic/Common%20Language%20Runtime%20\(CLR\)%20Integration%20Overview.md).  
...
##  <a name="sb_applications"></a> Aplicaciones de Service Broker  
