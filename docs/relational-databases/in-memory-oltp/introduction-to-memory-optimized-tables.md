...
[!INCLUDE[tsql-appliesto-ss2016-asdb-xxxx-xxx_md](../../includes/tsql-appliesto-ss2016-asdb-xxxx-xxx-md.md)]
  ...
  Las tablas con optimización para memoria son tablas creadas por medio de [CREATE TABLE &#40;Transact-SQL&#41;](../../t-sql/statements/create-table-transact-sql.md).  
...
A partir de SQL Server 2016, y en Azure SQL Database, no existen limitaciones para [intercalaciones o páginas de códigos](../../relational-databases/collations/collation-and-unicode-support.md) que son específicas de OLTP en memoria.
 ...
 Además de las tablas con optimización para memoria durables predeterminadas, [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] también admite tablas con optimización para memoria no durables, que no se registran y cuyos datos no se guardan en el disco. Esto significa que las transacciones en estas tablas no requieren ninguna E/S de disco, pero los datos no se recuperarán si hay un bloqueo o una conmutación por error del servidor.  
 ...
 Las tablas con optimización para memoria son siempre con control de versiones. Esto significa que cada fila de la tabla puede tener varias versiones. Todas las versiones de fila se mantienen en la misma estructura de datos de la tabla. El control de versiones de fila se utiliza para permitir las lecturas y las escrituras simultáneas en la misma fila. Para obtener más información sobre las lecturas y las escrituras simultáneas en la misma fila, vea [Transactions with Memory-Optimized Tables](../../relational-databases/in-memory-oltp/transactions-with-memory-optimized-tables.md)(Transacciones con tablas con optimización para memoria).  
...
![Control de varias versiones.](../../relational-databases/in-memory-oltp/media/hekaton-tables-1.png "Control de varias versiones.")  
 ...
 La estructura de datos de una tabla con optimización para memoria se puede considerar como una colección de versiones de fila. Las filas de tablas basadas en disco se organizan en páginas y extensiones, las filas individuales se direccionan con el desplazamiento de página y número de página, y las versiones de fila de las tablas con optimización para memoria se direccionan con punteros de memoria de 8 bytes.  
  
 Se puede tener acceso a los datos de las tablas con optimización para memoria de dos maneras:  
 ...
 *No puede tener acceso a una tabla con optimización para memoria o a un procedimiento almacenado compilado de forma nativa desde la conexión de contexto (la conexión de [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] al ejecutar un módulo CLR). Sin embargo, puede crear y abrir otra conexión en la que pueda tener acceso a las tablas con optimización para memoria y a los procedimientos almacenados compilados de forma nativa.  
...
*[!INCLUDE[tsql](../../includes/tsql-md.md)] :* OLTP en memoria alcanza el máximo rendimiento cuando se usan procedimientos almacenados compilados de forma nativa en lugar de procedimientos almacenados interpretados o de ejecución de consultas. Puede haber un beneficio para obtener acceso a las tablas con optimización para memoria desde este tipo de procedimientos almacenados.  
  
*Examen de rango frente a búsqueda de punto:* los índices no agrupados con optimización para memoria admiten los exámenes de rango y los exámenes ordenados. Para las búsquedas de puntos, los índices hash con optimización para memoria tienen mejor rendimiento que los índices no clúster con optimización para memoria. Los índices no clúster con optimización para memoria tienen mejor rendimiento que los índices basados en disco.

- A partir de SQL Server 2016, el plan de consulta para una tabla con optimización para memoria puede examinar la tabla en paralelo. Esto mejora el rendimiento de las consultas de análisis.
