---
title: Metadatos de parámetros y resultados ? Microsoft Docs
ms.custom: ''
ms.date: 03/04/2017
ms.prod: sql
...
|SQL_DESC_TYPE_NAME|**Fecha**|**time**|**smalldatetime** en IRD, **datetime2** en IPD|**datetime** en IRD, **datetime2** en IPD|**datetime2**|datetimeoffset|  
 ...
 **datetime2** se devuelve como el nombre de tipo para **smalldatetime** y **datetime** porque el controlador usa esto como un tipo común para transmitir todos los valores **de SQL_TYPE_TIMESTAMP** al servidor.  
  
 SQL_CA_SS_VARIANT_SQL_TYPE es un nuevo campo descriptor. Este campo se agregó al IRD y al IPD para permitir que las aplicaciones especifiquen el tipo de valor asociado con las columnas y parámetros **de sqlvariant** (SQL_SSVARIANT)  
  
 SQL_CA_SS_SERVER_TYPE es un nuevo campo solo IPD que permite a las aplicaciones controlar la forma en que se envían al servidor los valores para parámetros enlazados como SQL_TYPE_TYPETIMESTAMP (o como SQL_SS_VARIANT con un tipo C SQL_C_TYPE_TIMESTAMP). Si SQL_DESC_CONCISE_TYPE es SQL_TYPE_TIMESTAMP (o se SQL_SS_VARIANT y el tipo C se SQL_C_TYPE_TIMESTAMP) cuando se llama a SQLExecute o SQLExecDirect, el valor de SQL_CA_SS_SERVER_TYPE determina el tipo de secuencia de datos tabular (TDS) del valor del parámetro, como se indica a continuación:  
...
-   Cuando una aplicación fuerza una preparación no diferida llamando a SQLPrepare con disabled aferred prepare o llamando a SQLNumResultCols, SQLDescribeCol o SQLDescribeParam para una instrucción que está preparada pero no ejecutada.  
 ...
 El atributo SQL_CA_SS_SERVER_TYPE puede ser utilizado por aplicaciones que dependen de la funcionalidad admitida por **datetime** y **smalldatetime**, pero no **datetime2**. Por ejemplo, **datetime2** requiere el uso de las funciones **dateadd** y **datediif,** mientras que **datetime** y **smalldatetime** también permiten operadores aritméticos. La mayoría de las aplicaciones no necesitarán usar este atributo, y debe evitarse su uso.  
...
|SQL_DESC_LOCAL_TYPE_NAME|**Fecha**|**time**|**smalldatetime**|**datetime**|**datetime2**|datetimeoffset|  
|SQL_DESC_OCTET_LENGTH|6|12|4|8|16|20|  
|SQL_DESC_PRECISION|0|0..7|0|3|0..7|0..7|  
|SQL_DESC_SCALE|0|0..7|0|3|0..7|0..7|  
|SQL_DESC_SEARCHABLE|SQL_PRED_SEARCHABLE|SQL_PRED_SEARCHABLE|SQL_PRED_SEARCHABLE|SQL_PRED_SEARCHABLE|SQL_PRED_SEARCHABLE|SQL_PRED_SEARCHABLE|  
|SQL_DESC_TYPE|SQL_DATETIME|SQL_SS_TIME2|SQL_DATETIME|SQL_DATETIME|SQL_DATETIME|SQL_SS_TIMESTAMPOFFSET|  
|SQL_DESC_TYPE_NAME|**Fecha**|**time**|**smalldatetime**|**datetime**|**datetime2**|datetimeoffset|  
