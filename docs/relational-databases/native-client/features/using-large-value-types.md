...
ms.locfileid: "73761293"
---
# <a name="using-large-value-types"></a>Usar tipos de valor grande
[!INCLUDE[appliesto-ss-asdb-asdw-pdw-md](../../../includes/appliesto-ss-asdb-asdw-pdw-md.md)]

  Antes de [!INCLUDE[ssVersion2005](../../../includes/ssversion2005-md.md)], trabajar con tipos de datos de valores grandes requería un tratamiento especial. Los tipos de datos de valores grandes son aquellos que superan el tamaño de fila máximo de 8 KB. [!INCLUDE[ssVersion2005](../../../includes/ssversion2005-md.md)] presentó un especificador **Max** para los tipos de datos **VARCHAR**, **nvarchar** y **varbinary** para permitir el almacenamiento de valores de hasta 2 ^ 31-1 bytes. Las columnas de tabla y las variables de [!INCLUDE[tsql](../../../includes/tsql-md.md)] pueden especificar tipos de datos **VARCHAR (Max)** , **nvarchar (Max)** o **varbinary (Max)** .  
...
## <a name="sql-server-native-client-ole-db-provider"></a>Proveedor OLE DB de SQL Server Native Client  
 ...
 Para admitir el streaming, el proveedor de OLE DB de [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Native Client requiere que se tenga acceso a los parámetros de longitud variable en orden secuencial. Esto significa que DBPROP_ACCESSORDER tiene que establecerse en DBPROPVAL_AO_SEQUENTIALSTORAGEOBJECTS o DBPROPVAL_AO_SEQUENTIAL cada vez que las columnas o los parámetros de salida **varchar(max)** , **nvarchchar(max)** o **varbinary(max)** se enlazan a DBTYPE_IUNKNOWN. Si no se cumple esta restricción de orden de acceso, se producirá un error en las llamadas a **IRowset::GetData** con DBSTATUS_E_UNAVAILABLE. Esta restricción no se aplica cuando no se realizan enlaces de salida mediante DBTYPE_IUNKNOWN.  
  
 El proveedor de [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] de OLE DB Native Client también admite parámetros de salida de enlace como DBTYPE_IUNKNOWN para tipos de datos de valores grandes a fin de facilitar escenarios en los que un procedimiento almacenado devuelve tipos de valores grandes como valores devueltos que se exponen como DBTYPE_IUNKNOWN al Nº.  
...
-   Enlazar como DBTYPE_IUNKNOWN y usar la transmisión por secuencias.  
 ...
 Al notificar el tamaño máximo de una columna, el proveedor de OLE DB de [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Native Client informará de:  
...
```  
 ...
 Para obtener más información sobre cómo el proveedor de OLE DB de [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Native Client expone tipos de datos de valores grandes, vea [blobs y objetos OLE](../../../relational-databases/native-client-ole-db-blobs/blobs-and-ole-objects.md).  
  
## <a name="sql-server-native-client-odbc-driver"></a>Controlador ODBC de SQL Server Native Client  
 El controlador ODBC de [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Native Client expone los tipos **VARCHAR (Max)** , **varbinary (Max)** y **nvarchar (max)** como SQL_VARCHAR, SQL_VARBINARY y SQL_WVARCHAR en funciones de la API de ODBC que aceptan o devuelven tipos de datos ODBC SQL.  
