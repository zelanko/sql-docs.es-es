...
ms.locfileid: "81303758"
---
# <a name="filestream-support-odbc"></a>Compatibilidad con FILESTREAM (ODBC)
[!INCLUDE[appliesto-ss-xxxx-xxxx-xxx-md](../../../includes/appliesto-ss-xxxx-xxxx-xxx-md.md)]

  ODBC en [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Native Client admite la característica mejorada FILESTREAM. Para obtener más información acerca de esta característica, vea [Compatibilidad con FILESTREAM](../../../relational-databases/native-client/features/filestream-support.md). Para obtener un ejemplo que muestra la compatibilidad de ODB con FILESTREAM, vea [Enviar y recibir datos de forma incremental con FILESTREAM &#40;&#41;ODBC ](../../../relational-databases/native-client-odbc-how-to/send-and-receive-data-incrementally-with-filestream-odbc.md).  
  
 Para enviar y recibir valores **varbinary(max)** superiores a 2 GB, una aplicación debe enlazar parámetros mediante SQLBindParameter con *ColumnSize* establecido en **SQL_SS_LENGTH_UNLIMITED**y establecer el contenido de *StrLen_or_IndPtr* **en SQL_DATA_AT_EXEC** antes de SQLExecDirect o SQLExecute.  
  
 Al igual que con cualquier parámetro de datos en ejecución, los datos se proporcionarán con SQLParamData y SQLPutData.  
 ...
 Puede actualizar los datos de FILESTREAM si están enlazados con SQLBindCol.  
  
 Si llama a SQLFetch en una columna enlazada, recibirá una advertencia de "datos truncados" si el búfer no es lo suficientemente grande como para contener todo el valor. Ignore esta advertencia y actualice los datos de esta columna enlazada con SQLParamData y SQLPutData llamadas. Puede actualizar los datos de FILESTREAM mediante SQLSetPos si está enlazado con SQLBindCol.  
  
## <a name="example"></a>Ejemplo  
 Las columnas FILESTREAM se comportan exactamente igual que las columnas **varbinary(max),** pero sin un límite de tamaño. Se enlazan como SQL_VARBINARY. (SQL_LONGVARBINARY se utiliza con columnas de imagen y hay restricciones en este tipo. Por ejemplo, SQL_LONGVARBINARY connot se utilice como parámetro de salida.) En los ejemplos siguientes se muestra acceso NTFS directo para las columnas FILESTREAM. En estos ejemplos se entiende que se ha ejecutado el siguiente código [!INCLUDE[tsql](../../../includes/tsql-md.md)] en la base de datos:  
