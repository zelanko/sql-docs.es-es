---
title: "Configurar IIS para la sincronizaci&#243;n web | Microsoft Docs"
ms.custom: ""
ms.date: "03/14/2017"
...
helpviewer_keywords: 
  ...
  - "websync.log"
  - "Sincronización web, servidores IIS"
...
---
# Configurar IIS para la sincronizaci&#243;n web
...
-   Replicación [!INCLUDE[tsql](../../includes/tsql-md.md)] programming: [Cómo: configurar una suscripción para usar la sincronización Web (programación de replicación de Transact-SQL)](http://msdn.microsoft.com/library/ms345206.aspx)  
  
-   RMO: [Cómo: configurar una suscripción para utilizar la sincronización Web (programación RMO)](http://msdn.microsoft.com/library/ms345207.aspx)  
...
#### Para especificar un certificado para su implementación  
  
1.  Inicie sesión como administrador en el equipo en el que se ejecuta IIS.  
  
2.  Iniciar **Internet Information Services (IIS) Manager**:  
...
3.  Ejecute el Asistente para certificados IIS:  
    ...
    1.  En **Internet Information Services (IIS) Manager**, expanda la **equipo local** nodo y, a continuación, expanda la **sitios Web** carpeta.  
...
#### Para especificar un certificado para su comprobación en IIS 6.0  
  
1.  Inicie sesión como administrador en el equipo en el que se ejecuta IIS.  
  
2.  Descargue e instale SelfSSL. De forma predeterminada, la aplicación se instala en \<*unidad*>: \Program Files\IIS Resources\SelfSSL. Métodos abreviados de la aplicación y la documentación se copian en \<*unidad*>: \Documents and Settings\All Users\Menú inicio\programas\iis Resources\SelfSSL.  
...
## Instalar los componentes de conectividad y SQL Server Management Studio  
  
#### Para instalar los componentes de conectividad de SQL Server y SQL Server Management Studio  
  
1.  Inicie sesión como administrador en el equipo en el que se ejecuta IIS.  
  
2.  Inicie el Asistente para la instalación de [!INCLUDE[ssSQL11](../../includes/sssql11-md.md)] desde el disco de instalación de [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] . Para obtener más información acerca de cómo utilizar este asistente, consulte [instalar SQL Server 2016 desde el Asistente para instalación & #40; El programa de instalación & #41;](../../database-engine/install-windows/install-sql-server-2016-from-the-installation-wizard-setup.md).  
  
3.  En la página **Selección de características** , seleccione **Conectividad con las herramientas de cliente**.  
  
4.  Si planea usar el Asistente para configurar la sincronización Web, seleccione **Herramientas de administración - básica**.  
...
## Configurar el equipo en el que se ejecuta IIS mediante el Asistente para configurar la sincronización web  
...
3.  Expanda el **publicaciones locales** carpeta, haga clic en la publicación y, a continuación, haga clic en **Configurar sincronización Web**.  
...
6.  En la página **Información de directorio virtual** :  
    ...
    2.  Escriba una ruta de acceso al directorio virtual en el cuadro **Ruta de acceso** . Por ejemplo, si escribió **websync1** en el **Alias** escriba **C:\Inetpub\wwwroot\websync1** en el **ruta de acceso** cuadro. Haga clic en **Siguiente**.  
...
8.  En la página **Acceso a directorio** :  
    ...
    1.  Haga clic en **Agregar**y, a continuación, en el cuadro de diálogo **Seleccionar usuarios o grupos** , agregue las cuentas con las que los suscriptores se conectarán a IIS. Estas son las cuentas que se especifican en la **información del servidor Web** página del Asistente para nueva suscripción o como el valor de la [sp_addmergepullsubscription_agent](../../relational-databases/system-stored-procedures/sp-addmergepullsubscription-agent-transact-sql.md)*@internet_login* parámetro.  
  
9. En la página **Acceso a recurso compartido de instantáneas** , escriba el recurso compartido de instantáneas: en este recurso compartido se establecen los permisos adecuados para que los suscriptores puedan obtener acceso a los archivos de instantáneas. Para obtener más información acerca de los permisos para el recurso compartido, consulte [proteger la carpeta de instantáneas](../../relational-databases/replication/security/secure-the-snapshot-folder.md).  
...
## Configurar manualmente el equipo en el que se ejecuta IIS  
...
1.  Cree un directorio de archivos en el equipo que ejecute IIS para que contenga replisapi.dll. Puede crear el directorio donde desee, pero se recomienda que cree el directorio en el \<*unidad*>: \Inetpub directory. Por ejemplo, cree el directorio \<*unidad*>: \Inetpub\SQLReplication\\.  
...
1.  Cree una cuenta local en el equipo en el que se ejecuta IIS:  
    ...
    1.  Haga clic en **Mi PC**, y, a continuación, haga clic en **administrar**.  
...
2.  Agregue la cuenta al grupo IIS_WPG:  
    ...
    2.  Haga clic en **IIS_WPG**, y, a continuación, haga clic en **Agregar al grupo**.  
  
    3.  En la **Propiedades de IIS_WPG** cuadro de diálogo, haga clic en **Agregar**.  
    ...
    6.  En la **Seleccionar usuarios** cuadro de diálogo y la **Propiedades de IIS_WPG** cuadro de diálogo, haga clic en **Aceptar**.  
  
3.  Conceda a la cuenta los permisos mínimos en la carpeta que contiene el archivo replisapi.dll:  
  
    1.  Busque la carpeta que ha creado para replisapi.dll, haga clic en la carpeta y, a continuación, haga clic en **Compartir y seguridad**.  
    ...
    5.  Asegúrese de que la cuenta tiene permisos de sólo **lectura**, **leer y ejecutar**, y **Enumerar contenido de carpeta** permisos.  
...
4.  Crear un grupo de aplicaciones en **Internet Information Services (IIS) Manager**:  
    ...
    3.  En **Internet Information Services (IIS) Manager**, expanda la **equipo local** nodo.  
  
    4.  Haga clic en **grupos de aplicaciones**, seleccione **nuevo** y, a continuación, haga clic en **grupo de aplicaciones**.  
...
5.  Asocie la cuenta al grupo de aplicaciones:  
    ...
    1.  En **Internet Information Services (IIS) Manager**, expanda la **equipo local** nodo y, a continuación, expanda **grupos de aplicaciones**.  
  
    2.  Haga clic en el grupo de aplicaciones que creó y, a continuación, haga clic en **propiedades**.  
  
    3.  En el **\< NombreGrupoAplicaciones> propiedades** cuadro de diálogo el **identidad** haga clic en **Configurable**.  
...
6.  Asocie el grupo de aplicaciones al directorio virtual que se utiliza para la sincronización web:  
    ...
    1.  En **Internet Information Services (IIS) Manager**, expanda la **equipo local** nodo y, a continuación, expanda **sitios Web**.  
  
    2.  Expanda el sitio Web que se utiliza para la sincronización Web, haga clic en el directorio virtual que creó para la sincronización Web y, a continuación, haga clic en **propiedades**.  
  
    3.  En el **Directorio Virtual** ficha de la **\< nombreDeDirectorioVirtual> propiedades** cuadro de diálogo desde el **grupo de aplicaciones** lista desplegable, seleccione el grupo de aplicaciones que se creó en el paso 5.  
