---
title: "Configurar la publicaci&#243;n y la distribuci&#243;n | Microsoft Docs"
ms.custom: ""
ms.date: "08/25/2016"
...
helpviewer_keywords: 
  ...
  - "publicación [replicación de SQL Server], configurar"
...
2.  Cree una instancia de la <xref:Microsoft.SqlServer.Replication.ReplicationServer> clase. Pase la clase <xref:Microsoft.SqlServer.Management.Common.ServerConnection> del paso 1.  
  
3.  Cree una instancia de la <xref:Microsoft.SqlServer.Replication.DistributionDatabase> clase.  
  
4.  Establecer el <xref:Microsoft.SqlServer.Replication.DistributionDatabase.Name%2A> propiedad al nombre de la base de datos y establezca la <xref:Microsoft.SqlServer.Replication.ReplicationObject.ConnectionContext%2A> propiedad a la <xref:Microsoft.SqlServer.Management.Common.ServerConnection> del paso 1.  
  
5.  Instale el distribuidor mediante una llamada a la <xref:Microsoft.SqlServer.Replication.ReplicationServer.InstallDistributor%2A> método. Pasar el <xref:Microsoft.SqlServer.Replication.DistributionDatabase> objeto del paso 3.  
  
6.  Cree una instancia de la <xref:Microsoft.SqlServer.Replication.DistributionPublisher> clase.  
  
7.  Establezca las siguientes propiedades de <xref:Microsoft.SqlServer.Replication.DistributionPublisher>:  
  
    -   <xref:Microsoft.SqlServer.Replication.DistributionPublisher.Name%2A> : el nombre del publicador.  
  
    -   <xref:Microsoft.SqlServer.Replication.ReplicationObject.ConnectionContext%2A> - la <xref:Microsoft.SqlServer.Management.Common.ServerConnection> del paso 1.  
    ...
    -   <xref:Microsoft.SqlServer.Replication.DistributionPublisher.PublisherSecurity%2A> -modo de seguridad utilizado al conectarse al publicador. <xref:Microsoft.SqlServer.Replication.ConnectionSecurityContext.WindowsAuthentication%2A> se recomienda.  
  
8.  Llame a la <xref:Microsoft.SqlServer.Replication.DistributionPublisher.Create%2A> método.  
...
5.  Instale el distribuidor mediante una llamada a la <xref:Microsoft.SqlServer.Replication.ReplicationServer.InstallDistributor%2A> método. Especifique una contraseña segura (utilizada por el publicador al conectarse al distribuidor remoto) y la <xref:Microsoft.SqlServer.Replication.DistributionDatabase> objeto del paso 3. Para más información, vea [Proteger el distribuidor](../../relational-databases/replication/security/secure-the-distributor.md).  
...
7.  Establezca las siguientes propiedades de <xref:Microsoft.SqlServer.Replication.DistributionPublisher>:  
    ...
    -   <xref:Microsoft.SqlServer.Replication.DistributionPublisher.Name%2A> : el nombre del servidor publicador local.  
...
10. Cree una instancia de la <xref:Microsoft.SqlServer.Replication.ReplicationServer> clase. Pase la clase <xref:Microsoft.SqlServer.Management.Common.ServerConnection> del paso 9.  
  
11. Llame a la <xref:Microsoft.SqlServer.Replication.ReplicationServer.InstallDistributor%2A> método. Pase el nombre del distribuidor remoto y la contraseña para el distribuidor remoto especificado en el paso 5.  
