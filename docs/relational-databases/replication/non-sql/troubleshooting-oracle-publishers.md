...
ms.topic: "article"
helpviewer_keywords: 
  - "publicación de Oracle [replicación de SQL Server], solución de problemas"
  - "solución de problemas, [replicación de SQL Server], publicación de Oracle"
...
## Aparece un error referente al cliente Oracle y al software de red  
 La cuenta bajo la que [!INCLUDE[msCoName](../../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] se ejecuta en el distribuidor debe tener permisos de lectura y permisos de ejecución para el directorio (y todos los subdirectorios) en que se instala el software de red de cliente de Oracle. Si no se conceden estos permisos o los componentes del cliente Oracle no están instalados correctamente, recibirá el siguiente mensaje de error:  
...
## El distribuidor de SQL Server no puede conectarse a la instancia de base de datos de Oracle  
...
-   El inicio de sesión que utiliza la replicación para conectarse al publicador de Oracle tiene permisos suficientes. Para obtener más información, consulte [configurar un publicador de Oracle](../../../relational-databases/replication/non-sql/configure-an-oracle-publisher.md).  
...
-   Se utiliza el directorio de inicio de Oracle (Oracle Home) y la ruta de acceso correctos. Aunque solamente tenga un conjunto de archivos binarios de Oracle instalados en el distribuidor [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] , asegúrese de que las variables de entorno relacionadas con el directorio de inicio de Oracle están configuradas correctamente. Si cambia los valores de las variables de entorno, deberá detener y reiniciar [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] para que se aplique el cambio.  
 ...
 Para obtener más información sobre cómo configurar y probar la conectividad, vea "instalar y configurar el Software de red de cliente de Oracle en el [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] distribuidor" de [configurar un publicador de Oracle](../../../relational-databases/replication/non-sql/configure-an-oracle-publisher.md).  
  
## El publicador de Oracle está asociado con otro distribuidor  
...
-   "Instancia de servidor de oracle ' \<*OraclePublisherName*>' se ha configurado anteriormente para utilizar ' \<*SQLServerDistributorName*>' como su distribuidor. Para empezar a utilizar ' \<*NewSQLServerDistributorName*>' como su distribuidor, debe quitar la configuración actual de replicación en la instancia de servidor de Oracle, lo que eliminará todas las publicaciones en esa instancia del servidor. "  
  
-   "Servidor de oracle ' \<*OracleServerName*>' ya está definido como editor ' \<*OraclePublisherName*>' en el distribuidor ' \<*SQLServerDistributorName*>.*\< Nombredelabasededatosdedistribución>*'. Quite el publicador o el sinónimo público '*\< Nombredelsinónimo>*' para volver a. "  
...
## Aparece el error 21663 de SQL Server referente a la falta de una clave principal  
 ...
 "Se encuentra ninguna clave principal válida para la tabla de origen [\<*TableOwner*>]. [ \<*TableName*>] "  
...
## Aparece el error 21642 de SQL Server referente a un inicio de sesión duplicado en el servidor vinculado  
 ...
 "Los publicadores heterogéneos requieren un servidor vinculado. Un servidor vinculado denominado '*\< Nombredelservidorvinculado>*' ya existe. Quítelo o elija otro nombre de publicador."  
  
 Este error puede aparecer si intenta crear el servidor vinculado directamente o si quitó anteriormente la relación entre el publicador de Oracle y el distribuidor de [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] , y está intentando volver a configurarla. Si recibe este error al intentar volver a configurar el publicador, quite el servidor vinculado con [sp_dropserver & #40; Transact-SQL & #41;](../../../relational-databases/system-stored-procedures/sp-dropserver-transact-sql.md).  
  
 Si necesita conectarse al publicador de Oracle a través de una conexión de servidor vinculado, cree otro nombre del servicio TNS y, a continuación, use este nombre cuando se llama a [sp_addlinkedserver & #40; Transact-SQL & #41;](../../../relational-databases/system-stored-procedures/sp-addlinkedserver-transact-sql.md). Para obtener información acerca de cómo crear nombres de servicio TNS, vea la documentación de Oracle.  
  
## Aparece el error 21617 de SQL Server  
 En la publicación de Oracle se utiliza la aplicación SQL*PLUS de Oracle para descargar el paquete de código de compatibilidad con el publicador a la base de datos de Oracle. Antes de intentar configurar el publicador de Oracle, [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] comprueba que SQL\*PLUS es accesible a través de la ruta de acceso del sistema en el distribuidor. Si SQL\*PLUS no se puede cargar, se muestra el mensaje de error siguiente:  
 ...
 Busque SQL\*PLUS en el distribuidor. Para una instalación de cliente Oracle 10g, el nombre de este ejecutable es sqlplus.exe. Normalmente está instalado en %ORACLE_HOME%/bin. Para comprobar que la ruta de acceso de SQL\*PLUS aparece en la ruta de acceso del sistema, examine el valor de la variable del sistema **ruta de acceso**:  
  
1.  Haga clic en **Mi PC**, y, a continuación, haga clic en **propiedades**.  
...
5.  Haga clic en **Aceptar** en todos los cuadros de diálogo abiertos para salir y guardar los cambios.  
 ...
 Si no encuentra sqlplus.exe en el distribuidor, instale la versión actual del software de cliente de Oracle en el distribuidor. Para obtener más información, consulte [configurar un publicador de Oracle](../../../relational-databases/replication/non-sql/configure-an-oracle-publisher.md).  
  
## Aparece el error 21620 de SQL Server  
...
## Aparece el error 21624 o 21629 de SQL Server  
...
## Aparece el error 21626 o 21627 de SQL Server  
...
## Aparece el error 21628 de SQL Server  
...
3.  Navegue a HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Microsoft SQL Server\\*\< nombreInstancia>*\Providers.  
...
4.  Si determina que **AllowInProcess** tiene el valor **0**, actualice la entrada del Registro a **1**:  
    ...
    1.  Haga clic en la entrada y, a continuación, haga clic en **modificar**.  
  
    2.  En el cuadro de diálogo **Editar cadena** , escriba **1** en el campo **Datos del valor** .  
  
## Aparece el error 21684 de SQL Server  
...
## Tiene problemas de permisos para el esquema de usuario de la replicación  
 El esquema de usuario de replicación debe tener los permisos descritos en "Crear el usuario esquema manualmente" en [configurar un publicador de Oracle](../../../relational-databases/replication/non-sql/configure-an-oracle-publisher.md).  
  
## Error ORA-01000 de Oracle  
 ...
 Para evitar este problema, asegúrese de que el **max_open_cursors** configuración de las bases de datos de Oracle está establecido en un número suficientemente alto (al menos 1000). Para obtener más información acerca de este parámetro, vea la documentación de Oracle.  
  
## Error ORA-01555 de Oracle  
...
## Se han realizado cambios que requieren volver a configurar el publicador  
 Los cambios en los procedimientos o en las tablas de metadatos de la replicación obligan a quitar y configurar de nuevo el publicador. Para volver a configurar el publicador, debe quitarlo y configurarlo de nuevo con [!INCLUDE[ssManStudioFull](../../../includes/ssmanstudiofull-md.md)], Transact-SQL o RMO. Para obtener información acerca de cómo configurar el publicador, vea [configurar un publicador de Oracle](../../../relational-databases/replication/non-sql/configure-an-oracle-publisher.md).  
...
2.  Haga clic en **replicación**, y, a continuación, haga clic en **Propiedades del distribuidor**.  
...
-   Ejecutar **sp_dropdistpublisher**. Para obtener más información, consulte [sp_dropdistpublisher & #40; Transact-SQL & #41;](../../../relational-databases/system-stored-procedures/sp-dropdistpublisher-transact-sql.md).  
