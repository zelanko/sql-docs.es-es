---
title: Planificación de la atestación del Servicio de protección de host | Microsoft Docs
ms.custom: ''
ms.date: 10/12/2019
ms.prod: sql
ms.reviewer: vanto
...
No es necesario que el equipo que ejecuta [!INCLUDE [ssnoversion-md](../../../includes/ssnoversion-md.md)] esté unido a un dominio, pero, si es así, debe ser un dominio diferente del que usa el servidor HGS.
...
Tanto el cliente SQL como [!INCLUDE [ssnoversion-md](../../../includes/ssnoversion-md.md)] deberán ser capaces de comunicarse con HGS a través de HTTP. Opcionalmente, puede configurar HGS con un certificado de TLS para admitir conexiones HTTPS cifradas.
...
El arranque seguro debe contar con la atestación de TPM para garantizar que UEFI cargó un cargador de arranque legítimo firmado por Microsoft y que ningún rootkit o bootkit haya interceptado el proceso de arranque del hipervisor.
...
| RAM | 8 GB (mín) |
| NIC | 2 NIC recomendadas (1 para el tráfico de clúster, 1 para el servicio de HGS) |
...
- Compatibilidad con la CPU para tecnologías de virtualización:
  ...
  - Si está ejecutando [!INCLUDE [ssnoversion-md](../../../includes/ssnoversion-md.md)] en una máquina virtual, el hipervisor y la CPU física deben ofrecer funciones de virtualización anidadas. Consulte la sección del [modelo de confianza](#trust-model) para obtener información sobre las garantías al ejecutar enclaves de VBS en una máquina virtual.
    ...
    - Otros hipervisores y nubes públicas pueden admitir funciones de virtualización anidada que permiten Always Encrypted con enclaves de VBS también. Consulte la documentación de la solución de virtualización para obtener instrucciones de compatibilidad y configuración.
