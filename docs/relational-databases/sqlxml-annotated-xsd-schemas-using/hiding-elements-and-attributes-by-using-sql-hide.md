...
description: Aprenda a utilizar la anotación sql:hide para ocultar elementos y atributos al ejecutar una consulta XPath en un esquema XSD.
ms.custom: ''
ms.date: 03/16/2017
ms.prod: sql
...
### <a name="a-specifying-sqlhide-on-an-attribute"></a>A. Especificar sql:hide en un atributo  
 El esquema XSD de este ejemplo consta de **ContactID**un ** \<elemento de>Person.Contact** con los atributos ContactID , **FirstName**y **LastName** .  
  
 El ** \<elemento de>Person.Contact** es de tipo complejo y, por lo tanto, se asigna a la tabla del mismo nombre (asignación predeterminada). Todos los atributos de ** \<Person.Contact>** elemento son de tipo simple y se asignan a columnas con los mismos nombres en el Person.Contacttable en la base de datos AdventureWorks. En el esquema, la anotación **sql:hide** se especifica en el atributo **ContactID.** Cuando se especifica una consulta XPath en este esquema, el **ContactID** no se devuelve en el documento XML.  
...
```  
 ...
 Cuando **sql:hide** se especifica en un elemento, el elemento y sus atributos o elementos secundarios no aparecen en el documento XML que se genera. Aquí hay otro esquema XSD en el que **sql:hide** se especifica en el ** \<elemento oD>:**  
...
```  
 ...
 Cuando se especifica una `/Customers[@CID="1"]`consulta XPath (por ejemplo) en este esquema, el documento XML que se genera no incluye el ** \<** elemento oD>y sus elementos secundarios, como se muestra en este resultado parcial:  
