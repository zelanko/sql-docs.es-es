...
## <a name="sqlidentity-annotation"></a>Anotación sql:identity  
 Puede especificar el **: Identity** anotación en el esquema XSD en cualquier nodo que se asigna a una columna de base de datos de tipo IDENTITY. El valor especificado para esta anotación define cómo se actualiza la columna de tipo IDENTITY (ya sea utilizando el valor proporcionado en el diagrama de actualización para modificar la columna u omitiendo el valor, en cuyo caso se utiliza un valor generado por [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] para esta columna).  
