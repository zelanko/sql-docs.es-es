...
|**owning_principal_id**|**int**|Id. de la entidad de seguridad propietaria de esta entidad de seguridad. Todas las entidades de seguridad, excepto los roles de base de datos, deben pertenecer a **DBO**.|  
...
### <a name="d-listing-permissions-on-schema-objects-within-a-database"></a>D: enumerar los permisos de los objetos de esquema en una base de datos  
 La siguiente consulta combina `sys.database_principals` y `sys.database_permissions` con `sys.objects` y `sys.schemas` para enumerar los permisos concedidos o denegados a objetos de esquema específicos.  
