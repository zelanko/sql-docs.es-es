---
title: Sys.dm_exec_describe_first_result_set (Transact-SQL) | Documentos de Microsoft
ms.custom: 
ms.date: 06/10/2016
...
[!INCLUDE[tsql-appliesto-ss2012-asdb-xxxx-xxx-md](../../includes/tsql-appliesto-ss2012-asdb-xxxx-xxx-md.md)]
  ...
  Esta función de administración dinámica toma un [!INCLUDE[tsql](../../includes/tsql-md.md)] instrucción como un parámetro y describe los metadatos del primer conjunto de resultados para la instrucción.  
  
 **Sys.dm_exec_describe_first_result_set** tiene el mismo resultado definición como del conjunto [sys.dm_exec_describe_first_result_set_for_object &#40; Transact-SQL &#41; ](../../relational-databases/system-dynamic-management-views/sys-dm-exec-describe-first-result-set-for-object-transact-sql.md) y es similar a [sp_describe_first_result_set &#40; Transact-SQL &#41; ](../../relational-databases/system-stored-procedures/sp-describe-first-result-set-transact-sql.md).  
...
|**Source_server**|**sysname**|Nombre del servidor de origen (si se origina desde un servidor remoto). Se asigna el nombre tal y como aparece en sys.servers. Devuelve NULL si la columna se origina en el servidor local o si no se puede determinar en qué servidor se origina. Solo se rellena si se solicita buscar información.|  
...
|**order_by_list_length**|**smallint**|Longitud de la lista de ORDER BY. Se devuelve NULL si no hay ninguna lista ORDER BY o si la lista ORDER BY no se puede determinar de forma exclusiva. Observe que este valor será el mismo para todas las filas devueltas por sp_describe_first_result_set.|  
...
|12|OBJECT_TYPE_NOT_SUPPORTED|El @object_id pasado a la función es no admite (es decir, no un procedimiento almacenado)|  
|13|OBJECT_DOES_NOT_EXIST|El @object_id pasado a la función no se encontró en el catálogo del sistema.|  
...
### <a name="b-returning-information-about-a-procedure"></a>B. Devolver información acerca de un procedimiento  
 En el ejemplo siguiente se crea un procedimiento almacenado denominado pr_TestProc que devuelve dos conjuntos de resultados. En el ejemplo se muestra que **sys.dm_exec_describe_first_result_set** devuelve información sobre el primer conjunto de resultados en el procedimiento.  
