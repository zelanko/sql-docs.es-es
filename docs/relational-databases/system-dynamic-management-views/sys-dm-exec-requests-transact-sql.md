...
Devuelve información sobre cada una de las solicitudes que se [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]ejecutan en. Para obtener más información sobre las solicitudes, vea la [Guía de arquitectura de subprocesos y tareas](../../relational-databases/thread-and-task-architecture-guide.md).
...
|request_id|**Inter**|Id. de la solicitud. Es único en el contexto de la sesión. No admite valores NULL.|  
|start_time|**DateTime**|Marca de tiempo de la llegada de la solicitud. No admite valores NULL.|  
|status|**nvarchar(30**|Estado de la solicitud. Puede ser uno de los siguientes:<br /><br /> Información preliminar:<br />Running<br />Ejecutable<br />En espera<br />Suspended<br /><br /> No admite valores NULL.|  
...
|statement_start_offset|**Inter**|Número de caracteres en el lote que se está ejecutando actualmente o procedimiento almacenado en el que se inicia la instrucción que se está ejecutando actualmente. Se puede utilizar junto con la función de administración dinámica sql_handle, statement_end_offset y sys.dm_exec_sql_text para recuperar la instrucción de ejecución actual para la solicitud. Acepta valores NULL.|  
|statement_end_offset|**Inter**|Número de caracteres en el lote que se está ejecutando actualmente o procedimiento almacenado en el que finaliza la instrucción que se está ejecutando actualmente. Se puede utilizar junto con la función de administración dinámica sql_handle, statement_end_offset y sys.dm_exec_sql_text para recuperar la instrucción de ejecución actual para la solicitud. Acepta valores NULL.|  
...
|user_id|**Inter**|Identificador del usuario que envió la solicitud. No admite valores NULL.|  
...
|wait_time|**Inter**|Si la solicitud está actualmente bloqueada, esta columna devuelve la duración en milisegundos de la espera actual. No admite valores NULL.|  
...
|open_transaction_count|**Inter**|Número de transacciones abiertas para esta solicitud. No admite valores NULL.|  
|open_resultset_count|**Inter**|Número de conjuntos de resultados abiertos para esta solicitud. No admite valores NULL.|  
...
|cpu_time|**Inter**|Tiempo de CPU en milisegundos utilizado por la solicitud. No admite valores NULL.|  
|total_elapsed_time|**Inter**|Tiempo total transcurrido en milisegundos desde que llegó la solicitud. No admite valores NULL.|  
|scheduler_id|**Inter**|Identificador del programador que programa esta solicitud. No admite valores NULL.|  
...
|text_size|**Inter**|Valor de TEXTSIZE para esta solicitud. No admite valores NULL.|  
...
|quoted_identifier|**poco**|1 = El valor de QUOTED_IDENTIFIER es ON para la solicitud. De lo contrario, es 0.<br /><br /> No admite valores NULL.|  
|arithabort|**poco**|1 = El valor de ARITHABORT es ON para la solicitud. De lo contrario, es 0.<br /><br /> No admite valores NULL.|  
|ansi_null_dflt_on|**poco**|1 = El valor de ANSI_NULL_DFLT_ON es ON para la solicitud. De lo contrario, es 0.<br /><br /> No admite valores NULL.|  
|ansi_defaults|**poco**|1 = El valor de ANSI_DEFAULTS es ON para la solicitud. De lo contrario, es 0.<br /><br /> No admite valores NULL.|  
|ansi_warnings|**poco**|1 = El valor de ANSI_WARNINGS es ON para la solicitud. De lo contrario, es 0.<br /><br /> No admite valores NULL.|  
|ansi_padding|**poco**|1 = El valor de ANSI_PADDING es ON para la solicitud.<br /><br /> De lo contrario, es 0.<br /><br /> No admite valores NULL.|  
|ansi_nulls|**poco**|1 = El valor de ANSI_NULLS es ON para la solicitud. De lo contrario, es 0.<br /><br /> No admite valores NULL.|  
|concat_null_yields_null|**poco**|1 = El valor de CONCAT_NULL_YIELDS_NULL es ON para la solicitud. De lo contrario, es 0.<br /><br /> No admite valores NULL.|  
...
|lock_timeout|**Inter**|Tiempo de espera de bloqueo en milisegundos para esta solicitud. No admite valores NULL.|  
|deadlock_priority|**Inter**|Valor de DEADLOCK_PRIORITY para la solicitud. No admite valores NULL.|  
|row_count|**bigint**|Número de filas que esta solicitud ha devuelto al cliente. No admite valores NULL.|  
|prev_error|**Inter**|Último error que se ha producido durante la ejecución de la solicitud. No admite valores NULL.|  
|nest_level|**Inter**|Nivel de anidamiento actual del código que se está ejecutando en la solicitud. No admite valores NULL.|  
|granted_query_memory|**Inter**|Número de páginas asignadas a la ejecución de una consulta en la solicitud. No admite valores NULL.|  
|executing_managed_code|**poco**|Indica si una solicitud concreta está ejecutando actualmente objetos Common Language Runtime, como rutinas, tipos y desencadenadores. Se establece para todo el tiempo que un objeto Common Language Runtime esté en la pila, incluso mientras se ejecuta [!INCLUDE[tsql](../../includes/tsql-md.md)] desde Common Language Runtime. No admite valores NULL.|  
|group_id|**Inter**|Identificador del grupo de cargas de trabajo al que pertenece esta consulta. No admite valores NULL.|  
...
|statement_sql_handle|**varbinary (64)**|**Se aplica a**: [!INCLUDE[ssSQL14](../../includes/sssql14-md.md)] y versiones posteriores.<br /><br /> Identificador SQL de la consulta individual.<br /><br />Esta columna es NULL si Almacén de consultas no está habilitada para la base de datos. |  
|statement_context_id|**bigint**|**Se aplica a**: [!INCLUDE[ssSQL14](../../includes/sssql14-md.md)] y versiones posteriores.<br /><br /> La clave externa opcional para sys. query_context_settings.<br /><br />Esta columna es NULL si Almacén de consultas no está habilitada para la base de datos. |  
|dop |**Inter** |**Se aplica a**: [!INCLUDE[ssSQL15](../../includes/sssql15-md.md)] y versiones posteriores.<br /><br /> Grado de paralelismo de la consulta. |  
|parallel_worker_count |**Inter** |**Se aplica a**: [!INCLUDE[ssSQL15](../../includes/sssql15-md.md)] y versiones posteriores.<br /><br /> El número de trabajadores paralelos reservados si se trata de una consulta en paralelo.  |  
|external_script_request_id |**uniqueidentifier** |**Se aplica a**: [!INCLUDE[ssSQL15](../../includes/sssql15-md.md)] y versiones posteriores.<br /><br /> IDENTIFICADOR de solicitud de script externo asociado a la solicitud actual. |  
|is_resumable |**poco** |**Se aplica a**: [!INCLUDE[sssqlv14-md](../../includes/sssqlv14-md.md)] y versiones posteriores.<br /><br /> Indica si la solicitud es una operación de índice reanudable. |  
...
Si el usuario tiene `VIEW SERVER STATE` permiso en el servidor, el usuario verá todas las sesiones en ejecución en la instancia de [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]; de lo contrario, el usuario solo verá la sesión actual. `VIEW SERVER STATE`no se puede conceder en `sys.dm_exec_requests` Azure SQL Database por lo que siempre está limitado a la conexión actual.
...
En el ejemplo siguiente se consulta **Sys. dm_exec_requests** para buscar el lote interesante y `transaction_id` copiar su desde la salida.
...
A continuación, para buscar información de bloqueo, use `transaction_id` el copiado con la función del sistema **Sys. dm_tran_locks**.  
