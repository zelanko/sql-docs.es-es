...
> [!NOTE]  
> Para llamar [!INCLUDE[ssSDWfull](../../includes/sssdwfull-md.md)] a [!INCLUDE[ssPDW](../../includes/sspdw-md.md)]esto desde o , utilice el nombre **sys.dm_pdw_nodes_tran_locks**.  
...
|**request_mode**|**nvarchar(60)**|Modo de la solicitud. En el caso de solicitudes concedidas, se trata del modo concedido; en el caso de solicitudes en espera, se trata del modo que se está solicitando. <br /><br /> NULL = No se concede acceso al recurso. Sirve como marcador de posición.<br /><br /> Sch-S (estabilidad del esquema): garantiza que un elemento de esquema, como una tabla o un índice, no se descarta mientras que cualquier sesión contiene un bloqueo de estabilidad de esquema en el elemento de esquema.<br /><br /> Sch-M (modificación de esquema): debe tenerlo cualquier sesión que desee cambiar el esquema del recurso especificado. Garantiza que ninguna otra sesión se refiera al objeto indicado.<br /><br /> S (Compartido): a la sesión de retención se le concede acceso compartido al recurso.<br /><br /> U (Actualización): indica un bloqueo de actualización adquirido en los recursos que finalmente se pueden actualizar. Se utiliza para evitar una forma común de interbloqueo que tiene lugar cuando varias sesiones bloquean recursos para una posible actualización en el futuro.<br /><br /> X (Exclusivo): se concede acceso exclusivo a la sesión de retención al recurso.<br /><br /> IS (Intent Shared): indica la intención de colocar bloqueos S en algún recurso subordinado en la jerarquía de bloqueos.<br /><br /> IU (Actualización de intención): indica la intención de colocar bloqueos U en algún recurso subordinado en la jerarquía de bloqueos.<br /><br /> IX (Intención exclusiva): indica la intención de colocar bloqueos X en algún recurso subordinado en la jerarquía de bloqueos.<br /><br /> SIU (Actualización de intención compartida): indica el acceso compartido a un recurso con la intención de adquirir bloqueos de actualización en recursos subordinados en la jerarquía de bloqueos.<br /><br /> SIX (Exclusiva de intención compartida): indica el acceso compartido a un recurso con la intención de adquirir bloqueos exclusivos en recursos subordinados en la jerarquía de bloqueos.<br /><br /> UIX (Update Intent Exclusive): indica una retención de bloqueo de actualización en un recurso con la intención de adquirir bloqueos exclusivos en recursos subordinados en la jerarquía de bloqueos.<br /><br /> BU: se utiliza mediante operaciones masivas.<br /><br /> RangeS_S (Intervalo de claves compartido y bloqueo de recursos compartidos): indica el análisis del intervalo serializable.<br /><br /> RangeS_U (Intervalo de claves compartido y bloqueo de recursos de actualización): indica el análisis de actualizaciones serializable.<br /><br /> RangeI_N (Insertar bloqueo de intervalo de teclas y de recursos nulos): se utiliza para probar intervalos antes de insertar una nueva clave en un índice.<br /><br /> RangeI_S: bloqueo de conversión de intervalo de teclas, creado por una superposición de bloqueos de RangeI_N y S.<br /><br /> RangeI_U: bloqueo de conversión de intervalo de teclas, creado por una superposición de bloqueos RangeI_N y U.<br /><br /> RangeI_X: bloqueo de conversión de intervalo de teclas, creado por una superposición de bloqueos RangeI_N y X.<br /><br /> RangeX_S: bloqueo de conversión de intervalo de claves, creado por una superposición de RangeI_N y RangeS_S. Cerraduras.<br /><br /> RangeX_U: bloqueo de conversión de intervalo de claves, creado por una superposición de bloqueos de RangeI_N y RangeS_U.<br /><br /> RangeX_X (Intervalo de claves exclusivo y bloqueo de recursos exclusivos): se trata de un bloqueo de conversión que se usa al actualizar una clave en un intervalo.|  
|**request_type**|**nvarchar(60)**|Tipo de solicitud. El valor es LOCK.|  
|**request_status**|**nvarchar(60)**|Estado actual de esta solicitud. Los valores posibles son GRANTED, CONVERT, WAIT, LOW_PRIORITY_CONVERT, LOW_PRIORITY_WAIT o ABORT_BLOCKERS. Para obtener más información acerca de las esperas de prioridad baja y los bloqueadores de anulación, vea la sección *low_priority_lock_wait* de [ALTER INDEX &#40;Transact-SQLTransact-SQL&#41;](../../t-sql/statements/alter-index-transact-sql.md).|  
...
|**pdw_node_id**|**int**|**Se**aplica [!INCLUDE[ssSDWfull](../../includes/sssdwfull-md.md)]a : ,[!INCLUDE[ssPDW](../../includes/sspdw-md.md)]<br /><br /> Identificador del nodo en el que se encuentra esta distribución.|  
  
## <a name="permissions"></a>Permisos
On [!INCLUDE[ssNoVersion_md](../../includes/ssnoversion-md.md)], `VIEW SERVER STATE` requiere permiso.   
En [!INCLUDE[ssSDS_md](../../includes/sssds-md.md)] los niveles Premium, `VIEW DATABASE STATE` requiere el permiso en la base de datos. En [!INCLUDE[ssSDS_md](../../includes/sssds-md.md)] los niveles Estándar y Básico, requiere el administrador del **servidor** o una cuenta de administrador de **Azure Active Directory.**   
...
-   Sugerencias de bloqueo de tablas para especificar el nivel de bloqueo para una referencia individual de una tabla en una cláusula FROM. Para obtener sintaxis y restricciones, vea Sugerencias de [tabla &#40;Transact-SQLTransact-SQL&#41;](../../t-sql/queries/hints-transact-sql-table.md).  
...
|METADATA.XML_INDEX_QNAME|object_id = O, $qname_id = Q|[!INCLUDE[ssInternalOnly](../../includes/ssinternalonly-md.md)]|  
 ...
 Los siguientes XEvents están relacionados con la partición **SWITCH** y la reconstrucción de índices en línea. Para obtener información acerca de la sintaxis, vea [ALTER TABLE &#40;Transact-SQLTransact-SQL&#41;](../../t-sql/statements/alter-table-transact-sql.md) y ALTER INDEX &#40;[Transact-SQLTransact-SQL&#41;](../../t-sql/statements/alter-index-transact-sql.md).  
...
-   ddl_with_wait_at_low_priority  
 ...
 El **progress_report_online_index_operation** XEvent existente para las operaciones de índice en línea se amplió agregando **partition_number** y **partition_id**.  
...
[sys.dm_tran_database_transactions &#40;Transact-SQLTransact-SQL&#41;](../../relational-databases/system-dynamic-management-views/sys-dm-tran-database-transactions-transact-sql.md)      
...
[Funciones y vistas de administración dinámica relacionadas con transacciones &#40;Transact-SQLTransact-SQL&#41;](../../relational-databases/system-dynamic-management-views/transaction-related-dynamic-management-views-and-functions-transact-sql.md)      
