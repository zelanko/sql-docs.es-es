 ...
 **Espere, ¿qué es un LSN?** Todos los registros de la [registro de transacciones de SQL Server](../logs/the-transaction-log-sql-server.md) se identifica mediante un número de secuencia de registro (LSN). LSN se ordenan de manera que si LSN2 es mayor que LSN1, se produce el cambio descrito por la entrada del registro que hace referencia LSN2 **después** del cambio descrito por la entrada de registro LSN.  
  
 El LSN de una entrada de registro donde se ha producido un evento importante puede resultar útil para generar secuencias de restauración correcta. Porque los números LSN se ordenan, se puede comparar para comprobar su igualdad y desigualdad (es decir, \<, >, =, \<=, > =). Estas comparaciones son útiles para generar secuencias de restauración.  
 ...
 *< row_filter_option >* :: = {todos | todos con máscara | todos con combinación}  
 ...
 all  
 Devuelve el LSN del último cambio realizado en la fila y la operación necesaria para aplicar la fila en las columnas de metadatos __ $start_lsn y \_ \_$operation. La columna \_ \_$update_mask siempre es NULL.  
...
|*\<captura de columnas de la tabla de origen >*|varía|Las columnas restantes devueltas por la función son las columnas de la tabla de origen que se identificaron como columnas capturadas cuando se creó la instancia de captura. Si no se especificó ninguna columna en la lista de columnas capturadas, se devuelven todas las columnas de la tabla de origen.|  
