...
`[ @delete_history = ] delete_history`Especifica si se debe eliminar el historial del trabajo. *delete_history* es de **bit**y su valor predeterminado es **1**. Cuando *delete_history* es **1**, se elimina el historial de trabajos del trabajo. Cuando *delete_history* es **0**, no se elimina el historial de trabajos.  
 ...
 Tenga en cuenta que cuando se elimina un trabajo y no se elimina el historial, la información histórica del trabajo no se mostrará [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] en el historial de trabajos de la interfaz gráfica de usuario del agente, pero la información seguirá residiendo en la tabla **sysjobhistory** de la base de datos **msdb** .  
