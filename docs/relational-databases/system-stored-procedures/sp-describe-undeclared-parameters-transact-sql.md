...
-   Una expresión con tipos de datos que no dependen de los parámetros no declarados para todas las entradas.  
 ...
 Por ejemplo, considere la consulta `SELECT dbo.tbl(@p1) + c1 FROM t1 WHERE c2 = @p2 + 2`. Las expresiones dbo.tbl (@p1) + c1 y c2 tienen tipos de datos y expresión @p1 y @p2 + 2 no lo hacen.  
...
-   El tipo de datos al que la instrucción se va a convertir.  
 ...
 Si @p no está incluido en ninguna expresión enumerada al principio del paso 2, el algoritmo de deducción de tipo determina que E (@p) es la expresión escalar más grande que contiene @p, mientras que el algoritmo de deducción de tipo no proceso de un tipo de datos de destino TT (@p) para E (@p). Por ejemplo, si la consulta es SELECT `@p + 2` , a continuación, E (@p) = @p + 2, y no hay ningún TT (@p).  
...
-   Deducción simple  
    ...
    ```  
     ...
     Tipo de datos de @p1, @p2, y @p3 será el tipo de datos de c1, el tipo de datos devuelto de dbo.tbl y el tipo de datos de parámetro para dbo.tbl respectivamente.  
