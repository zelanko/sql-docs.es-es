...
Cree una credencial con ámbito de base de datos en una base de datos de usuario y especifique las credenciales de administrador para la base de datos maestra. Cree un origen de datos externo que apunte a la base de datos maestra y especifique la credencial con ámbito de base de datos. A continuación, en el ejemplo de la base de datos de `sp_set_firewall_rule` usuario, se ejecuta el procedimiento en la base de datos maestra. El `sp_set_firewall_rule` procedimiento requiere 3 parámetros y requiere que el `@name` parámetro sea Unicode.
