...
## <a name="remarks"></a>Comentarios  
 parámetros de sp_executesql deben especificarse en el orden específico tal como se describe en la sección "Sintaxis" anteriormente en este tema. Si los parámetros se escriben desordenados, se producirá un mensaje de error.  
  
 sp_executesql tiene el mismo comportamiento que EXECUTE en cuanto a los lotes, el ámbito de los nombres y el contexto de las bases de datos. El [!INCLUDE[tsql](../../includes/tsql-md.md)] instrucción o lote de sp_executesql @stmt parámetro no se compila hasta que se ejecuta la instrucción sp_executesql. Entonces, el contenido de @stmt se compila y se ejecuta como un plan de ejecución independiente del que corresponde al lote que ha llamado a sp_executesql. El lote de sp_executesql no puede hacer referencia a las variables declaradas en el lote que llama a sp_executesql. Los cursores o las variables locales del lote de sp_executesql no son visibles para el lote que llama a sp_executesql. Los cambios en el contexto de base de datos solo se mantienen hasta el final de la instrucción sp_executesql.  
...
### <a name="c-using-the-output-parameter"></a>C. Utilizar el parámetro OUTPUT  
 En el ejemplo siguiente se usa un `OUTPUT` parámetro para almacenar el conjunto de resultados generado por la `SELECT` instrucción en el `@SQLString` parámetro. Dos `SELECT` , a continuación, se ejecutan las instrucciones que utilizan el valor de la `OUTPUT` parámetro.  
...
```  
 ...
 Para obtener ejemplos adicionales, vea [sp_executesql (Transact-SQL)](http://msdn.microsoft.com/library/ms188001.aspx).  
