...
|Value|Descripción|  
|-----------|-----------------|  
|**TODOS**|Información del aspecto del trabajo|  
...
|**dimensional**|En ejecución.|  
|**dos**|En espera de subproceso.|  
|**3**|Entre reintentos.|  
|**4**|Activos.|  
|**5**|Suspendido.|  
...
|**start_step_id**|**Inter**|Id. del paso del trabajo en el que debe comenzar la ejecución.|  
|**Categoría**|**sysname**|Categoría del trabajo|  
|**propietario**|**sysname**|Propietario del trabajo.|  
|**notify_level_eventlog**|**Inter**|**Máscara** de archivo que indica en qué circunstancias se debe registrar un evento de notificación en el registro de aplicación de Microsoft Windows. Puede ser uno de estos valores:<br /><br /> **0** = nunca<br /><br /> **1** = cuando un trabajo se realiza correctamente<br /><br /> **2** = cuando se produce un error en el trabajo<br /><br /> **3** = cada vez que se completa el trabajo (independientemente del resultado del trabajo)|  
|**notify_level_email**|**Inter**|**Máscara de máscara** que indica en qué circunstancias se debe enviar una notificación por correo electrónico cuando se completa un trabajo. Los valores posibles son los mismos que para **notify_level_eventlog**.|  
|**notify_level_netsend**|**Inter**|**Máscara** de red que indica en qué circunstancias se debe enviar un mensaje de red cuando se completa un trabajo. Los valores posibles son los mismos que para **notify_level_eventlog**.|  
|**notify_level_page**|**Inter**|**Máscara de máscara** que indica en qué circunstancias se debe enviar una página cuando se completa un trabajo. Los valores posibles son los mismos que para **notify_level_eventlog**.|  
...
|**delete_level**|**Inter**|**Máscara de máscara** que indica en qué circunstancias se debe eliminar el trabajo cuando se completa un trabajo. Los valores posibles son los mismos que para **notify_level_eventlog**.|  
...
|**version_number**|**Inter**|Versión del trabajo (se actualiza automáticamente cada vez que se modifica el trabajo).|  
|**last_run_date**|**Inter**|Fecha de inicio de la última ejecución del trabajo.|  
|**last_run_time**|**Inter**|Hora de inicio de la última ejecución del trabajo.|  
|**last_run_outcome**|**Inter**|Resultado del trabajo la última vez que se ejecutó:<br /><br /> **0** = error<br /><br /> **1** = correcto<br /><br /> **3** = cancelado<br /><br /> **5** = desconocido|  
|**next_run_date**|**Inter**|Fecha de la próxima ejecución programada del trabajo.|  
|**next_run_time**|**Inter**|Hora de la próxima ejecución programada del trabajo.|  
|**next_run_schedule_id**|**Inter**|Número de identificación de la próxima ejecución programada.|  
|**current_execution_status**|**Inter**|Estado de ejecución actual:<br /><br /> **1** = en ejecución<br /><br /> **2** = esperando subproceso<br /><br /> **3** = entre reintentos<br /><br /> **4** = inactivo<br /><br /> **5** = suspendido<br /><br /> **6** = obsoleto<br /><br /> **7** = PerformingCompletionActions|  
|**current_execution_step**|**sysname**|Paso actual de ejecución del trabajo.|  
|**current_retry_attempt**|**Inter**|Si el trabajo se está ejecutando y el paso se ha intentado más de una vez, es el número actual de reintentos.|  
|**has_step**|**Inter**|Número de pasos que tiene el trabajo.|  
|**has_schedule**|**Inter**|Número de programaciones que tiene el trabajo.|  
|**has_target**|**Inter**|Número de servidores de destino que tiene el trabajo.|  
|**automáticamente**|**Inter**|Tipo del trabajo.<br /><br /> 1 = Trabajo local.<br /><br /> **2** = trabajo multiservidor.<br /><br /> **0** = el trabajo no tiene ningún servidor de destino.|  
...
|Nombre de la columna|Tipo de datos|Descripción|  
|-----------------|---------------|-----------------|  
|**step_id**|**Inter**|Identificador único (de este trabajo) para el paso.|  
...
|**marcas**|**nvarchar(4000)**|**Máscara de máscara** de valores que controlan el comportamiento del paso.|  
|**cmdexec_success_code**|**Inter**|En el caso de un paso **CmdExec** , es el código de salida del proceso de un comando correcto.|  
...
|**on_success_step_id**|**Inter**|Si **on_success_action** es **4**, indica el siguiente paso que se debe ejecutar.|  
...
|**on_fail_step_id**|**Inter**|Si **on_fail_action** es **4**, indica el siguiente paso que se debe ejecutar.|  
...
|**retry_attempts**|**Inter**|Número máximo de veces que se vuelve a intentar el comando (si no termina correctamente) antes de determinar que el paso ha dado error.|  
|**retry_interval**|**Inter**|Intervalo (en minutos) entre los reintentos.|  
...
|**last_run_outcome**|**Inter**|Resultado del paso la última vez que se ejecutó:<br /><br /> **0** = error<br /><br /> **1** = correcto<br /><br /> **3** = cancelado<br /><br /> **5** = desconocido|  
|**last_run_duration**|**Inter**|Duración (en segundos) del paso la última vez que se ejecutó.|  
|**last_run_retries**|**Inter**|Número de veces que se ha intentado el comando desde que se ejecutó el paso por última vez.|  
|**last_run_date**|**Inter**|Fecha en que se inició la ejecución del paso por última vez.|  
|**last_run_time**|**Inter**|Hora en que se inició la ejecución del paso por última vez.|  
|**proxy_id**|**Inter**|Proxy del paso de trabajo.|  
  
 Éste es el conjunto de resultados de las programaciones del trabajo.  
  
|Nombre de la columna|Tipo de datos|Descripción|  
|-----------------|---------------|-----------------|  
|**schedule_id**|**Inter**|Identificador de la programación (único para todos los trabajos).|  
|**schedule_name**|**sysname**|Nombre de la programación (único solo para este trabajo).|  
|**activó**|**Inter**|Indica si la programación está activa (**1**) o no (**0**).|  
|**freq_type**|**Inter**|Valor que indica cuándo se va a ejecutar el trabajo:<br /><br /> **1** = una vez<br /><br /> **4** = diariamente<br /><br /> **8** = semanalmente<br /><br /> **16** = mensualmente<br /><br /> **32** = mensualmente, con respecto al **freq_interval**<br /><br /> **64** = ejecutar cuando se inicia el servicio **SQLServerAgent** .|  
|**freq_interval**|**Inter**|Días en que se ejecuta el trabajo. El valor depende del valor de **freq_type**. Para obtener más información, vea [sp_add_schedule &#40;Transact-SQL&#41;](../../relational-databases/system-stored-procedures/sp-add-schedule-transact-sql.md)|  
|**freq_subday_type**|**Inter**|Unidades para **freq_subday_interval**. Para obtener más información, vea [sp_add_schedule &#40;Transact-SQL&#41;](../../relational-databases/system-stored-procedures/sp-add-schedule-transact-sql.md)|  
|**freq_subday_interval**|**Inter**|Número de períodos de **freq_subday_type** entre cada ejecución del trabajo. Para obtener más información, vea [sp_add_schedule &#40;Transact-SQL&#41;](../../relational-databases/system-stored-procedures/sp-add-schedule-transact-sql.md)|  
|**freq_relative_interval**|**Inter**|Repetición del trabajo programado del **freq_interval** cada mes. Para obtener más información, vea [sp_add_schedule &#40;Transact-SQL&#41;](../../relational-databases/system-stored-procedures/sp-add-schedule-transact-sql.md)|  
|**freq_recurrence_factor**|**Inter**|Número de meses entre las ejecuciones programadas del trabajo.|  
|**active_start_date**|**Inter**|Fecha en que empieza la ejecución del trabajo.|  
|**active_end_date**|**Inter**|Fecha en que termina la ejecución del trabajo.|  
|**active_start_time**|**Inter**|Hora de inicio de la ejecución del trabajo en **active_start_date.**|  
|**active_end_time**|**Inter**|Tiempo de finalización de la ejecución del trabajo en **active_end_date**.|  
...
|**next_run_date**|**Inter**|Fecha en que la programación hará que se vuelva a ejecutar el trabajo.|  
|**next_run_time**|**Inter**|Hora a la que la programación hará que se vuelva a ejecutar el trabajo.|  
|**schedule_uid**|**uniqueidentifier**|Identificador de la programación.|  
|**job_count**|**Inter**|Devuelve el número de trabajos que hacen referencia a esta programación.|  
  
 Éste es el conjunto de resultados de los servidores de destino del trabajo.  
  
|Nombre de la columna|Tipo de datos|Descripción|  
|-----------------|---------------|-----------------|  
|**server_id**|**Inter**|Identificador del servidor de destino.|  
...
|**last_run_date**|**Inter**|Fecha del inicio de la última ejecución del trabajo en este servidor de destino.|  
|**last_run_time**|**Inter**|Hora del inicio de la última ejecución del trabajo en este servidor de destino.|  
|**last_run_duration**|**Inter**|Duración de la última ejecución del trabajo en este servidor de destino.|  
