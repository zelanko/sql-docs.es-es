...
`[ @operator_name = ] 'operator_name'` El nombre del operador. *nombre_operador* es **sysname**. Si *nombre_operador* no es se especifica, se devuelve información sobre todos los operadores.  
  
`[ @operator_id = ] operator_id` El número de identificación del operador para el que se solicita información. *operator_id*es **int**, su valor predeterminado es null.  
