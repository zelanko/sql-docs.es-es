...
`[ @OptionValue = ] 'value'` Especifica si el *option_name* configuración está habilitado (TRUE, ON, yes o 1) o deshabilitado (FALSE, OFF, no o 0). *valor* es **varchar (12)** , no tiene ningún valor predeterminado.  
...
## <a name="remarks"></a>Comentarios  
 ...
 Para mostrar la fila actual y la página de propiedades de bloqueo, use [INDEXPROPERTY](../../t-sql/functions/indexproperty-transact-sql.md) o [sys.indexes](../../relational-databases/system-catalog-views/sys-indexes-transact-sql.md) vista de catálogo.  
  
-   Fila, página y bloqueos de tabla se permiten al tener acceso al índice cuando **AllowRowLocks** = TRUE o **DisAllowRowLocks** = FALSE, y **AllowPageLocks** = TRUE o  **DisAllowPageLocks** = FALSE. [!INCLUDE[ssDE](../../includes/ssde-md.md)] elige el bloqueo apropiado y puede cambiar de escala el bloqueo: de un bloqueo de fila o página a un bloqueo de tabla.  
...
-   Cuando **AllowRowLocks** o **DisAllowRowLocks** está establecido en TRUE o FALSE, la configuración se aplica al montón y cualquier índice no clúster asociado.  
...
-   Cuando **AllowPageLocks** opción está establecida en FALSE o **DisAllowPageLocks** está establecida en TRUE, la configuración se aplica completamente a los índices no agrupados. Es decir, no se permite ningún bloqueo de página en los índices no clúster. En el montón, no se permiten únicamente los bloqueos compartidos (S), de actualización (U) y exclusivos (X) de la página. El [!INCLUDE[ssDE](../../includes/ssde-md.md)] aún puede adquirir un bloqueo de página de intención (IS, IU o IX) por motivos internos.  
...
### <a name="a-setting-an-option-on-a-specific-index"></a>A. Definir una opción en un índice específico  
 El siguiente ejemplo no permite bloqueos de página en el `IX_Customer_TerritoryID` de índice en el `Customer` tabla.  
...
### <a name="c-setting-an-option-on-a-table-with-no-clustered-index"></a>C. Definir una opción en una tabla sin clúster  
 El siguiente ejemplo no permite bloqueos de página en una tabla sin clúster (un montón). El `sys.indexes` se consulta la vista de catálogo antes y después el `sp_indexoption` se ejecuta el procedimiento para mostrar los resultados de la instrucción.  
