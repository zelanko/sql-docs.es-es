...
`[ @spid1 = ] 'session ID1'` es un número de ID. de sesión [!INCLUDE[ssDE](../../includes/ssde-md.md)] de **Sys. DM _ exec_sessions** para el que el usuario desea información de bloqueo. la *sesión ID1* es de **tipo int** y su valor predeterminado es NULL. Ejecute **sp_who** para obtener información del proceso sobre la sesión. Si no se especifica *Session ID1* , se muestra información sobre todos los bloqueos.  
  
`[ @spid2 = ] 'session ID2'` es otro número de ID. de sesión @no__t 1 de **Sys. DM _ exec_sessions** que puede tener un bloqueo al mismo tiempo que *ID1 de sesión* y sobre el que el usuario también desea información. la *sesión ID2* es de **tipo int** y su valor predeterminado es NULL.  
...
|**Recurso**|**nchar(32)**|El valor que identifica el recurso bloqueado. El formato del valor depende del tipo de recurso identificado en la columna de **tipo** :<br /><br /> **Tipo** de Valor **Recurso** de Valor<br /><br /> LIBRA Un identificador con el formato fileid: PageNumber: RID, donde fileid identifica el archivo que contiene la página, PageNumber identifica la página que contiene la fila y RID identifica la fila específica en la página. fileid coincide con la columna **file_id** de la vista de catálogo **Sys. database_files** .<br /><br /> CLAVE Número hexadecimal utilizado internamente por el [!INCLUDE[ssDE](../../includes/ssde-md.md)].<br /><br /> PAG Un número con el formato fileid: PageNumber, donde fileid identifica el archivo que contiene la página y PageNumber identifica la página.<br /><br /> TOTAL Número que identifica la primera página de la extensión. El número tiene un formato idDeArchivo:númeroDePágina.<br /><br /> FICHA No se proporcionó información porque la tabla ya está identificada en la columna **ObjId** .<br /><br /> BASES No se proporcionó información porque la base de datos ya está identificada en la columna **DBID** .<br /><br /> FIL El identificador del archivo, que coincide con la columna **file_id** de la vista de catálogo **Sys. database_files** .<br /><br /> APLICACIONES Un identificador único para el recurso de aplicación que se está bloqueando. En el formato DbPrincipleId: \<first de dos a 16 caracteres de la cadena de recursos > @no__t 1hashed >.<br /><br /> MD: varía por tipo de recurso. Para obtener más información, vea la descripción de la columna **resource_description** en [Sys. DM &#40;_ _tran_locks Transact&#41;-SQL](../../relational-databases/system-dynamic-management-views/sys-dm-tran-locks-transact-sql.md).<br /><br /> HBT: No se proporcionó información. Use la vista de administración dinámica **Sys. DM _ _tran_locks** en su lugar.<br /><br /> ESTÉ No se proporcionó información. Use la vista de administración dinámica **Sys. DM _ _tran_locks** en su lugar.|  
...
|**Estado**|**nvarchar(5)**|Estado de solicitud de bloqueo:<br /><br /> CNVRT: El bloqueo se está convirtiendo desde otro modo, pero la conversión está bloqueada por otro proceso que mantiene un bloqueo con un modo en conflicto.<br /><br /> GARANTIZAR Se obtuvo el bloqueo.<br /><br /> CURRIR El bloqueo está bloqueado por otro proceso que mantiene un bloqueo con un modo en conflicto.|  
