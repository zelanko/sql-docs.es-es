...
Cuando se modifican las propiedades de cifrado `sp_refresh_parameter_encryption` de una tabla, se debe ejecutar para cualquier módulo que haga referencia directa o indirectamente a la tabla. Se puede llamar a este procedimiento almacenado en los módulos en cualquier orden, sin necesidad de que el usuario actualice primero el módulo interno antes de pasar a sus llamadores.
...
Requiere `ALTER` el permiso en el módulo `REFERENCES` y el permiso en los tipos definidos por el usuario CLR y las colecciones de esquemas XML a los que hace referencia el objeto.   

Cuando el módulo especificado es un desencadenador DDL de nivel de base `ALTER ANY DATABASE DDL TRIGGER` de datos, requiere el permiso en la base de datos actual.    

Cuando el módulo especificado es un desencadenador DDL de nivel de servidor `CONTROL SERVER` , requiere el permiso.

En el caso de los módulos definidos `EXECUTE AS` con la `IMPERSONATE` cláusula, se requiere el permiso en la entidad de seguridad especificada. Por lo general, la actualización de un objeto `EXECUTE AS` no cambia su entidad de seguridad, a `EXECUTE AS USER` menos que el módulo se haya definido con y el nombre de usuario de la entidad de seguridad se resuelva ahora como un usuario diferente del que tenía en el momento en que se creó el módulo.
 
## <a name="examples"></a>Ejemplos

En el ejemplo siguiente se crea una tabla y un procedimiento que hace referencia a la tabla, se configura Always Encrypted y, a continuación, se muestra `sp_refresh_parameter_encryption` cómo modificar la tabla y cómo ejecutar el procedimiento.  
...
Por último, se reemplaza la columna SSN con la columna cifrada y, `sp_refresh_parameter_encryption` después, se ejecuta el procedimiento para actualizar los componentes de Always encrypted.
```sql
