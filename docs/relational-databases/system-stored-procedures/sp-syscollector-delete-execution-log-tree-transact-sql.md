...
`[ @log_id = ] log_id` Es el identificador único para la colección establecer registro. *log_id* es **int**.  
  
`[ @from_collection_set = ] from_collection_set` Es el identificador del conjunto de recopilación. *from_collection_set* es **bits = 1**.  
