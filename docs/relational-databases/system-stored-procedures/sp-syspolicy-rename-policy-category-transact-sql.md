...
`[ @name = ] 'name'` Es el nombre de la categoría de directiva que desea cambiar el nombre. *nombre* es **sysname**y debe especificarse si *policy_category_id* es NULL.  
  
`[ @policy_category_id = ] policy_category_id` Es el identificador de la categoría de directiva que desea cambiar el nombre. *policy_category_id* es **int**y debe especificarse si *nombre* es NULL.  
  
`[ @new_name = ] 'new_name'` Es el nuevo nombre para la categoría de directiva. *new_name* es **sysname**y es necesario. No puede ser NULL ni una cadena vacía.  
