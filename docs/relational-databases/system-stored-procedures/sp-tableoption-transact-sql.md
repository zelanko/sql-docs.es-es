...
ms.locfileid: "33263576"
---
# <a name="sptableoption-transact-sql"></a>sp_tableoption (Transact-SQL)
[!INCLUDE[tsql-appliesto-ss2008-asdb-xxxx-xxx-md](../../includes/tsql-appliesto-ss2008-asdb-xxxx-xxx-md.md)]

  Establece los valores de las opciones de las tablas definidas por el usuario. sp_tableoption puede usarse para controlar el comportamiento de las filas de las tablas con **varchar (max)**, **nvarchar (max)**, **varbinary (max)**, **xml**, **texto**, **ntext**, **imagen**, o las columnas de tipo definido por el usuario grandes.  
  
> [!IMPORTANT]  
>  La característica text in row se quitará en una versión futura de [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]. Para almacenar datos de valores grandes, se recomienda que utilice la **varchar (max)**, **nvarchar (max)** y **varbinary (max)** tipos de datos.  
...
## <a name="remarks"></a>Comentarios  
 ...
 Cuando se habilita la opción text in row, el @OptionValue parámetro permite a los usuarios especificar el tamaño máximo que debe almacenarse en una fila de un BLOB. El valor predeterminado es 256 bytes, pero los valores pueden oscilar entre 24 y 7.000 bytes.  
  
 **texto**, **ntext**, o **imagen** cadenas se almacenan en la fila de datos si se cumplen las condiciones siguientes:  
...
-   Hay suficiente espacio disponible en la fila de datos.  
 ...
 Cuando se almacenan cadenas BLOB en la fila de datos, leer y escribir el **texto**, **ntext**, o **imagen** cadenas pueden ser tan rápidas como la lectura o escritura de cadenas de caracteres y binarias. [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] no necesita obtener acceso a páginas independientes para leer o escribir la cadena BLOB.  
...
### <a name="a-storing-xml-data-out-of-the-row"></a>A. Almacenar datos xml fuera de la fila  
 En el ejemplo siguiente se especifica que la **xml** datos en el `HumanResources.JobCandidate` tabla se almacena de manera no consecutiva.  
...
### <a name="b-enabling-vardecimal-storage-format-on-a-table"></a>B. Habilitar el formato de almacenamiento vardecimal en una tabla  
 En el ejemplo siguiente se modifica la `Production.WorkOrderRouting` tabla para almacenar el `decimal` tipo de datos en el `vardecimal` el formato de almacenamiento.  
