...
|**status**|**tinyint**|Indica el estado del artículo, que puede ser uno de los siguientes:<br /><br /> **1** = Unsynced - el script de procesamiento inicial para publicar la tabla se ejecutará la próxima vez que se ejecuta el agente de instantáneas.<br /><br /> **2** = active - se ha ejecutado el script de procesamiento inicial para publicar la tabla.|  
...
|**expand_proc**|**sysname**|El nombre del procedimiento que vuelve a evaluar para todas las filas secundarias de una fila principal recién insertada y para las filas principales que han experimentado un cambio en la partición o han sido eliminadas de identificadores de partición.|  
...
|**logical_record_deleted_view_rule**|**sysname**|Similar a **logical_record_view**, salvo que muestre las filas secundarias en la tabla "eliminada" en actualización y eliminación desencadenadores.|  
