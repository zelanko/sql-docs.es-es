...
|Purchasing|Compras de artículos bursátiles de proveedores y detalles acerca de los proveedores.|  
...
|Countrie|Los Estados o provincias forman parte de los países. En esta tabla se detallan los detalles, incluidos los datos espaciales que describen los límites de cada país.|
...
- Para estas tablas se usa una sola secuencia (TransactionID): CustomerTransactions, SupplierTransactions y StockItemTransactions. Esto muestra cómo un conjunto de tablas puede tener una sola secuencia.
...
|Configuration_ApplyFullTextIndexing|Aplica los índices de texto `Application.People`completo `Sales.Customers`a `Purchasing.Suppliers`,, `Warehouse.StockItems`y. Reemplaza `Website.SearchForPeople`, `Website.SearchForSuppliers`, ,`Website.SearchForCustomers` ,por`Website.SearchForStockItemsByTags` los procedimientos de reemplazo que usan la indización de texto completo `Website.SearchForStockItems`.|
|Configuration_ApplyPartitioning|Aplica la creación de particiones `Purchasing.SupplierTransactions`de tabla a `Sales.CustomerTransactions` y, y reorganiza los índices para que se adapten.|
