---
title: Archivos de aplicación de SQL Server | Microsoft Docs
ms.custom: ''
ms.date: 07/27/2017
ms.prod: sql
...
ms.locfileid: "39228676"
---
# <a name="sql-applies-to-and-includes"></a>"Applies to" e "Includes" de SQL
Las referencias de la documentación se pueden modificar fácilmente sin cambiar el texto real de artículos individuales mediante "Includes" en Markdown. Hay tres tipos de archivos de inclusión en el mundo del contenido de SQL: versiones de SQL, applies-to y texto referencial. Las versiones de SLQ se usan para indicar la versión de SQL que se está tratando (por ejemplo, SQL 2016 frente a 2017) y los archivos applies-to indican la versión de SQL Server a la que se aplica el documento (SQL Server en Linux frente a Azure SQL Database). El texto referencial son archivos de inclusión que no corresponden a las otras dos categorías, como el archivo de inclusión Obtener ayuda (una lista de vínculos que los clientes pueden usar para obtener ayuda con SQL). 

Este artículo está pensado para usarse como punto de referencia para solo los dos primeros tipos de archivos de inclusión. 

## <a name="sql-server-version-includes"></a>La versión de SQL Server incluye

Los escritores de contenido de SQL suelen tener que incluir el nombre del producto y la versión de SQL Server. Así, si algo cambia en el nombre, el archivo de inclusión se actualiza en lugar de actualizar manualmente el valor en todos los artículos. Estos archivos de inclusión se utilizan como marcadores de posición para los nombres de productos, pero no se han usado de forma coherente en toda la documentación de SQL. SQL Server vNext hace referencia a una futura versión de SQL que aún no tiene número de versión y es la excepción a esto.  
...
## <a name="sql-server-non-version-specific-applies-to"></a>Archivo applies-to específico sin versión de SQL Server
...
## <a name="sql-server-version-specific-applies-to"></a>Archivo applies-to específico de la versión de SQL Server
Estos archivos applies-to especifican las versiones de SQL a las que se aplica la documentación. 
