...
/>  
```  
**Nota:** La aplicación de consola de SSMA para Access admite la compatibilidad con versiones anteriores. Podrá abrir proyectos creados por la versión anterior de SSMA.  
...
3.  `on-error:`Especifica si se deben especificar errores de actualización como advertencias o errores. Opciones disponibles para en caso de error:  
...
4.  `report-errors-to:`Especifica la ubicación del informe de errores para la operación de actualización (atributo opcional) si solo se especifica la ruta de acceso de la carpeta, se crea File by Name **SourceDBRefreshReport. XML** .  
