...
Considere el ejemplo siguiente, en el que hemos creado un `MATH_PKG` paquete que contiene una `GET_SUM`función.
...
A continuación se muestra el código de reescritura de un ejemplo anterior de Oracle. a continuación, se `(n1 INT DEFAULT 0)` asigna el valor predeterminado al `GET_SUM` primer parámetro de la función:
