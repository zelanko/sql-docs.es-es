...
En este artículo se explica por qué SQL Server Migration Assistant (SSMA) para Oracle no convierte los bloques PL/ `SELECT` SQL cuando se utiliza `BULK COLLECT INTO` la instrucción `DISTINCT` con la cláusula and.
...
SSMA proporciona una ruta de migración para colecciones de Oracle. Sin embargo, si `SELECT` la instrucción que se usa para rellenar la colección utiliza una `DISTINCT` cláusula, SSMA genera el mensaje de error.

## <a name="example"></a>Ejemplo

Considere el ejemplo siguiente en el `SELECT` que se usa la `DISTINCT` instrucción con `BULK COLLECT INTO` la cláusula y la cláusula:
...
Para convertir el bloque PL/SQL anterior, necesitamos reemplazar `DISTINCT` la cláusula con `UNIQUE` la cláusula (un sinónimo `DISTINCT`de) como se muestra a continuación:
