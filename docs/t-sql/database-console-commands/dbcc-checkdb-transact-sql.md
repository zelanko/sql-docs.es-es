...
> **Nota:** DBCC CHECKDB se admite en bases de datos que contienen tablas optimizadas en memoria, pero solo se produce la validación en las tablas basadas en disco. Sin embargo, como parte de la copia de seguridad y la recuperación de bases de datos, la validación de CHECKSUM se realiza para los archivos de grupos de archivos con optimización para memoria.    
>     
>  Puesto que las opciones de reparación de DBCC no están disponibles para las tablas con optimización para memoria, debe hacer periódicamente copia de seguridad de las bases de datos y probar dichas copias. Si se producen problemas de integridad de datos en una tabla con optimización para memoria, debe restaurar desde la última copia de seguridad válida conocida.    
...
> [!WARNING]
> - La opción REPAIR_ALLOW_DATA_LOSS es una característica compatible pero no siempre es la mejor opción para poner una base de datos a un estado físicamente coherente. 
> -Si se realiza correctamente, la opción REPAIR_ALLOW_DATA_LOSS puede ocasionar una pérdida de datos. De hecho, puede producir la pérdida de más datos que si un usuario tuviera que restaurar la base de datos desde la última copia de seguridad en buen estado. 
>
...
> En [!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)], el número máximo de mensajes de error devueltos es 1000. Cuando se especifica ALL_ERRORMSGS, le recomendamos que ejecute el comando DBCC mediante el uso de la [utilidad sqlcmd](../../tools/sqlcmd-utility.md) o mediante la programación de un [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] trabajo del agente para ejecutar el comando y dirigir la salida a un archivo. Cualquiera de estos métodos garantizará que el comando se ejecute una vez e informará de todos los mensajes de error.    
 ...
 Para obtener más información, vea "Realizar comprobaciones de coherencia lógica en índices" en la sección "Comentarios", más adelante en este tema.  
...
> Si se especifica PHYSICAL_ONLY, no se realizan comprobaciones de integridad de columna.
 ...
 MAXDOP  
 **Se aplica a**: [!INCLUDE[ssSQL14](../../includes/sssql14-md.md)] SP2 a través de [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)].  
...
En las versiones de [!INCLUDE[ssVersion2005](../../includes/ssversion2005-md.md)] anteriores al Service Pack 2, al ejecutar DBCC CHECKDB se borra la caché del plan de la instancia de [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]. Al borrar la caché del plan, se provoca una recompilación de todos los planes de ejecución posteriores y puede ocasionar una disminución repentina y temporal del rendimiento de las consultas. En el Service Pack 2 y posteriores, al ejecutar DBCC CHECKDB no se borra la caché del plan.
