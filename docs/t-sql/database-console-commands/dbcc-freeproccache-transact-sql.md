...
En [!INCLUDE[ssSDW](../../includes/sssdw-md.md)] o [!INCLUDE[ssPDW](../../includes/sspdw-md.md)], borrar la caché del plan de consulta puede ocasionar una disminución temporal en el rendimiento de las consultas cuando se vuelven a compilar las consultas. 
...
En el ejemplo siguiente se especifica el identificador de plan de consulta para borrar el plan de consulta de la caché del plan. Para asegurarse de que la consulta del ejemplo está en la caché del plan, la consulta se ejecuta primero. El `sys.dm`_`exec` \_ `cached_plans` y `sys.dm` \_ `exec` \_ `sql` \_ `text` se consultan las vistas de administración dinámica para devolver el identificador del plan de la consulta. 
