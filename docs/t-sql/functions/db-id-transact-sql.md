...
En el ejemplo siguiente se utiliza `DB`_`ID` para devolver el identificador de base de datos de la [!INCLUDE[ssSampleDBobject](../../includes/sssampledbobject-md.md)] base de datos en la función del sistema `sys.dm_db` \_ `index` \_ `operational` \_ `stats`. La función toma un Id. de base de datos como primer parámetro.
