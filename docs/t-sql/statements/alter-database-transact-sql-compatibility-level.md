...
>  
>  Para obtener más información acerca de cómo evaluar las diferencias de rendimiento de las consultas más importantes, entre los dos niveles de compatibilidad de [!INCLUDE[ssSDS](../../includes/sssds-md.md)], consulte [mejora el rendimiento de la consulta con 130 de nivel de compatibilidad de base de datos de SQL Azure](http://azure.microsoft.com/documentation/articles/sql-database-compatibility-level-query-performance-130/).
...
## <a name="using-compatibility-level-for-backward-compatibility"></a>Usar el nivel de compatibilidad para la compatibilidad con versiones anteriores  
 ...
 Si existen [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] aplicaciones se ven afectadas por las diferencias de comportamiento en la versión de [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)], convertir la aplicación para trabajar sin problemas con el nuevo modo de compatibilidad. A continuación, use `ALTER DATABASE` para cambiar el nivel de compatibilidad a 130. La nueva configuración de compatibilidad de una base de datos surte efecto cuando un `USE <database>` emitido o un nuevo inicio de sesión se procesa con esa base de datos como la base de datos de forma predeterminada.  
 
> [!IMPORTANT]
> No incluye la funcionalidad incluida en un determinado [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] versión no está protegida por el nivel de compatibilidad.
> 
> Por ejemplo, el `FASTFIRSTROW` sugerencia se interrumpió en [!INCLUDE[ssSQL11](../../includes/sssql11-md.md)] y se reemplazaron por la `OPTION (FAST n )` sugerencia. Establecer el nivel de compatibilidad de base de datos a 110 no se restaurará la sugerencia no incluida.
> Para obtener más información sobre la funcionalidad no incluida, consulte [funcionalidad del motor de base de datos no incluida en SQL Server 2016](../../database-engine/discontinued-database-engine-functionality-in-sql-server-2016.md), [funcionalidad del motor de base de datos no incluida en SQL Server 2014](http://msdn.microsoft.com/library/ms144262(v=sql.120)), [No incluye la funcionalidad del motor de base de datos de SQL Server 2012](http://msdn.microsoft.com/library/ms144262(v=sql.110)), y [no incluye la funcionalidad del motor de base de datos en SQL Server 2008](http://msdn.microsoft.com/library/ms144262(v=sql.100)).

> [!IMPORTANT]
> Cambios importantes introdujeron en un determinado [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] versión **puede** no estará protegida por nivel de compatibilidad. [!INCLUDE[tsql](../../includes/tsql-md.md)]comportamiento normalmente está protegido por el nivel de compatibilidad. Sin embargo, son objetos del sistema eliminados o modificados **no** protegidos por nivel de compatibilidad.
>
> Un ejemplo de un cambio importante **protegido** compatibilidad nivel es una conversión implícita de fecha y hora en tipos de datos datetime2. En el nivel de compatibilidad de base de datos 130, estos Mostrar precisión mejorada por teniendo en cuenta las fracciones de milisegundos, lo que resulta en diferentes valores convertidos. Para restaurar el comportamiento de conversión anterior, establezca el nivel de compatibilidad de base de datos en 120 o inferior.
>
> Ejemplos de cambios **no protegido** por compatibilidad con el nivel son:
> -  Nombres de columna modificada en objetos del sistema. En [!INCLUDE[ssSQL11](../../includes/sssql11-md.md)] la columna *single_pages_kb* en sys.dm_os_sys_info se ha cambiado a *pages_kb*. Independientemente del nivel de compatibilidad, la consulta `SELECT single_pages_kb FROM sys.dm_os_sys_info` generará el error 207 (nombre de columna no válido).
> -  Objetos del sistema quitado. En [!INCLUDE[ssSQL11](../../includes/sssql11-md.md)] el `sp_dboption` se quitó. Independientemente del nivel de compatibilidad, la instrucción `EXEC sp_dboption 'AdventureWorks2016CTP3', 'autoshrink', 'FALSE';` se producirá el error 2812 (no se encontró el procedimiento almacenado 'sp_dboption').
>
> Para obtener más información sobre cambios importantes, consulte [cambios recientes en las características del motor de base de datos en SQL Server 2017](../../database-engine/breaking-changes-to-database-engine-features-in-sql-server-2017.md), [cambios recientes en las características del motor de base de datos en SQL Server 2016](../../database-engine/breaking-changes-to-database-engine-features-in-sql-server-2016.md), [ Cambios de última hora a la base de datos del motor de características de SQL Server 2014](http://msdn.microsoft.com/library/ms143179(v=sql.120)), [cambios substanciales en las características del motor de base de datos de SQL Server 2012](http://msdn.microsoft.com/library/ms143179(v=sql.110)), y [cambios substanciales en las características del motor de base de datos en SQL Server 2008](http://msdn.microsoft.com/library/ms143179(v=sql.100)).
...
|El `sp_execute_external_script` UDX [operador] solo puede ejecutarse en modo de fila.|El `sp_execute_external_script` UDX [operador] es apto para la ejecución en modo por lotes.|  
|Las funciones con valores de tabla de múltiples instrucciones (TVF) no tiene la ejecución intercalada |Ejecución intercalada de TVF de múltiples instrucciones mejorar la calidad del plan. |

Correcciones que se encontraban bajo la marca de seguimiento 4199 en versiones anteriores de SQL Server anterior a SQL Server 2017 están habilitadas de forma predeterminada. Con el modo de compatibilidad 140. Marca de seguimiento 4199 todavía se puede aplicar de nuevo correcciones del optimizador de consultas que se publicaron con posterioridad 2017 de SQL Server. Para obtener información acerca de la marca de seguimiento 4199, consulte [marca de seguimiento 4199](../../t-sql/database-console-commands/dbcc-traceon-trace-flags-transact-sql.md#4199).  
...
|Modo por lotes frente a cambios de modo de fila con los índices de almacén de columnas<br /><br /> Ordena en una tabla con índice de almacén de columnas está en modo de fila<br /><br /> Agregados de la función de ventana operan en el modo de fila como `LAG` o`LEAD`<br /><br /> Consultas en tablas de almacén de columnas con varias cláusulas distinct operadas en modo de fila<br /><br /> Consultas que se ejecuten con MAXDOP 1 o un plan serie que se ejecuta en modo de fila | Modo por lotes frente a cambios de modo de fila con los índices de almacén de columnas<br /><br /> Ordena en una tabla con un índice de almacén de columnas se encuentran ahora en modo por lotes<br /><br /> Agregados de ventana funcionan ahora en modo por lotes como `LAG` o`LEAD`<br /><br /> Las consultas en las tablas de almacén de columnas con varias cláusulas distinct funcionan en modo por lotes<br /><br /> Las consultas que se ejecuten con Maxdop1 o un plan en serie se ejecuten en modo por lotes|  
...
||[!INCLUDE[ssSQL15](../../includes/sssql15-md.md)] incluye mejoras en algunas conversiones de tipos de datos y algunas operaciones (principalmente poco frecuente). Para obtener más información, consulte [mejoras de SQL Server 2016 en el control de algunos tipos de datos y operaciones raro](https://support.microsoft.com/help/4010261/sql-server-2016-improvements-in-handling-some-data-types-and-uncommon).|
  
Marca de correcciones que se encontraban bajo seguimiento 4199 en versiones anteriores de [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] anteriores a [!INCLUDE[ssSQL15](../../includes/sssql15-md.md)] están habilitados de forma predeterminada. Con el modo de compatibilidad 130. Marca de seguimiento 4199 seguirán aplicable para nuevas correcciones del optimizador de consultas que se publicaron con posterioridad [!INCLUDE[ssSQL15](../../includes/sssql15-md.md)]. Para utilizar el optimizador de consultas anterior en [!INCLUDE[ssSDS](../../includes/sssds-md.md)] debe seleccionar el nivel de compatibilidad 110. Para obtener información acerca de la marca de seguimiento 4199, consulte [marca de seguimiento 4199](../../t-sql/database-console-commands/dbcc-traceon-trace-flags-transact-sql.md#4199).  
...
|Las referencias recursivas en el lado derecho de un `EXCEPT` cláusula crear un bucle infinito. Ejemplo C en la sección ejemplos siguiente demuestra este comportamiento.|Referencias recursivas en un `EXCEPT` cláusula genera un error cumple el estándar ANSI SQL.|  
|Expresión de tabla común recursiva (CTE) permite nombres de columna duplicados.|CTE recursiva no admite nombres de columna duplicados.|  
...
|La cláusula OUTPUT INTO tabla omite la `IDENTITY_INSERT SETTING = OFF` y permite insertar valores explícitos.|No se puede insertar valores explícitos para una columna de identidad en una tabla cuando `IDENTITY_INSERT` se establece en OFF.|  
|Cuando la contención de base de datos está establecida en parcial, validar la `$action` campo el `OUTPUT` cláusula de una `MERGE` instrucción puede devolver un error de intercalación.|La intercalación de los valores devueltos por la `$action` cláusula de una `MERGE` instrucción es la intercalación de base de datos en lugar de la intercalación del servidor y no se devuelve un error de conflicto de intercalación.|  
...
|`PIVOT`se permite en una consulta de expresión (CTE) de tabla común recursiva. Sin embargo, la consulta devuelve resultados incorrectos cuando hay varias filas por agrupación.|`PIVOT`no se permite en una consulta de expresión (CTE) de tabla común recursiva. Se devuelve un error.|  
...
|El estilo predeterminado de `CAST` y `CONVERT` operaciones en **tiempo** y **datetime2** tipos de datos es 121, a menos que se utilizan cualquiera de estos tipos en una expresión de columna calculada. Para las columnas calculadas, el estilo predeterminado es 0. Este comportamiento afecta a las columnas calculadas cuando se crean, cuando se utilizan en las consultas que implican parametrización automática o cuando se usan en definiciones de restricciones.<br /><br /> Ejemplo D en la siguiente sección de ejemplos muestra la diferencia entre los estilos 0 y 121. No se muestra el comportamiento descrito anteriormente. Para obtener más información sobre los estilos de fecha y hora, vea [CAST y CONVERT &#40; Transact-SQL &#41; ](../../t-sql/functions/cast-and-convert-transact-sql.md).|En el nivel de compatibilidad 110, el estilo predeterminado de `CAST` y `CONVERT` operaciones en **tiempo** y **datetime2** tipos de datos es siempre 121. Si su consulta se basa en el comportamiento anterior, use un nivel de compatibilidad menor de 110, o especifique explícitamente el estilo 0 en la consulta correspondiente.<br /><br /> Actualizar la base de datos al nivel de compatibilidad 110 no cambiará los datos de usuario que se hayan almacenado en disco. Debe corregir manualmente estos datos según convenga. Por ejemplo, si usó `SELECT INTO` para crear una tabla de un origen que contiene una expresión de columna calculada se ha descrito anteriormente, los datos (utilizando el estilo 0) se almacenaría en lugar de la propia definición de columna calculada. Debería actualizar manualmente estos datos para que coincidieran con el estilo 121.|  
...
|`SOUNDEX`la función implementa las reglas siguientes:<br /><br /> (1)-mayúsculas H o W mayúsculas se omiten al separar dos consonantes que tienen el mismo número en el `SOUNDEX` código.<br /><br /> 2) si los 2 primeros caracteres de *character_expression* tienen el mismo número el `SOUNDEX` código, ambos caracteres se incluyen. Asimismo, si un conjunto de consonantes side-by-side tiene el mismo número el `SOUNDEX` código, todas ellas se excluyen excepto el primero.|`SOUNDEX`la función implementa las reglas siguientes:<br /><br /> (1) si H en mayúsculas o mayúsculas W separa dos consonantes que tienen el mismo número en el `SOUNDEX` se omite el código, codifica la consonante situada a la derecha<br /><br /> (2) si un conjunto de consonantes side-by-side tiene el mismo número el `SOUNDEX` código, todas ellas se excluyen excepto el primero.<br /><br /> <br /><br /> Las reglas adicionales pueden hacer que los valores calculados por la `SOUNDEX` función sea diferente de los valores computados bajo niveles de compatibilidad inferiores. Después de actualizar al nivel de compatibilidad 110, puede que necesite volver a generar los índices, los montones o las restricciones CHECK que usen la `SOUNDEX` función. Para obtener más información, vea [SOUNDEX &#40; Transact-SQL &#41;](../../t-sql/functions/soundex-transact-sql.md)|  
...
|El `FOR BROWSE` cláusula está permitida (y omiten) en `INSERT` y `SELECT INTO` las instrucciones.|El `FOR BROWSE` cláusula no se permite en `INSERT` y `SELECT INTO` las instrucciones.|Media|  
|Predicados de texto completo se permiten en la `OUTPUT` cláusula.|Predicados de texto completo no están permitidos en el `OUTPUT` cláusula.|Baja|  
|`CREATE FULLTEXT STOPLIST`, `ALTER FULLTEXT STOPLIST`, y `DROP FULLTEXT STOPLIST` no son compatibles. La lista de palabras irrelevantes del sistema se asocia automáticamente a nuevos índices de texto completo.|`CREATE FULLTEXT STOPLIST`, `ALTER FULLTEXT STOPLIST`, y `DROP FULLTEXT STOPLIST` son compatibles.|Baja|  
|`MERGE`no se aplica como una palabra clave reservada.|MERGE es una palabra clave totalmente reservada. El `MERGE` instrucción es compatible con los niveles de compatibilidad 90 y 100.|Baja|  
...
|A menos que `NOINDEX` se especifica, `DBCC CHECKDB` o `DBCC CHECKTABLE` realiza comprobaciones de coherencia física y lógica en una sola tabla o vista indizada y en todas sus no clúster y los índices XML. Los índices espaciales no se admiten.|A menos que `NOINDEX` se especifica, `DBCC CHECKDB` o `DBCC CHECKTABLE` realiza dos comprobaciones de coherencia física y lógica en una sola tabla y en todos sus índices no clúster. Sin embargo, en los índices XML, índices espaciales y vistas indexadas solamente se realizan comprobaciones de coherencia física de forma predeterminada.<br /><br /> Si `WITH EXTENDED_LOGICAL_CHECKS` se especifica, se realizarán comprobaciones lógicas en las vistas indizadas, índices XML e índices espaciales, cuando esté presente. De forma predeterminada, las comprobaciones de coherencia física se realizan antes que las comprobaciones de coherencia lógica. Si `NOINDEX` también se especifica, se realizan solo las comprobaciones lógicas.|Baja|  
|Cuando una cláusula OUTPUT se utiliza con una instrucción del lenguaje de manipulación de datos (DML) y se produce un error en tiempo de ejecución durante la ejecución de la instrucción, toda la transacción se termina y se revierte.|Cuando un `OUTPUT` cláusula se utiliza con una instrucción de lenguaje (DML) de manipulación de datos y se produce un error de tiempo de ejecución durante la ejecución de la instrucción, el comportamiento depende de la `SET XACT_ABORT` configuración. Si `SET XACT_ABORT` es OFF, un error de anulación de instrucción generado por la instrucción DML mediante el `OUTPUT` cláusula terminará la instrucción, pero continúa la ejecución del lote y la transacción no se revierte. Si `SET XACT_ABORT` es ON, todos los errores de tiempo de ejecución generados por la instrucción DML con la cláusula OUTPUT terminarán el lote y se revierte la transacción.|Baja|  
...
|Las propiedades de columna `ROWGUIDCOL` y `IDENTITY` se pueden denominar incorrectamente como una restricción. Por ejemplo, la instrucción `CREATE TABLE T (C1 int CONSTRAINT MyConstraint IDENTITY)` se ejecuta, pero el nombre de restricción no se conserva y no es accesible para el usuario.|Las propiedades de columna `ROWGUIDCOL` y `IDENTITY` no se pueden denominar como una restricción. Se devuelve el error 156.|Baja|  
|Actualizar las columnas utilizando una asignación bidireccional como `UPDATE T1 SET @v = column_name = <expression>` puede producir resultados inesperados porque el valor real de la variable se puede usar en otras cláusulas como el `WHER`E y `ON` cláusula durante la ejecución de la instrucción en lugar de la valor inicial de la instrucción. Esto puede hacer que los significados de los predicados cambien de forma imprevisible según cada fila.<br /><br /> Este comportamiento solo es aplicable cuando el nivel de compatibilidad está establecido en 90.|Al actualizar las columnas utilizando una asignación bidireccional, se generan los resultados previstos porque solo se obtiene acceso al valor inicial de la columna de la instrucción durante la ejecución de la misma.|Baja|  
|Vea el ejemplo E en la siguiente sección de ejemplos.|Vea el ejemplo F en la siguiente sección de ejemplos.|Baja|  
|La función ODBC {fn CONVERT()} utiliza el formato de fecha predeterminado del lenguaje. Para algunos idiomas, el formato predeterminado es adm, lo que puede producir errores de conversión cuando CONVERT() se combina con otras funciones, como `{fn CURDATE()}`, que espera un formato AMD.|La función ODBC `{fn CONVERT()}` usa estilo 121 (un formato independiente del lenguaje de AMD) al convertir a los datos ODBC tipos SQL_TIMESTAMP, SQL_DATE, SQL_TIME, SQLDATE, SQL_TYPE_TIME y SQL_TYPE_TIMESTAMP.|Baja|  
|Los tipos de fecha y hora intrínsecos como DATEPART no requieren que los valores de entrada de cadena sean literales de fecha y hora válidos. Por ejemplo, `SELECT DATEPART (year, '2007/05-30')` se compila correctamente.|Fecha y hora intrínsecos como `DATEPART` requieren valores de entrada de cadena sean literales de fecha y hora válidos. Se devuelve el error 241 cuando se utiliza un literal de fecha y hora no válido.|Baja|  
