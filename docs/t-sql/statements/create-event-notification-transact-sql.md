---
title: "CREAR la notificación de eventos (Transact-SQL) | Documentos de Microsoft"
ms.custom: 
ms.date: 03/14/2017
 ...
 *event_notification_name*  
 Es el nombre de la notificación de eventos. Un nombre de la notificación de eventos debe cumplir las reglas de [identificadores](../../relational-databases/databases/database-identifiers.md) y debe ser único dentro del ámbito en el que se crean: servidor, base de datos, o *object_name*.  
 ...
 *nombre_de_cola*  
 Es el nombre de la cola a la que se aplica la notificación de eventos. *nombre_de_cola* solo se puede especificar si se especifica la cola.  
 ...
 *event_type*  
 Es el nombre del tipo de evento que ejecuta la notificación de eventos. *event_type* puede ser un [!INCLUDE[tsql](../../includes/tsql-md.md)] tipo de evento DDL, un tipo de evento de seguimiento de SQL o un [!INCLUDE[ssSB](../../includes/sssb-md.md)] tipo de evento. Para obtener una lista de los [!INCLUDE[tsql](../../includes/tsql-md.md)] tipos de eventos DDL, consulte [eventos DDL](../../relational-databases/triggers/ddl-events.md). Los tipos de evento de [!INCLUDE[ssSB](../../includes/sssb-md.md)] son QUEUE_ACTIVATION y BROKER_QUEUE_DISABLED. Para más información, consulte [Event Notifications](../../relational-databases/service-broker/event-notifications.md).  
  
 *event_group*  
 Es el nombre de un grupo predefinido de tipos de evento de [!INCLUDE[tsql](../../includes/tsql-md.md)] o de Seguimiento de SQL. Se puede activar una notificación de eventos tras la ejecución de cualquier evento que pertenece a un grupo de eventos. Para obtener una lista de grupos de eventos DDL, la [!INCLUDE[tsql](../../includes/tsql-md.md)] eventos que cubren y el ámbito en el que se pueden definir, vea [grupos de eventos DDL](../../relational-databases/triggers/ddl-event-groups.md).  
  
 *event_group* también actúa como una macro, cuando finaliza la instrucción CREATE EVENT NOTIFICATION, al agregar los tipos de evento que cubre a la **sys.events** vista de catálogo.  
 ...
 **'** *broker_service* **'**  
 ...
 Especifica una instancia de service broker en el que *broker_service* se resuelve. El valor de un service broker específico puede obtenerse consultando la **service_broker_guid** columna de la **sys.databases** vista de catálogo. Use **'current database'** para especificar la instancia de service broker en la base de datos actual. **'current database'** es literal entre mayúsculas y minúsculas de una cadena.  
...
>   
>  Para copiar y ejecutar estos ejemplos, necesita reemplazar este GUID por uno de su equipo y su instancia de [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]. Como se explica en la sección argumentos anterior, puede adquirir el **'***broker_instance_specifier***'** consultando la columna de service_broker_guid de la sys.databases vista de catálogo.  
