...
[!INCLUDE[tsql-appliesto-ss2016-xxxx-asdw-pdw_md](../../includes/tsql-appliesto-ss2016-xxxx-asdw-pdw-md.md)]
 ...
 PolyBase es compatible con estos formatos de archivo:  
 ...
 *file_format_name*  
 ...
 RCFILE (en combinación con SERDE_METHOD = *SERDE_method*)  
...
-   FORMAT_TYPE = RCFILE, SERDE_METHOD = 'org.apache.hadoop.hive.serde2.columnar.ColumnarSerDe'  
 ...
 FIELD_TERMINATOR = *field_terminator*  
 Se aplica solo a los archivos de texto delimitado. Especifica uno o varios caracteres que marcan el final de cada campo (columna) en el archivo de texto delimitado. El valor predeterminado es el ꞌ de carácter de canalización | ꞌ. Para obtener soporte garantizada, se recomienda para usar uno o varios caracteres ascii.
...
PolyBase solo utiliza el formato de fecha personalizado para importar los datos. No utiliza el formato personalizado para escribir datos en un archivo externo.
 ...
 Cuando DATE_FORMAT no se especifica o es una cadena vacía, PolyBase utilizará los formatos predeterminados siguientes:  
...
-   Año, mes y día pueden tener una variedad de formatos y pedidos. La tabla solo muestra AMD. Mes puede tener 1 o 2 dígitos o 3 caracteres. Día puede tener 1 o 2 dígitos. Año puede tener 2 ó 4 dígitos.  
...
|---------------|-------------|-----------------|  
|DateTime|DATE_FORMAT = "aaaa-MM-dd fff"|Además de año, mes y día, esto incluye 00-24 horas, de 00 a 59 minutos, de 00 a 59 segundos y 3 dígitos para milisegundos.|  
|DateTime|DATE_FORMAT = 'aaaa-MM-dd hh:mm:ss.ffftt'|Además de año, mes y día, esto incluye 00-12 horas, de 00 a 59 minutos, de 00 a 59 segundos, 3 dígitos para milisegundos y AM, am, PM o pm.|  
|SmallDateTime|DATE_FORMAT = "aaaa-MM-dd hh: mm"|Además de año, mes y día, esto incluye 00-23 horas, de 00 a 59 minutos.|  
|SmallDateTime|DATE_FORMAT = 'aaaa-MM-dd hh:mmtt'|Además de año, mes y día, esto incluye 11: 00 horas, minutos 00-59, ningún segundos y a. M., am, PM o pm.|  
...
|datetime2|DATE_FORMAT = 'se ss aaaa-MM-dd. ' fffffff|Además de año, mes y día, esto incluye 00-23 horas, de 00 a 59 minutos, de 00 a 59 segundos y 7 dígitos de milisegundos.|  
|datetime2|DATE_FORMAT = 'aaaa-MM-dd hh:mm:ss.ffffffftt'|Además de año, mes y día, esto incluye 11: 00 horas, de 00 a 59 minutos, de 00 a 59 segundos, 7 dígitos para milisegundos y AM, am, PM o pm.|  
...
|DateTimeOffset|DATE_FORMAT = 'aaaa-MM-dd hh:mm:ss.ffffffftt zzz'|Además de año, mes y día, esto incluye 11: 00 horas, de 00 a 59 minutos, de 00 a 59 segundos, 7 dígitos para milisegundos, (a. M., am, PM o p.m.) y el desplazamiento de zona horaria. Vea la descripción de la fila anterior.|  
...
-   Las letras 'zzz' designan el desplazamiento de zona horaria para la zona de horaria actual del sistema en el formato {+ |-} HH:ss].  
 ...
 USE_TYPE_DEFAULT = {TRUE | **FALSE** }  
 Especifica cómo controlar los valores que faltan en archivos de texto delimitado cuando PolyBase recupera datos del archivo de texto.  
  
 TRUE  
 Cuando se recuperan datos del archivo de texto, almacenar cada valor que falta mediante el valor predeterminado para el tipo de datos de la columna correspondiente en la definición de tabla externa. Por ejemplo, reemplace un valor que falta con:  
...
-   1900-01-01 si la columna es una columna de fecha.  
 ...
 FALSE  
 Almacenar todos los valores que faltan como NULL. Los valores NULL que se almacenan utilizando la palabra NULL en el archivo de texto delimitado se importarán como la cadena 'NULL'.  
  
   Codificación = {'UTF8' | 'UTF16'}   
 En el almacén de datos de SQL Azure, PolyBase puede leer UTF8 y UTF16-LE codificado delimitado por archivos de texto. En SQL Server y PDW, PolyBase no admite leer UTF16 archivos codificados.
  
 DATA_COMPRESSION = *data_compression_method*  
 Especifica el método de compresión de datos para los datos externos. Cuando no se especifica DATA_COMPRESSION, el valor predeterminado es datos sin comprimir.  
...
-   COMPRESIÓN de datos = 'org.apache.hadoop.io.compress.SnappyCodec'  
 ...
 El tipo de formato de archivo PARQUET admite estos métodos de compresión:  
...
## <a name="general-remarks"></a>Notas generales  
 ...
 Cuando los datos se almacenan en uno de los formatos comprimidos, PolyBase en primer lugar descomprime los datos antes de devolver los registros de datos.  
  
## <a name="limitations-and-restrictions"></a>Limitaciones y restricciones   
  
 Se debe admitir el delimitador de filas en los archivos de texto delimitado por LineRecordReader de Hadoop es decir, debe ser '\r', '\n' o '\r\n'. Estas no son configurables por el usuario.  
...
### <a name="a-create-a-delimitedtext-external-file-format"></a>A. Crear un formato de archivo externo DELIMITEDTEXT  
 Este ejemplo crea un formato de archivo externo denominado *textdelimited1* para un archivo de texto delimitado. El FORMAT_OPTIONS especificar los campos en el archivo estarán separados por un carácter de canalización ' |'. También se comprime el archivo de texto con el códec de Gzip. Si no se especifica DATA_COMPRESSION, se descomprime el archivo de texto.  
