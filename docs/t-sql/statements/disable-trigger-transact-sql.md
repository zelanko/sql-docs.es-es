---
title: DISABLE TRIGGER (Transact-SQL) | Documentos de Microsoft
ms.custom: 
ms.date: 05/10/2017
...
## <a name="remarks"></a>Comentarios  
 De forma predeterminada, los desencadenadores se habilitan cuando se crean. Al deshabilitar un desencadenador no se quita. Sigue siendo un objeto de la base de datos actual. Sin embargo, el desencadenador no se activa cuando se da alguna [!INCLUDE[tsql](../../includes/tsql-md.md)] se ejecutan las instrucciones en el que se programó. Los desencadenadores se vuelven a habilitar mediante [ENABLE TRIGGER](../../t-sql/statements/enable-trigger-transact-sql.md). Desencadenadores DML definidos en tablas pueden ser también puede habilitar o deshabilitar mediante el uso de [ALTER TABLE](../../t-sql/statements/alter-table-transact-sql.md).  
  
 Cambiar el desencadenador mediante la **ALTER TRIGGER** instrucción habilita el desencadenador.  
