...
### <a name="e-using-merge-to-perform-insert-or-update-on-a-target-edge-table-in-a-graph-database"></a>E. Usar MERGE para realizar una operación INSERT o UPDATE en una tabla perimetral de destino en una base de datos de gráficos
En este ejemplo, se crean tablas de nodo `Person` y `City` y una tabla perimetral `livesIn`. Se usa la instrucción MERGE en la tabla perimetral `livesIn` para insertar una fila nueva si aún no existe el perímetro entre `Person` y `City`. Si ya existe el perímetro, simplemente se actualiza el atributo StreetAddress en la tabla perimetral `livesIn`.
