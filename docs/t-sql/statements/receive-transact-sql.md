---
title: RECIBIR (Transact-SQL) | Documentos de Microsoft
ms.custom: 
ms.date: 07/26/2017
 ...
 *database_name*  
 Nombre de la base de datos que contiene la cola desde la que se van a recibir los mensajes. Si no *nombre de base de datos* se proporciona, el valor predeterminado es la base de datos actual.  
  
 *schema_name*  
 Nombre del esquema al que pertenece la cola desde la que se van a recibir los mensajes. Si no *nombre de esquema* se proporciona, el valor predeterminado es el esquema predeterminado para el usuario actual.  
 ...
 *nombre_de_cola*  
 ...
 Especifica la conversación para los mensajes recibidos. El *identificador de conversación* proporcionado debe ser un **uniqueidentifier**, o un tipo que pueda convertirse en **uniqueidentifier**.  
  
 conversation_group_id = *conversation_group_id*  
 Especifica el grupo de conversación para los mensajes recibidos. El *Id. de grupo de conversación es* proporcionado debe ser un **uniqueidentifier**, o un tipo convertible en **uniqueidentifier**.  
...
>  Si la instrucción RECEIVE no es la primera instrucción de un lote o de un procedimiento almacenado, la instrucción anterior deberá finalizar con un punto y coma (;).  
 ...
 La instrucción RECEIVE lee los mensajes de una cola y devuelve un conjunto de resultados. El conjunto de resultados consta de cero o más filas, cada una de las cuales contiene un mensaje. Si no se utiliza la cláusula INTO, y *column_specifier* no asigna los valores a variables locales, la instrucción devuelve un conjunto de resultados para el programa que realiza la llamada.  
  
 Los mensajes devueltos por la instrucción RECEIVE pueden ser de distintos tipos de mensaje. Las aplicaciones pueden utilizar el **message_type_name** columna para enrutar cada mensaje al código que controla el tipo de mensaje asociado. Hay dos clases de tipos de mensaje:  
...
-   Mensajes de sistema de [!INCLUDE[ssSB](../../includes/sssb-md.md)] que devuelven información de estado o error.  
 ...
 La instrucción RECEIVE quita los mensajes recibidos de la cola a menos que la cola especifique la retención de mensajes. Si la configuración de retención de la cola es ON, la instrucción RECEIVE actualiza la **estado** columna **0** y deja los mensajes en la cola. Si se revierte una transacción que contiene la instrucción RECEIVE, también se revierten todos los cambios de la cola de la transacción y se devuelven los mensajes a la cola.  
...
-   Para una conversación determinada, una instrucción RECEIVE devuelve los mensajes en orden ascendente **message_sequence_number** orden.  
  
 La cláusula WHERE de la instrucción RECEIVE solo puede contener una condición de búsqueda que use **conversation_handle** o **conversation_group_id**. La condición de búsqueda no puede contener una o más columnas de las demás columnas de la cola. El **conversation_handle** o **conversation_group_id** no puede ser una expresión. El conjunto de mensajes que se devuelve depende de las condiciones que se especifican en la cláusula WHERE:  
  
-   Si **conversation_handle** se especifica, RECEIVE devuelve todos los mensajes de la conversación especificada que están disponibles en la cola.  
  
-   Si **conversation_group_id** se especifica, RECEIVE devuelve todos los mensajes que están disponibles en la cola de cualquier conversación que sea miembro del grupo de conversación especificado.  
...
|-----------------|---------------|-----------------|  
|**status**|**tinyint**|Estado del mensaje. Para los mensajes devueltos por el comando RECEIVE, el estado siempre es **0**. Los mensajes de la cola pueden contener uno de los siguiente valores:<br /><br /> **0**= listo**1**= mensaje recibido**2**= sin completar**3**= mensaje enviado retenido|  
|**prioridad**|**tinyint**|Nivel de prioridad de la conversación que se aplica al mensaje.|  
...
|**validación**|**nchar(2)**|Validación utilizada para el mensaje.<br /><br /> **E**= vacío**N**= None**X**= XML|  
